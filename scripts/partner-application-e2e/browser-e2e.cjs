...
(async () => {
  ...
  const ctx = await browser.newContext({ viewport: { width: 1280, height: 900 }, geolocation: { latitude: 10.7769, longitude: 106.7009 }, permissions: ["geolocation"] });
  ...
  check("multi-branch note shown", await page.getByText("chi nhánh đầu tiên", { exact: false }).first().isVisible());
  await page.getByRole("button", { name: /Tiếp tục/ }).click();
  ...
  await page.getByRole("button", { name: "Xem lại & gửi" }).click();
