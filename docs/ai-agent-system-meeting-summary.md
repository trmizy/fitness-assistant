...
## Addendum 2026-09-18 — Mở rộng năng lực sản phẩm: tạo lịch tập & kế hoạch dinh dưỡng độc lập qua chat

Sau khi nền tảng điều phối hội thoại được Codex chốt an toàn (CRITICAL 0/HIGH 0/MEDIUM 0), xây thêm 2 năng lực sản phẩm còn thiếu đã ghi nhận rõ ở addendum 2026-09-15 (v5): `CREATE_WORKOUT_PLAN` (tạo lịch tập độc lập) và `CREATE_NUTRITION_PLAN` (tạo kế hoạch dinh dưỡng độc lập qua chat). Cả hai đều XÂY THÊM trên nền tảng có sẵn — không sửa `agent-workflow/orchestrator.ts`, không thêm LLM, không đụng vào PT search/hire, Program v2, Claim Catalog hay memory provenance.

`CREATE_WORKOUT_PLAN`: chỉ hỏi 2 điều thật sự chưa biết (số buổi/tuần, thời lượng mỗi buổi), sau đó tái sử dụng đúng cơ chế `recommendation_engine.ts` mà tính năng "gán lịch tập vừa tạo" (SAVE_GENERATED_PLAN) đã có sẵn để sinh lịch tập, khớp từng bài tập với thư viện thật (không bao giờ lưu tên bài tập dạng chữ thô), rồi cho xem trước dạng thẻ có thể chỉnh sửa bằng lời (đổi bài, đổi số buổi/thời lượng, giảm nhẹ cường độ một ngày cụ thể, xin phương án khác) trước khi lưu thật vào lịch tập.

`CREATE_NUTRITION_PLAN` khó hơn về mặt kỹ thuật: việc tính thực đơn là một tác vụ nền (BullMQ, có thể mất 1-3 phút), trong khi mọi tin nhắn chat khác trong hệ thống đều xử lý ngay lập tức. Giải pháp: tái sử dụng ĐÚNG quy trình REST đã có (`POST /plans/nutrition/generate` → theo dõi trạng thái → `save-to-nutrition`), theo dõi tiến trình (đang tính/đã xong/lỗi) ngay trong bản ghi hành động chờ xác nhận có sẵn, hỏi lại ở lượt chat tiếp theo của người dùng — không cần thêm trạng thái mới cho bộ điều phối trung tâm dù trước đó đã có vài trạng thái dự phòng cho việc này. Phát hiện thêm 1 lỗi thật (không sửa, chỉ ghi nhận): tính năng "điều chỉnh" (adjust) sẵn có của trang web tự làm mất nội dung yêu cầu điều chỉnh của người dùng trước khi gửi cho AI (field `notes` không tồn tại trong schema xử lý tác vụ nền) — luồng chat mới không đi theo đường này mà dùng field `restrictions` thật sự có tác dụng.

Đã chứng minh bằng DB thật (10 test E2E mới + 2 test liên-luồng xác nhận không hỏi lại thông tin đã biết khi chuyển từ lộ trình sang lịch tập sang dinh dưỡng, và chuyển hướng sạch sang tìm PT khi đang có lịch tập dở dang): tổng 40/40 test qua (bao gồm toàn bộ test hồi quy cũ), build frontend/backend sạch. Chi tiết đầy đủ: `docs/standalone-workout-workflow-{audit,design,evaluation}.md`, `docs/standalone-nutrition-workflow-{audit,design,evaluation}.md`, `docs/ai-coach-product-capability-completion.md`.

## Addendum 2026-09-19 — Sửa lỗi sản phẩm sau đánh giá E2E độc lập của Codex (M1–M7, L1)

Codex chấm độc lập 2 năng lực mới (tạo lịch tập / kế hoạch dinh dưỡng độc lập) và trả về RETURN TO CLAUDE với 7 lỗi mức MEDIUM + 1 LOW (không có CRITICAL/HIGH; nền tảng điều phối vẫn được giữ nguyên). Lỗi đều là lỗi *ngữ nghĩa sản phẩm* mà 40 test trước không thấy vì test giả lập đúng chỗ lỗi nằm: (1) câu "không ăn cá" nói ở tin nhắn đầu bị mất khi AI hỏi số bữa; (2) dù đã loại cá, bước chọn món tất định vẫn thêm 28 món cá; (3) thực đơn dùng mức calo/macro chung chung (2200/165) trong khi hệ thống đã có mục tiêu chuẩn (1992/128) cho cùng người dùng; (4) bản nháp cũ "nuốt" tin nhắn của việc khác (đang chờ thực đơn, chuyển sang PT, câu hỏi tiếp theo về PT lại bị trả lời "thực đơn đang tính"); (5) cảnh báo chấn thương đọc sai trường nên không hiện; (6) "không có máy cable"/"không muốn deadlift" không hoạt động; (7) lịch xem trước Thứ 2/4/6 nhưng lưu vào DB thành Thứ 6/7/CN.

Đã sửa tất cả, mỗi lỗi có tái hiện + test chạm đúng lớp lỗi: loại trừ thực phẩm giờ được ÉP BUỘC ở bước chọn món của bộ xử lý (không chỉ gửi vào prompt AI; cụm không ép buộc được thì từ chối thay vì hứa suông); mục tiêu dinh dưỡng lấy từ mục tiêu đang hoạt động (hoặc công thức chuẩn của onboarding, qua endpoint chỉ-đọc mới) — AI chỉ chọn món; xác nhận lưu sẽ kiểm tra lại mục tiêu và nội dung đúng bản đã xem; chọn bản nháp nào nhận tin nhắn theo quy tắc rõ ràng (từ khóa lĩnh vực > bản nháp gần nhất chưa bị việc khác chen ngang > hỏi lại nếu mơ hồ); ngày tập xem trước = ngày lưu thật (chứng minh bằng DB thật: trước sửa Thứ 6/7/CN, sau sửa Thứ 2/4/6); nút "Để sau" đổi thành "Bỏ qua bản này" và thực sự hủy bản nháp trên server (không còn hiện "Đã lưu" sai). Vẫn giữ nguyên & nói rõ: lịch tập do AI tạo chỉ có CẢNH BÁO chấn thương/trình độ, không có bộ lọc cứng như khi chọn chương trình có sẵn; chưa chạy thật BullMQ + Ollama (nhà cung cấp cấu hình từ chối kết nối) và chưa kiểm tra trình duyệt thật. Chi tiết: `docs/ai-coach-product-remediation-1.md`.

## Addendum 2026-09-19 — Đóng 3 lỗi MEDIUM cuối cùng (Codex recheck)

Codex xác nhận M3–M7 đã đóng, còn 3 lỗi thật: (1) người dùng nói thêm "tôi cũng không ăn thịt bò" ở lượt giữa (khi AI còn đang hỏi số bữa) thì bị mất; (2) câu ghép "không ăn cá và không ăn cay" chỉ nhận cá, bỏ qua "cay" rồi vẫn tạo thực đơn; (3) bản nháp đã bấm "Bỏ qua" vẫn xác nhận lưu được từ thẻ cũ. Đã sửa cả ba mà không đụng bộ điều phối: ràng buộc thực phẩm được cộng dồn qua mọi lượt (gộp, không ghi đè); mọi mệnh đề loại trừ đều được xét độc lập — bất kỳ loại trừ cứng nào không ép buộc được đều bị từ chối rõ ràng, không tạo thực đơn; trạng thái CANCELLED là cuối cùng — chỉ hành động PENDING mới được thực thi (COMPLETED vẫn trả kết quả cũ, không ghi lặp). 103/103 test workflow+loại trừ+parser, evaluator nền tảng 30/30, test mục tiêu dinh dưỡng 20/20, PT/Program v2/memory 175/175, build sạch. Vẫn chưa chạy thật BullMQ/Ollama và hành trình trình duyệt có đăng nhập. Chi tiết: `docs/ai-coach-product-final-remediation.md`.

## Addendum 2026-09-19 — Đóng lỗi race xác nhận vs bỏ qua

Lỗi cuối Codex tìm ra: khi người dùng bấm "Lưu" và "Bỏ qua" gần như cùng lúc (2 tab, thẻ cũ), cả hai đều báo thành công — bỏ qua nói "chưa lưu gì" trong khi lưu vẫn hoàn tất. Đã sửa ở cơ sở dữ liệu: cả hai thao tác cạnh tranh một phép UPDATE có điều kiện duy nhất (`PENDING → EXECUTING` khi lưu, `PENDING → CANCELLED` khi bỏ qua), nên chỉ một bên thắng, bên thua trả lời đúng sự thật ("đang được lưu nên không thể bỏ qua"/"đã lưu trước đó"). Không giữ transaction qua lời gọi HTTP; nếu ghi thất bại thì trả bản nháp về PENDING (không còn ghi COMPLETED giả); claim treo quá 2 phút có thể lấy lại, an toàn nhờ idempotency phía dịch vụ. 118/118 test workflow (15 test race mới cho cả lịch tập và thực đơn), evaluator nền tảng 30/30, PT/Program/memory 175/175, build sạch. Chi tiết: `docs/ai-coach-finalization-race-closure.md`.
