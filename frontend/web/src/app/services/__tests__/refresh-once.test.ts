...
describe("makeRefreshOnce — concurrent refresh guard", () => {
  it("3 concurrent callers → fn called exactly once", async () => {
    ...
    assert.equal(
      callCount,
      1,
      "refresh fn must be called only once for concurrent callers",
    );
    assert.ok(
  ...
  it("refresh fail → _retry flag blocks a second refresh attempt", async () => {
    ...
    async function simulateInterceptor(
      req: Request,
    ): Promise<"session-cleared" | "rejected-no-refresh"> {
    ...
    assert.equal(
      callCount,
      callCountBefore,
      "no new refresh triggered for _retry=true requests",
    );
