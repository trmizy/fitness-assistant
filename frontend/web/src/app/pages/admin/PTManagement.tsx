...
function InfoRow({
  ...
  return (
    <div className="flex gap-3 text-sm py-1.5 border-b border-zinc-800/40 last:border-0">
      <span className="text-zinc-500 w-36 flex-shrink-0 text-xs mt-0.5">
        {label}
      </span>
      <span
        className={`${highlight ? "text-green-400 font-semibold" : "text-zinc-300"} font-medium text-xs`}
      >
        {value || "—"}
      </span>
    </div>
    ...
    <div className="flex items-center justify-between p-3 rounded-xl bg-zinc-800/30 border border-zinc-700/30">
      ...
      <span className="text-sm text-zinc-100 font-bold">
        {value != null ? (
          ...
          <span className="text-zinc-600 font-normal">Liên hệ</span>
        )}
      </span>
    </div>
  );
...
function DocThumb({
  ...
  return (
    ...
    >
        ...
        {fullUrl ? (
          <>
            ...
            <div className="relative z-10 text-center pointer-events-none bg-black/40 px-2 py-1 rounded-md backdrop-blur-xs">
              ...
              <p className="text-[10px] text-zinc-100 font-bold">
                Click to Expand
              </p>
            </div>
            {tag && (
              <span className="absolute top-2 right-2 text-[10px] bg-zinc-900/90 text-green-400 border border-green-500/30 px-1.5 py-0.5 rounded-md font-bold z-30">
                {tag}
              </span>
            )}
          </>
      ...
      <div
        className={`px-3 py-2 flex items-center justify-between ${url ? "bg-zinc-900" : "bg-red-500/5"}`}
      >
        <span className="text-[10px] font-bold text-zinc-300 truncate uppercase tracking-tighter">
          {label}
        </span>
        {url ? (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="bg-zinc-950 border border-zinc-800 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-zinc-100">
                Document Review — {app.user?.firstName} {app.user?.lastName}
              </h3>
              <p className="text-xs text-zinc-500">
                Submitted {new Date(app.updatedAt).toLocaleDateString()}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex border-b border-zinc-800/60 bg-zinc-900/60">
          {[
            ...
            <button
              ...
              className={`flex flex-1 items-center justify-center gap-1.5 px-2 py-2.5 text-[10px] font-bold border-b-2 transition-all min-w-0 ${
                section === t.key
            ...
            <>
              <div className="bg-zinc-900 rounded-xl border border-zinc-800/60 p-4">
                <p className="text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2">
                  Declared Citizen ID Number
                </p>
                <p
                  className={`text-base font-mono tracking-widest font-bold ${app.nationalIdNumber ? "text-green-400" : "text-red-400 italic"}`}
                >
              ...
              <div className="grid grid-cols-2 gap-3">
                <DocThumb
                  label="ID Card (Front)"
                  url={app.idCardFrontUrl}
                  tag="Front"
                />
                <DocThumb
                  label="ID Card (Back)"
                  url={app.idCardBackUrl}
                  tag="Back"
                />
              </div>
              <DocThumb
          ...
          {section === 2 && (
            <div className="space-y-4">
              {app.certificates?.map((c, i) => (
                <div
                  key={i}
                  className="bg-zinc-900 rounded-xl border border-zinc-800/60 p-4 space-y-3"
                >
          ...
          {section === 3 && (
            <div className="grid grid-cols-2 gap-3">
              {app.media
                ?.filter(
                  (m) =>
                    m.groupType === "PORTFOLIO" || m.mediaGroup === "PORTFOLIO",
                )
                .map((m, i) => (
                  <DocThumb
                    key={i}
        ...
        <div className="p-4 border-t border-zinc-800/60 flex justify-end">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-zinc-800 text-zinc-300 text-xs font-bold rounded-lg hover:bg-zinc-700"
          >
            Close Review
          </button>
        </div>
...
function DetailView({ app, onBack }: { app: App; onBack: () => void }) {
  ...
  return (
    <>
      {showDocs && (
        <DocumentViewer app={app} onClose={() => setShowDocs(false)} />
      )}
      <div className="p-4 md:p-6 max-w-6xl mx-auto space-y-6 animate-in fade-in duration-500">
        ...
        <div className="flex items-center gap-4">
          <button
            onClick={onBack}
            className="p-2.5 rounded-xl text-zinc-500 hover:text-zinc-200 hover:bg-zinc-800 border border-transparent hover:border-zinc-700 transition-all"
          >
          ...
          <div>
            <h1 className="text-xl font-bold text-zinc-100 flex items-center gap-3">
              ...
              <span
                className={`text-[10px] px-2.5 py-1 rounded-full font-bold border ${cfg.bg} ${cfg.text} ${cfg.border} uppercase tracking-wider`}
              >
                {cfg.label}
              </span>
            </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          ...
          <div className="space-y-6">
            <div className="bg-zinc-900 rounded-2xl border border-zinc-800/60 p-6 text-center shadow-xl">
              ...
              <div className="mt-6 pt-6 border-t border-zinc-800/60 space-y-3">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2.5 py-3 bg-green-500/10 hover:bg-green-500/20 border border-green-500/30 text-green-400 text-xs font-bold rounded-xl transition-all shadow-lg shadow-green-500/5"
                >
                ...
                <div className="flex items-center gap-3 p-3 rounded-xl bg-zinc-800/30 border border-zinc-700/30">
                  ...
                  <div className="text-left min-w-0">
                    <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-wider leading-none mb-1">
                      Phone
                    </p>
            ...
            <div className="bg-zinc-900 rounded-2xl border border-zinc-800/60 p-6 space-y-4 shadow-xl">
              ...
              <div className="space-y-4">
                <div>
                  <p className="text-[10px] text-zinc-600 font-bold uppercase mb-2">
                    Internal Note
                  </p>
                  <textarea
                    value={adminNote}
                    ...
                    rows={2}
                    className="w-full bg-zinc-950 border border-zinc-800 rounded-xl p-3 text-xs text-zinc-300 resize-none outline-none focus:border-green-500 transition-colors"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full bg-zinc-950 border border-zinc-800 rounded-xl p-3 text-xs text-zinc-300 resize-none outline-none focus:border-amber-500 transition-colors"
                ...
                <div className="grid grid-cols-2 gap-2 pt-2">
                  <button
                    ...
                    className="bg-green-500 hover:bg-green-400 text-black py-2.5 rounded-xl font-bold text-xs transition-all shadow-lg shadow-green-500/20"
                  >
                    Approve
                  </button>
                  <button
                    ...
                    className="bg-zinc-800 hover:bg-red-500/10 border border-zinc-700 hover:border-red-500/30 text-zinc-400 hover:text-red-400 py-2.5 rounded-xl font-bold text-xs transition-all"
                  >
                    Reject
                  </button>
                  <button
                    ...
                    className="col-span-1 bg-zinc-800 hover:bg-amber-500/10 border border-zinc-700 hover:border-amber-500/30 text-zinc-400 hover:text-amber-400 py-2.5 rounded-xl font-bold text-xs transition-all"
                  >
                    Need Info
                  </button>
                  <button
                    ...
                    className="col-span-1 bg-zinc-800 hover:bg-blue-500/10 border border-zinc-700 hover:border-blue-500/30 text-zinc-400 hover:text-blue-400 py-2.5 rounded-xl font-bold text-xs transition-all"
                  >
                    Investigate
                  </button>
                </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-zinc-900 rounded-2xl border border-zinc-800/60 p-6 space-y-6 shadow-xl">
              <div>
                <p className="text-xs font-bold text-zinc-500 uppercase flex items-center gap-2 mb-4">
                  <User className="w-4 h-4 text-green-500" /> Professional Bio &
                  Approach
                </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-zinc-800/40">
                <div>
                  ...
                  <p className="text-sm text-zinc-100 font-medium">
                    {app.educationBackground || "None provided"}
                  </p>
                </div>
                <div>
                  <p className="text-[10px] text-zinc-600 font-bold uppercase mb-2">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {app.mainSpecialties?.length > 0 ? (
                      app.mainSpecialties.map((s) => (
                        <span
                          key={s}
                          className="px-2.5 py-1 rounded-lg bg-green-500/10 text-green-400 text-[10px] font-bold border border-green-500/20 uppercase tracking-tight"
                        >
                          {s}
                        </span>
                      ))
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-zinc-900 rounded-2xl border border-zinc-800/60 p-6 space-y-5 shadow-xl">
                <p className="text-xs font-bold text-zinc-500 uppercase flex items-center gap-2">
                  <Calendar className="w-4 h-4 text-blue-400" /> Service &
                  Availability
                </p>
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-2">
                    <div className="p-3 rounded-xl bg-zinc-950/50 border border-zinc-800/40">
                      <p className="text-[10px] text-zinc-600 font-bold uppercase mb-1">
                        Exp.
                      </p>
                    ...
                    <div className="p-3 rounded-xl bg-zinc-950/50 border border-zinc-800/40">
                      ...
                      <p className="text-xs text-green-400 font-bold">
                        {app.serviceMode === "ONLINE"
                          ? "Online qua video call"
                          : app.serviceMode === "OFFLINE"
                            ? "Offline tại phòng gym"
                            : app.serviceMode === "HYBRID"
                  ...
                  <div className="pt-2">
                    ...
                    <div className="flex flex-wrap gap-1.5 overflow-hidden mb-3">
                      {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map(
                        (d) => {
                          const isAvailable =
                            app.availableDays?.includes(d) ||
                            app.availabilityBlocks?.some((b: any) =>
                              b.dayOfWeek?.startsWith(d.substring(0, 3)),
                            );
                          return (
                            <span
                              key={d}
                              className={`px-2 py-1 rounded-md text-[10px] font-bold border transition-colors ${
                                isAvailable
                                  ...
                                  : "bg-zinc-950 text-zinc-700 border-zinc-800/40"
                              }`}
                            >
                              {d}
                            </span>
                    ...
                    {app.availabilityBlocks &&
                    app.availabilityBlocks.length > 0 ? (
                      <div className="space-y-1.5">
                        {app.availabilityBlocks.map((b: any, i: number) => (
                          <div
                            key={i}
                            className="flex items-center gap-3 p-2 rounded-lg bg-zinc-950/50 border border-zinc-800/40"
                          >
                            <Clock className="w-3.5 h-3.5 text-green-400" />
                            <div className="text-xs text-zinc-300 font-medium">
                              <span className="text-zinc-500 w-10 inline-block">
                                {b.dayOfWeek}
                              </span>{" "}
                  ...
                  <div className="text-[10px] text-zinc-500 italic flex items-start gap-2 px-1">
                    ...
                    <span className="truncate">
                      {(() => {
                        const locs: any[] =
                          app.applicationTrainingLocations ?? [];
                        const names = locs
                          .map((l: any) => l.gymName)
                          .filter(Boolean);
                        if (names.length > 0) return names.join(", ");
                        if (app.operatingAreas?.length)
                          return app.operatingAreas.join(", ");
                        return "Online";
                      })()}
                    </span>
                  </div>
              ...
              <div className="bg-zinc-900 rounded-2xl border border-zinc-800/60 p-6 space-y-5 shadow-xl">
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {app.targetClientGroups?.length > 0 ? (
                        app.targetClientGroups.map((t) => (
                          <span
                            key={t}
                            className="px-2 py-0.5 rounded-md bg-blue-500/5 text-blue-400 text-[10px] border border-blue-500/10 font-medium"
                          >
                            {t}
                          </span>
                        ))
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {app.primaryTrainingGoals?.length > 0 ? (
                        app.primaryTrainingGoals.map((g) => (
                          <span
                            key={g}
                            className="px-2 py-0.5 rounded-md bg-purple-500/5 text-purple-400 text-[10px] border border-purple-500/10 font-medium"
                          >
                            {g}
                          </span>
                        ))
                  ...
                  <div className="pt-2 border-t border-zinc-800/40">
                    <p className="text-[10px] text-zinc-600 font-bold uppercase mb-1.5">
                      Methods
                    </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2.5">
                  ...
                  {app.socialLinks?.instagram && (
                    <SocialLink
                      ...
                      label="Instagram"
                      url={`https://instagram.com/${app.socialLinks.instagram}`}
                    />
                  )}
                  {app.socialLinks?.tiktok && (
                    <SocialLink
                      ...
                      label="TikTok"
                      url={`https://tiktok.com/@${app.socialLinks.tiktok}`}
                    />
                  )}
                  {!app.portfolioUrl &&
                    !app.linkedinUrl &&
              ...
              <div className="bg-zinc-900 rounded-2xl border border-zinc-800/60 p-6 space-y-5 shadow-xl">
                <p className="text-xs font-bold text-zinc-500 uppercase flex items-center gap-2">
                  <Briefcase className="w-4 h-4 text-amber-500" /> Service
                  Pricing
                </p>
                <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {(app.desiredSessionPrice ?? 0) > 0 && (
                          <PriceItem
                            label="Theo buổi"
                            value={app.desiredSessionPrice}
                          />
                        )}
                        {(app.packagePrice ?? 0) > 0 && (
                          <PriceItem
                            label={`Gói (${app.sessionsPerPackage || 10} buổi)`}
                            value={app.packagePrice}
                          />
                        )}
                        {(app.monthlyProgramPrice ?? 0) > 0 && (
                          <PriceItem
                            label="Gói tháng"
                            value={app.monthlyProgramPrice}
                          />
                        )}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {(app.onlinePricePerSession ?? 0) > 0 && (
                          <PriceItem
                            label="Theo buổi"
                            value={app.onlinePricePerSession}
                          />
                        )}
                        {(app.onlinePackagePrice ?? 0) > 0 && (
                          <PriceItem
                            label={`Gói (${app.sessionsPerPackage || 10} buổi)`}
                            value={app.onlinePackagePrice}
                          />
                        )}
                      </div>
                    </div>
                  ...
                  {((app.offlinePricePerSession ?? 0) > 0 ||
                    ...
                    <div>
                      <p className="text-[10px] text-orange-500 font-bold uppercase mb-2">
                        Offline tại phòng gym
                      </p>
                      <div className="space-y-2">
                        {(app.offlinePricePerSession ?? 0) > 0 && (
                          <PriceItem
                            label="Theo buổi"
                            value={app.offlinePricePerSession}
                          />
                        )}
                        {(app.offlinePackagePrice ?? 0) > 0 && (
                          <PriceItem
                            label={`Gói (${app.sessionsPerPackage || 10} buổi)`}
                            value={app.offlinePackagePrice}
                          />
                        )}
                      </div>
                    </div>
                  ...
                  {!(app.desiredSessionPrice ?? 0) &&
                    !(app.packagePrice ?? 0) &&
                    !(app.onlinePricePerSession ?? 0) &&
                    !(app.offlinePricePerSession ?? 0) && (
    ...
    <div className="p-4 md:p-6 max-w-6xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-500">
            Review and manage professional certifications
          </p>
        </div>
        <div className="relative">
          ...
          <input
            ...
            className="pl-10 pr-4 py-2 bg-zinc-900 border border-zinc-800 rounded-xl text-xs text-zinc-300 w-full md:w-64 focus:outline-none focus:border-green-500"
          />
        </div>
      </div>
          ...
          {filtered.map((app) => (
            ...
            >
              ...
              <div className="flex items-start justify-between relative z-10">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-zinc-200">
                      {app.user?.firstName || "N/A"} {app.user?.lastName || ""}
                    </h3>
                    <p className="text-xs text-zinc-500">
                ...
                <div
                  className={`w-2 h-2 rounded-full ${statusConfig[app.status || "DRAFT"].dot}`}
                />
              </div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3 relative z-10">
                <div className="flex items-center gap-2 text-[10px] text-zinc-500">
                  ...
                  {app.yearsOfExperience ? `${app.yearsOfExperience}y` : "?y"}{" "}
                  Exp
                </div>
                <div className="flex items-center gap-2 text-[10px] text-zinc-500">
                ...
                <div className="flex items-center gap-2 text-[10px] text-zinc-500">
                  <Award className="w-3 h-3" /> {app.certificates?.length || 0}{" "}
                  Certs
                </div>
                <div className="flex items-center gap-2 text-[10px] text-zinc-500">
                  <ImageIcon className="w-3 h-3" /> {app.media?.length || 0}{" "}
                  Media
                </div>
              </div>
              ...
              <div className="mt-4 flex items-center justify-between relative z-10">
                <span
                  className={`text-[10px] px-2 py-0.5 rounded-full font-bold bg-zinc-800 border border-zinc-700 ${statusConfig[app.status || "DRAFT"].text}`}
                >
