...
export function NutritionPage() {
  ...
  const weekStartStr = formatLocalDate(
  ...
  const calMonthEnd = formatLocalDate(
  ...
  const { data: monthlySummaryRaw, refetch: refetchMonthly } = useQuery({
    ...
    queryFn: () =>
      nutritionService.getMonthlySummary(calMonthStart, calMonthEnd),
  });
  ...
  const completeMealMutation = useMutation({
    mutationFn: ({
      ...
      nutritionService.upsertMealCompletion(mealId, dateStr, status, {
        percentConsumed: pct,
        overrideCalories,
        overrideProtein,
        overrideCarbs,
  ...
  const undoMealMutation = useMutation({
    mutationFn: (mealId: string) =>
      nutritionService.deleteMealCompletion(mealId, dateStr),
    onSuccess: () => {
      void refetchDailyTask();
      void refetchMonthly();
      void queryClient.invalidateQueries({
  ...
  const deletePlanMealMutation = useMutation({
    ...
    onError: (e: any) =>
      toast.error(e?.response?.data?.error || "Không thể xoá bữa ăn."),
  });

  const deactivateProgramMutation = useMutation({
    mutationFn: (programId: string) =>
      nutritionService.deactivateNutritionProgram(programId),
    onSuccess: (result) => {
      ...
      if (result.hadCompletedMeals) {
        toast.success(
          "Đã dừng kế hoạch. Lịch sử bữa ăn đã hoàn thành được giữ lại.",
        );
      } else {
    ...
    onError: (e: any) =>
      toast.error(e?.response?.data?.error || "Không thể dừng kế hoạch."),
  });
  ...
  const [selectedFoodForAdd, setSelectedFoodForAdd] = useState<any | null>(
    null,
  );
  ...
  const updateItemMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("Đã cập nhật lượng.");
    },
    onError: (e: any) =>
  ...
  const deleteItemMutation = useMutation({
    ...
    onError: (e: any) =>
      toast.error(e?.response?.data?.error || "Không thể xoá món."),
  });
  ...
  const addItemMutation = useMutation({
    ...
    onError: (e: any) =>
      toast.error(e?.response?.data?.error || "Không thể thêm món."),
  });
  ...
  const { data: searchedFoods = [] as FoodItem[], isFetching: searchingFood } =
    useQuery({
  ...
  const hasDailyTaskItems =
    Array.isArray((dailyTask as any)?.meals) &&
    (dailyTask as any).meals.some(
      (meal: any) => Array.isArray(meal.items) && meal.items.length > 0,
    );
  ...
  const pieTotal =
    Math.round(actualNutrition.protein * 4) +
    Math.round(actualNutrition.carbs * 4) +
    Math.round(actualNutrition.fat * 9);
  const pieData = [
  ...
  if (isToday && goal && todayLogs.length > 0) {
    ...
    if (calPct > 1.1) {
      feedback.push({
        type: "warning",
        text: "Bạn đã vượt mục tiêu calories hôm nay, nên cân nhắc giảm khẩu phần ở các bữa còn lại.",
      });
    }
    if (protPct < 0.8 && feedback.length < 2) {
      feedback.push({
        type: "warning",
        text: "Protein hôm nay còn thấp, bạn nên bổ sung thêm thực phẩm giàu protein.",
      });
    }
    if (feedback.length === 0 && calPct < 0.8) {
      feedback.push({
        type: "warning",
        text: "Bạn còn thiếu năng lượng so với mục tiêu hôm nay.",
      });
    }
    if (feedback.length < 2 && fatPct > 1.1) {
      feedback.push({
        type: "warning",
        text: "Lượng chất béo hôm nay hơi cao, nên ưu tiên món ít dầu mỡ hơn.",
      });
    }
    if (
      feedback.length === 0 &&
      ...
      feedback.push({
        type: "success",
        text: "Dinh dưỡng hôm nay khá ổn so với mục tiêu của bạn.",
      });
    }
  }
  ...
  const createMutation = useMutation({
    mutationFn: (payload: Record<string, unknown>) =>
      nutritionService.createLog(payload),
    onSuccess: () => {
      invalidateNutrition();
  ...
  function openDefaultAddModal() {
    const breakfastMeal = (dailyTask as any)?.meals?.find(
      (meal: any) => meal.mealType === "BREAKFAST",
    );
    if (breakfastMeal?.id) {
  ...
  function handleAddFood() {
    ...
    if (cal === 0 && prot === 0 && carbs === 0 && fats === 0) {
      toast.warning(
        "Thực phẩm này có giá trị dinh dưỡng = 0, kiểm tra lại nếu cần.",
      );
    }
  ...
  function handleUpdateLog() {
    ...
    if (
      isNaN(editForm.protein) ||
      editForm.protein < 0 ||
      isNaN(editForm.carbs) ||
      editForm.carbs < 0 ||
      isNaN(editForm.fat) ||
      editForm.fat < 0
    ) {
    ...
    <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-0.5">
            Theo dõi calories và macro hàng ngày
          </p>
        </div>
      ...
      <div className="bg-zinc-900 border border-zinc-800/60 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-1">
            Tạo và quản lý AI Nutrition Plan tại trang AI Plans. Trang này chỉ
              ...
              <>
                <div className="flex items-center justify-between text-xs mb-1">
                  ...
                  <span className="text-zinc-400 font-semibold">
                    {dailyTask.actualProgress.calories} /{" "}
                    {dailyTask.program?.dailyCaloriesTarget ??
                      dailyTask.day.totalCalories ??
                      0}{" "}
                    kcal
                  </span>
                </div>
                <div className="h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${pct(dailyTask.actualProgress.calories, dailyTask.program?.dailyCaloriesTarget ?? dailyTask.day.totalCalories ?? 2000)}%`,
                    }}
                  />
                </div>
              </>
          ...
          <div className="bg-zinc-900 border border-zinc-700/50 rounded-2xl w-full max-w-sm shadow-2xl p-5 space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-zinc-500 text-sm mt-1">
                  Xoá{" "}
                  <strong className="text-zinc-300">
                    {confirmDeleteMeal.mealName}
                  </strong>{" "}
                  khỏi kế hoạch dinh dưỡng hôm nay. Các bữa đã hoàn thành/đã ăn
                  một phần không thể xoá.
                </p>
              </div>
            </div>
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-2.5 border border-zinc-700/60 text-zinc-300 text-sm rounded-xl hover:bg-zinc-800 transition-colors"
              >
                Hủy
              </button>
              <button
                onClick={() => {
                  deletePlanMealMutation.mutate(confirmDeleteMeal.mealId);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="bg-zinc-900 border border-zinc-700/50 rounded-2xl w-full max-w-sm shadow-2xl p-5 space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2 bg-amber-500/10 border border-amber-500/20 rounded-xl p-3">
                      ...
                      <p className="text-amber-300 text-xs">
                        Kế hoạch này đã có bữa ăn được hoàn thành. Không thể xoá
                        hoàn toàn. Hệ thống sẽ <strong>dừng áp dụng</strong> và
                        lưu lại lịch sử bữa đã thực hiện.
                      </p>
                    </div>
                    <p className="text-zinc-500 text-xs">
                      Những bữa chưa thực hiện sẽ bị huỷ. Lịch sử ăn uống của
                      bạn vẫn được giữ nguyên.
                    </p>
                  </div>
                ) : (
                  <p className="text-zinc-500 text-sm">
                    Toàn bộ kế hoạch dinh dưỡng sẽ bị xoá. Hành động này không
                    thể hoàn tác.
                  </p>
                );
              })()}
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => {
                  deactivateProgramMutation.mutate(dailyTask.program!.id);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="bg-zinc-900 border border-zinc-700/50 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
              ...
              {selectedFoodForAdd && (
                <div className="bg-zinc-800/50 border border-zinc-700/40 rounded-xl p-3 space-y-3">
                  ...
                  <div className="text-xs text-zinc-500">
                    Per 100g: {selectedFoodForAdd.calories}kcal · P
                    {selectedFoodForAdd.protein}g · C{selectedFoodForAdd.carbs}g
                    · F{selectedFoodForAdd.fats ?? selectedFoodForAdd.fat ?? 0}g
                  </div>
                  <div className="flex items-center gap-3">
            ...
            <div className="px-4 pb-4 flex gap-2">
              ...
              <button
                onClick={() => {
                  if (selectedFoodForAdd && addItemMealId)
                    addItemMutation.mutate({
                      mealId: addItemMealId,
                      food: selectedFoodForAdd,
                      qty: Number(addItemQty) || 100,
                    });
                }}
      ...
      {partialMeal &&
        (() => {
          ...
          const prevCalPct = Math.round(
            (partialMeal.plannedCal * pctNum) / 100,
          );
          const prevProPct = +(
            (partialMeal.plannedProtein * pctNum) /
            100
          ).toFixed(1);
          const prevCrbPct = +(
            (partialMeal.plannedCarbs * pctNum) /
            100
          ).toFixed(1);
          const prevFatPct = +((partialMeal.plannedFat * pctNum) / 100).toFixed(
            1,
          );

          // Auto-compute pct equivalent for amount mode display
          const pctEquiv =
            partialMeal.plannedCal > 0
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
              <div className="bg-zinc-900 border border-zinc-700/50 rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden">
                ...
                <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800/60">
                  <div>
                    <h3 className="text-zinc-100 font-bold text-sm">
                      Ghi nhận lượng thực ăn
                    </h3>
                    <p className="text-xs text-zinc-500 mt-0.5">
                      {partialMeal.mealName} · {partialMeal.plannedCal} kcal
                      theo kế hoạch
                    </p>
                  </div>
                  <button
                  ...
                  {partialMode === "amount" && (
                    <div className="space-y-3">
                      <p className="text-xs text-zinc-500">
                        Nhập calories thực tế đã ăn. Hữu ích khi ăn thêm hoặc ăn
                        món khác ngoài kế hoạch.
                      </p>
                      ...
                      <div>
                        <label className="text-xs text-zinc-500 font-semibold mb-1.5 flex items-center justify-between">
                          <span>
                            Calories thực tế (kcal){" "}
                            <span className="text-red-400">*</span>
                          </span>
                          {calInput > 0 && (
                            <span
                              className={`text-[10px] font-bold ${pctEquiv > 100 ? "text-orange-400" : pctEquiv === 100 ? "text-green-400" : "text-amber-400"}`}
                            >
                              ≈ {pctEquiv}% kế hoạch
                            </span>
                          )}
                      ...
                      {/* Macros — optional */}
                      <div>
                        <label className="text-xs text-zinc-500 font-semibold mb-1.5 block">
                          Macros chi tiết (tùy chọn, đơn vị: g)
                        </label>
                          ...
                          {[
                            {
                              ...
                              val: partialPro,
                              set: setPartialPro,
                              ph: String(partialMeal.plannedProtein.toFixed(0)),
                              color: "focus:border-green-500/60",
                            },
                            {
                              label: "Carbs",
                              val: partialCarb,
                              set: setPartialCarb,
                              ph: String(partialMeal.plannedCarbs.toFixed(0)),
                              color: "focus:border-blue-500/60",
                            },
                            {
                              label: "Fat",
                              val: partialFatAmt,
                              set: setPartialFatAmt,
                              ph: String(partialMeal.plannedFat.toFixed(0)),
                              color: "focus:border-amber-500/60",
                            },
                          ].map((f) => (
                            <div key={f.label}>
                      ...
                      <div className="flex items-center gap-1.5 text-[10px] text-zinc-600">
                        ...
                        Nếu ăn thêm ngoài kế hoạch, nhập tổng cả phần đã ăn. VD:
                        kế hoạch 400 kcal + ăn thêm 200 kcal → nhập 600.
                      </div>
                    </div>
                  )}
        ...
        {[
          {
            label: "Calories",
            consumed: Math.round(
              dailyTask?.actualProgress?.calories ?? totals.calories,
            ),
            target:
              dailyTask?.program?.dailyCaloriesTarget ?? goal?.calories ?? 2000,
            unit: "kcal",
            color: "#f97316",
          ...
          },
          {
            label: "Protein",
            consumed: +(
              dailyTask?.actualProgress?.protein ?? totals.protein
            ).toFixed(1),
            target:
              dailyTask?.program?.proteinTargetGrams ?? goal?.protein ?? 150,
            unit: "g",
            color: "#22c55e",
          ...
          },
          {
            label: "Carbs",
            consumed: +(
              dailyTask?.actualProgress?.carbs ?? totals.carbs
            ).toFixed(1),
            target: dailyTask?.program?.carbTargetGrams ?? goal?.carbs ?? 200,
            unit: "g",
            color: "#60a5fa",
          ...
          {
            label: "Fat",
            consumed: +(dailyTask?.actualProgress?.fat ?? totals.fat).toFixed(
              1,
            ),
            target: dailyTask?.program?.fatTargetGrams ?? goal?.fat ?? 65,
            unit: "g",
            color: "#f59e0b",
        ...
        <div className="lg:col-span-2 space-y-3">
          {!hasVisibleMealItems && isToday && (
            <div className="bg-zinc-900 rounded-xl border border-zinc-800/60 p-12 text-center">
              ...
              <p className="text-zinc-500 text-sm">
                Bạn chưa log món nào cho ngày này
              </p>
              <button
                onClick={openDefaultAddModal}
          ...
          {MEAL_TYPES.map((meal) => {
            ...
            const planIsActiveForDate =
              dailyTask?.hasProgram &&
              !!(dailyTask as any).day &&
              !(dailyTask as any).outOfRange;
            ...
            const planMeal = planIsActiveForDate
              ? (dailyTask.meals as any[]).find(
                  (m) => m.mealType?.toUpperCase() === PLAN_MEAL_TYPE[meal],
                )
              : null;
            ...
            const tableItems = (
              planMeal?.items && Array.isArray(planMeal.items)
                ? planMeal.items
                : logItems
            ) as any[];
            const mealCal = tableItems.reduce(
              (s, item) => s + Number(item.calories ?? 0),
              0,
            );
            const mealProtein = tableItems.reduce(
              (s, item) => s + Number(item.proteinGrams ?? item.protein ?? 0),
              0,
            );
            const mealCarbs = tableItems.reduce(
              (s, item) => s + Number(item.carbGrams ?? item.carbs ?? 0),
              0,
            );
            const mealFat = tableItems.reduce(
              (s, item) =>
                s + Number(item.fatGrams ?? item.fat ?? item.fats ?? 0),
              0,
            );
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between px-4 py-3 gap-2 flex-wrap">
                  ...
                  >
                    <span className="text-xs text-zinc-600 w-12 font-mono flex-shrink-0">
                      {MEAL_TIMES[meal]}
                    </span>
                    <span className="text-sm font-bold text-zinc-200">
                      {MEAL_LABELS[meal]}
                    ...
                    {/* Status badge */}
                    {isCompleted && (
                      <span className="text-[10px] px-1.5 py-0.5 bg-green-500/15 text-green-400 border border-green-500/25 rounded-full shrink-0">
                        ✓ Hoàn thành
                      </span>
                    )}
                    {isPartial && (
                      <span className="text-[10px] px-1.5 py-0.5 bg-amber-500/15 text-amber-400 border border-amber-500/25 rounded-full shrink-0">
                        ½ {completion.percentConsumed}%
                      </span>
                    )}
                    {isSkipped && (
                      <span className="text-[10px] px-1.5 py-0.5 bg-zinc-700/40 text-zinc-500 border border-zinc-700/30 rounded-full shrink-0">
                        ⊘ Bỏ qua
                      </span>
                    )}
                      ...
                      <>
                        ...
                        <button
                          onClick={() => {
                            setPartialMeal({
                              mealId: planMeal.id,
                              mealName: MEAL_VI[meal],
                              plannedCal: Math.round(mealCal),
                              plannedProtein: mealProtein,
                              plannedCarbs: mealCarbs,
                              plannedFat: mealFat,
                            });
                            setPartialMode("pct");
                            setPartialPct("50");
                ...
                {false && expanded && planMeal && (
                  <div className="border-t border-zinc-800/40 bg-zinc-800/20 px-4 py-2.5">
                    ...
                    <div className="space-y-1">
                      {(planMeal.items as any[]).map(
                        (item: any, idx: number) => {
                          const isEditing = editingItemId === item.id;
                          return (
                            <div
                              key={item.id ?? idx}
                              className="group flex items-center gap-2 text-xs py-0.5"
                            >
                              ...
                              {isEditing ? (
                                <div className="flex items-center gap-1 shrink-0">
                                  ...
                                  <button
                                    onClick={() => {
                                      if (item.id && editingQty)
                                        updateItemMutation.mutate({
                                          itemId: item.id,
                                          quantity: Number(editingQty),
                                        });
                                    }}
                      ...
                      {(planMeal.items || []).length === 0 && (
                        <p className="text-xs text-zinc-600 italic">
                          Chưa có món nào. Bấm "Thêm món" để bổ sung.
                        </p>
                      )}
                    </div>
                  </div>
                            ...
                            {tableItems.map((log: any) => (
                              ...
                              >
                                ...
                                <td className="px-4 py-2.5 text-zinc-500">
                                  {log.sourceType === "PLAN_ITEM" &&
                                  editingItemId ===
                                    (log.programMealItemId || log.id) ? (
                                    <>
                                      <input
                                        type="number"
                                        min="1"
                                ...
                                <td className="px-4 py-2.5 text-green-400">
                                  {(log.proteinGrams ?? log.protein) != null
                                    ? `${log.proteinGrams ?? log.protein}g`
                                    : "-"}
                                </td>
                                <td className="px-4 py-2.5 text-blue-400">
                                  {(log.carbGrams ?? log.carbs) != null
                                    ? `${log.carbGrams ?? log.carbs}g`
                                    : "-"}
                                </td>
                                <td className="px-4 py-2.5 text-amber-400">
                                  {(log.fatGrams ?? log.fat ?? log.fats) != null
                                    ? `${log.fatGrams ?? log.fat ?? log.fats}g`
                                    : "-"}
                                </td>
                                <td className="px-4 py-2.5">
                                  ...
                                  {(isCompleted || isPartial) && (
                                    <span
                                      ...
                                      title="Không thể sửa/xóa sau khi bữa đã hoàn thành"
                                    >
                                      🔒
                                    </span>
                                  )}
          ...
          <div className="bg-zinc-900 rounded-xl p-4 border border-zinc-800/60">
            ...
            <div className="space-y-1.5 mt-1">
              {[
                {
                  name: "Protein",
                  value: Math.round(actualNutrition.protein * 4),
                  color: COLORS[0],
                },
                {
                  name: "Carbs",
                  value: Math.round(actualNutrition.carbs * 4),
                  color: COLORS[1],
                },
                {
                  name: "Fat",
                  value: Math.round(actualNutrition.fat * 9),
                  color: COLORS[2],
                },
              ].map((d) => (
                <div
                  ...
                  <>
                    {foodQuery.length === 0 && (
                      <p className="text-xs text-zinc-600 mt-2 text-center">
                        Nhập tên thực phẩm để tìm kiếm
                      </p>
                    )}
                    {foodQuery.length === 1 && (
                      <p className="text-xs text-zinc-600 mt-2 text-center">
                    ...
                    {translation.translated && debouncedQuery.length >= 2 && (
                      <div className="mt-1.5 space-y-0.5">
                        ...
                        <p className="text-xs text-zinc-600">
                          Đây là kết quả gần đúng từ dữ liệu USDA — chọn món
                          giống nhất với món bạn đã ăn.
                        </p>
                      </div>
                    )}
                  ...
                  {addPreview && (
                    <div className="grid grid-cols-4 gap-2 text-center text-xs">
                      {[
                        {
                          label: "Kcal",
                          val: addPreview.calories,
                          color: "text-orange-400",
                        },
                        {
                          label: "Protein",
                          val: addPreview.protein + "g",
                          color: "text-green-400",
                        },
                        {
                          label: "Carbs",
                          val: addPreview.carbs + "g",
                          color: "text-blue-400",
                        },
                        {
                          label: "Fat",
                          val: addPreview.fat + "g",
                          color: "text-amber-400",
                        },
                      ].map((m) => (
                        <div
                          key={m.label}
                          className="bg-zinc-900/60 rounded-lg py-2"
                        >
            ...
            <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditForm((f) => ({ ...f, foodName: e.target.value }))
                  }
              ...
              <div>
                ...
                />
                <p className="text-[10px] text-zinc-600 mt-1">
                  Khi thay đổi khối lượng, vui lòng điều chỉnh macro thủ công
                  bên dưới
                </p>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    <div key={key}>
                      <label
                        className={`text-xs ${color} mb-1.5 block font-semibold`}
                      >
                        {label}
                      </label>
                      <input
            ...
            <div className="p-5 space-y-3">
              {[
                ...
                <div key={key} className="flex items-center gap-3">
                  <label
                    className={`text-sm font-semibold ${color} w-32 flex-shrink-0`}
                  >
                    {label}
                  </label>
...
function NutritionCalendarGrid({
  ...
  const monthLabel = month.toLocaleString("vi-VN", {
    month: "long",
    year: "numeric",
  });
    ...
    <div className="bg-zinc-900 rounded-2xl border border-zinc-800/60 p-4">
      ...
      <div className="grid grid-cols-7 mb-1">
        {DAY_LABELS.map((d) => (
          <span
            key={d}
            className="text-[10px] text-zinc-600 text-center py-1 uppercase tracking-wider font-semibold"
          >
            {d}
          </span>
        ))}
          ...
          return (
            ...
            >
              ...
              {!isToday && status && status !== "pending" && (
                <span
                  className={`absolute bottom-1.5 w-1.5 h-1.5 rounded-full ${STATUS_DOT[status] ?? "bg-zinc-600"}`}
                />
              )}
            </button>
          );
