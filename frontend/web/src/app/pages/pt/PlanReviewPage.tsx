...
export function PlanReviewPage() {
  ...
  const reviewMutation = useMutation({
    mutationFn: ({ action }: { action: "APPROVE" | "REJECT" }) =>
      ptPlanReviewService.submitReview(selected.id, {
        action,
        note: ptNote.trim() || undefined,
      }),
    onSuccess: () => {
    ...
    onError: (err: any) => {
      alert(
        err?.response?.data?.error || "Lỗi khi gửi review. Vui lòng thử lại.",
      );
    },
  });
              ...
              <div className="bg-zinc-900 rounded-xl border border-zinc-800/60 p-4">
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                  <div>
                    <h3 className="text-zinc-100 flex items-center gap-2">
                      ...
                      {selected.name || selected.goal}
                    </h3>
                    <p className="text-sm text-zinc-500 mt-0.5">
                      Tạo ngày {formatDate(selected.createdAt)} &middot;{" "}
                      {selected.duration} tuần &middot; {selected.daysPerWeek}{" "}
                      buổi/tuần
                    </p>
                  </div>
                ...
                <div className="mt-3">
                  <label className="text-xs text-zinc-500 mb-1.5 flex items-center gap-1.5 uppercase tracking-wider font-semibold">
                    <MessageSquare className="w-3.5 h-3.5" /> Ghi chú cho client
                    (tùy chọn)
                  </label>
                  <textarea
                    value={ptNote}
                      ...
                      <div key={i} className="px-4 py-3">
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex-1">
                            <div className="text-xs font-bold text-zinc-300 mb-0.5">
                              {day.day}
                            </div>
