...
export function PTClientList() {
  ...
  const filtered = contracts.filter((c) => {
    const name =
      `${c.clientProfile?.firstName ?? ""} ${c.clientProfile?.lastName ?? ""}`.toLowerCase();
    const matchSearch =
      !search ||
      name.includes(search.toLowerCase()) ||
      c.packageName.toLowerCase().includes(search.toLowerCase());
    const matchFilter =
      filter === "All" || contractStatusLabel(c.status) === filter;
    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-5">
      <div>
        ...
        <p className="text-zinc-500 text-sm mt-0.5">
          {isLoading
            ? "Loading…"
            : `${contracts.length} coaching relationship${contracts.length !== 1 ? "s" : ""}`}
        </p>
      </div>
      ...
      {isError && (
        <div className="flex items-center gap-3 bg-red-500/10 border border-red-500/20 rounded-xl p-4">
          ...
          <div>
            ...
            <button
              onClick={() => refetch()}
              className="text-xs text-red-400 hover:text-red-300 mt-0.5 underline"
            >
              Try again
            </button>
          </div>
                  ...
                  return (
                    ...
                    >
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <div className="w-8 h-8 bg-green-500/15 border border-green-500/20 rounded-full flex items-center justify-center text-xs font-bold text-green-400">
                            {initials(
                              c.clientProfile?.firstName,
                              c.clientProfile?.lastName,
                            )}
                          </div>
                          <div>
                      ...
                      <td className="px-4 py-3 text-sm text-zinc-400">
                        {goalLabel(c.clientProfile?.goal)}
                      </td>
                      <td className="px-4 py-3">
                        <span
              ...
              return (
                ...
                >
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    <div>
                      <div className="text-zinc-600">Sessions</div>
                      <div className="text-zinc-300 font-semibold">
                        {c.usedSessions}/{c.totalSessions}
                      </div>
                    </div>
                    <div>
                      <div className="text-zinc-600">Progress</div>
                      <div
                        className={`font-bold ${progress >= 80 ? "text-amber-400" : "text-green-400"}`}
                      >
                        {progress}%
                      </div>
                    </div>
