                    ...
                    <div key={day} className={`flex items-center gap-3 p-3 rounded-lg border transition-all ${active ? "border-green-500/20 bg-green-500/5" : "border-zinc-800/60 bg-zinc-800/20"}`}>
                      ...
                      {active && (
                        <div className="flex items-center gap-2 ml-auto">
                          <input type="time" aria-label={`${DAY_LABELS[day]} start time`} value={editSlots[day].start} onChange={e => updateSlotTime(day, "start", e.target.value)} className="bg-zinc-800 border border-zinc-700/60 rounded px-2 py-1 text-xs text-zinc-300 outline-none" />
                          <span className="text-xs text-zinc-600">–</span>
                          <input type="time" aria-label={`${DAY_LABELS[day]} end time`} value={editSlots[day].end} onChange={e => updateSlotTime(day, "end", e.target.value)} className="bg-zinc-800 border border-zinc-700/60 rounded px-2 py-1 text-xs text-zinc-300 outline-none" />
                        </div>
                      )}
                    </div>
                  );
