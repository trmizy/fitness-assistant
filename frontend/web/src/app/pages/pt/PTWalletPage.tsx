...
export function PTWalletPage() {
  ...
  const { data: transactions = [], isLoading: txLoading } = useQuery<WalletLedgerEntry[]>({
    queryKey: ["pt-wallet-transactions"],
    queryFn: () => walletService.getPtTransactions(),
  });
  ...
  const withdrawMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setAmount("");
      setPayoutInfo("");
      queryClient.invalidateQueries({ queryKey: ["pt-withdrawals"] });
    },
    ...
    <div className="p-4 md:p-6 max-w-3xl mx-auto space-y-5">
      <div>
        <h1 className="text-zinc-100 flex items-center gap-2 text-xl font-bold">
          <WalletIcon className="w-5 h-5 text-green-400" /> Earnings Wallet
        </h1>
        <p className="text-zinc-500 text-sm mt-0.5">Revenue from your coaching contracts</p>
      </div>
      ...
      <div className="bg-gradient-to-br from-green-500/15 to-zinc-900 rounded-2xl border border-green-500/20 p-6">
        ...
        {walletLoading ? (
          ...
          <div data-testid="pt-wallet-available-balance" data-value={wallet?.availableBalance ?? "0"} className="text-3xl font-bold text-zinc-100">{formatVND(Number(wallet?.availableBalance ?? 0))}</div>
        )}
        <button
          data-testid="pt-request-withdrawal-toggle"
        ...
        {showForm && (
          <div className="mt-4 bg-zinc-900/60 border border-zinc-800/60 rounded-xl p-3.5 space-y-2">
            <input
              data-testid="pt-withdraw-amount-input"
              type="number"
              value={amount}
            ...
            />
            <input
              data-testid="pt-withdraw-payout-info-input"
              value={payoutInfo}
            ...
            <div className="flex justify-end gap-2 pt-1">
              ...
              <button
                data-testid="pt-withdraw-submit-button"
        ...
        {openRequests.length > 0 && (
          <div data-testid="pt-open-withdrawal-requests" className="mt-4 space-y-1.5">
            {openRequests.map((w) => (
              <div key={w.id} data-testid="pt-withdrawal-request-row" data-status={w.status} data-amount={w.amount} className="flex items-center justify-between text-xs bg-zinc-900/60 border border-zinc-800/60 rounded-lg px-3 py-2">
