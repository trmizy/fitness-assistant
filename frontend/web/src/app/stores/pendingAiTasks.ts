...
function getUserTasks(userId?: string): PendingAiTask[] {
  ...
  if (!taskCache.has(userId)) {
    ...
    taskCache.set(userId, Array.isArray(persisted) ? persisted.map(normalizeTask) : []);
  }
...
function getCoachSession(userId?: string): AiCoachSessionState {
  ...
  if (!coachCache.has(userId)) {
    const persisted = readJson<AiCoachSessionState | null>(coachStorageKey(userId), null);
    coachCache.set(userId, persisted ?? {
      messages: [],
      status: 'idle',
      activeTaskId: null,
      lastError: null,
...
function startPlanPolling(userId: string, taskId: string): void {
  ...
  const poll = async () => {
    ...
    try {
      ...
      const nextStatus: PendingAiTaskStatus = status.status === 'FAILED'
        ? 'FAILED'
      ...
      setUserTasks(userId, tasks.map((item) => (item.id === taskId ? nextTask : item)));
...
function upsertTask(userId: string, task: Omit<PendingAiTask, 'userId' | 'createdAt' | 'updatedAt'> & Partial<Pick<PendingAiTask, 'createdAt' | 'updatedAt'>>): PendingAiTask {
...
function updateTask(userId: string, taskId: string, patch: Partial<PendingAiTask>): void {
  const current = getUserTasks(userId);
  const next = current.map((item) => (item.id === taskId ? normalizeTask({ ...item, ...patch, updatedAt: nowIso() }) : item));
  setUserTasks(userId, next);
}
...
export function usePendingAiTasks(userId?: string) {
  ...
  return {
    ...
    upsertTask: (task: Omit<PendingAiTask, 'userId' | 'createdAt' | 'updatedAt'>) => userId ? upsertTask(userId, task) : null,
    updateTask: (taskId: string, patch: Partial<PendingAiTask>) => userId ? updateTask(userId, taskId, patch) : undefined,
    removeTask: (taskId: string) => userId ? removeTask(userId, taskId) : undefined,
  };
}
...
export function useAiCoachSession(userId?: string) {
  ...
  const sendQuestion = (question: string): string | null => {
    ...
    const userMessage: AiChatMessage = { id: Date.now(), from: 'user', text: question.trim(), time: 'Now' };
    const placeholderMessage: AiChatMessage = { id: `ai-${taskId}`, from: 'ai', text: 'AI đang trả lời...', time: 'Now' };
    ...
    const applyIfCurrent = (updater: (session: AiCoachSessionState) => AiCoachSessionState) => {
      const active = coachRequests.get(userId);
    ...
    const streamAvailable = typeof window !== 'undefined' && 'ReadableStream' in window;
    if (!streamAvailable) {
        ...
        .catch(() => {
          ...
          applyIfCurrent((sessionState) => ({
            ...
            messages: sessionState.messages.map((message) => (
              message.id === placeholderMessage.id ? { ...message, text: `⚠️ ${errorText}` } : message
            )),
            status: 'failed',
            activeTaskId: null,
            lastError: errorText,
    ...
    const cancelStream = coachService.chatStream(question.trim(), {
      onStatus: (status) => {
        applyIfCurrent((sessionState) => ({
          ...sessionState,
          messages: sessionState.messages.map((message) => (
            message.id === placeholderMessage.id ? { ...message, text: status } : message
          )),
          updatedAt: nowIso(),
        }));
      },
      ...
      onError: (message) => {
        ...
        applyIfCurrent((sessionState) => ({
          ...
          messages: sessionState.messages.map((item) => (
            item.id === placeholderMessage.id ? { ...item, text: `⚠️ ${errorText}` } : item
          )),
          status: 'failed',
          activeTaskId: null,
          lastError: errorText,
...
export function enqueuePlanTask(userId: string, input: {
  jobId: string;
  planId: string;
