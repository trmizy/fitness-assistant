...
import { loadServerOverride } from "../config/serverUrl";
...
export async function bootstrapSession(): Promise<BootstrapResult> {
  try {
    // MUST come first. Web read the stored server address synchronously from localStorage while
    // api.ts was still being imported; AsyncStorage has no synchronous read, so the saved address
    // is loaded here instead — before the first request below can go out against the default one.
