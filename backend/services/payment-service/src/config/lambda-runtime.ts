...
function buildDatabaseUrl(secret: DatabaseSecret): string {
  const url = new URL(`postgresql://${secret.host}:${secret.port}/${secret.database}`);
...
export async function ensureDatabaseUrlConfigured(): Promise<void> {
  ...
  logger.info(
    ...
    "Payment Lambda database configuration loaded from Secrets Manager",
  );
}
...
export function validateRequiredRuntimeConfig(): void {
  ...
  if (problems.length > 0) {
    throw new Error(
      `${problems.join(", ")} must be configured before starting payment-service`,
    );
  }
}
