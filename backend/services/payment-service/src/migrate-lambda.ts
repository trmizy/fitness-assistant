...
const PAYMENT_DATABASE_NAME = "fitness_assistant_payment";
const REFUSAL_MESSAGE = "Refusing to run Payment Service migrations against non-payment database.";
...
function assertPaymentDatabase(database: string): void {
  if (database !== PAYMENT_DATABASE_NAME) {
...
async function loadDatabaseSecret(): Promise<DatabaseSecret> {
  ...
  if (!secretId) {
    throw new Error("DATABASE_SECRET_ID is required for Payment Service migration Lambda.");
  }
  ...
  assertPaymentDatabase(database);
...
async function ensurePaymentDatabaseExists(secret: DatabaseSecret): Promise<boolean> {
  assertPaymentDatabase(secret.database);
  ...
  try {
    ...
    await maintenanceClient.query(`CREATE DATABASE ${quotePostgresIdentifier(PAYMENT_DATABASE_NAME)}`);
    return true;
  } finally {
...
export async function handler(): Promise<MigrationResult> {
  ...
  const databaseCreated = await ensurePaymentDatabaseExists(secret);
  ...
  return {
    status: "ok",
    database: PAYMENT_DATABASE_NAME,
    databaseCreated,
