...
const ZIP_OUT = path.join(SERVICE_DIR, "artifacts", "payment-migrate-lambda.zip");
const TMP_ROOT = path.join(os.tmpdir(), "payment-migrate-lambda-build");
...
function main() {
  ...
  runOptional("pnpm", ["--filter", "@gym-coach/payment-service", "db:generate"], REPO_ROOT);

  console.log("== 3: build payment-service ==");
  fs.rmSync(DIST_DIR, { recursive: true, force: true });
  run("pnpm", ["--filter", "@gym-coach/payment-service", "build"], REPO_ROOT);
  ...
  run("pnpm", ["--filter", "@gym-coach/payment-service", "deploy", DEPLOY_DIR, "--prod"], REPO_ROOT);
