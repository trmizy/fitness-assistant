...
const EVIDENCE_QUERY =
  process.env.KNOWLEDGE_RAG_TEST_QUERY ||
...
const CHAT_QUERY =
  process.env.KNOWLEDGE_RAG_TEST_CHAT_QUERY ||
...
async function main(): Promise<void> {
  const [{ getQdrantClient }, { llmService }, { retriever }] =
    await Promise.all([
  ...
  try {
    ...
    throw new Error(
      `Qdrant collection unavailable for RAG test: ${COLLECTION}. Start Qdrant and run knowledge ingestion first. ${message}`,
    );
  }
  ...
  if (pointCount <= 0) {
    throw new Error(
      `${COLLECTION} exists but has no points. Run knowledge:pipeline with embedding first.`,
    );
  }
  ...
  if (evidenceDocs.length === 0) {
    ...
    throw new Error(
      `Retriever returned no evidence docs. Raw Qdrant matches: ${JSON.stringify(debug)}`,
    );
  }
  ...
  if (invalidEvidence) {
    throw new Error(
      `Evidence doc is missing expected metadata: ${JSON.stringify(docSummary(invalidEvidence))}`,
    );
  }

  const chatResult = await retriever.retrieveForChat(CHAT_QUERY);
  const chatEvidenceDocs = chatResult.documents.filter(
    (doc) => doc.source === `qdrant:${COLLECTION}`,
  );
  if (chatEvidenceDocs.length === 0) {
    throw new Error(
      `Chat retrieval did not include ${COLLECTION} evidence for the BIA query.`,
    );
  }
