...
async function main(): Promise<void> {
  ...
  const records = deduplicateResearchRecords(
    readNormalizedJsonl(normalizedPath),
  );
  const withTitle = records.filter((record) => record.title);
  const withUrl = records.filter((record) => record.source_url);
  const withStableId = records.filter(
    (record) => record.doi || record.pmid || record.external_id,
  );
  const highScore = records.filter(
    (record) => (record.evidence_score ?? 0) >= 0.65,
  );
  const topics = Array.from(
    new Set(records.map((record) => record.topic)),
  ).sort();
    ...
    JSON.stringify(
      {
        ...
        coverage: {
          ...
          highEvidenceScore: records.length
            ? highScore.length / records.length
            : 0,
        },
        topics,
        note:
          records.length > 0
            ? "Run ai:eval:retrieval after indexing to measure Qdrant retrieval quality."
            : "No normalized research records found; run knowledge:research:fetch first.",
      },
      null,
      2,
    ),
