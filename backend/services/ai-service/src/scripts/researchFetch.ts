...
import { connectorTimeoutMs, researchUserAgent } from '../knowledge/connectors/connector.interface';
...
import { appendReviewQueue, toReviewQueueRecord } from '../knowledge/pipeline/review_queue';
...
async function main(): Promise<void> {
  ...
  const contactEmail = process.env.RESEARCH_CONTACT_EMAIL || process.env.CROSSREF_MAILTO;
  ...
  for (const topic of topics) {
    const sourceIds = topic.source_preferences.filter((sourceId) => sources.some((source) => source.id === sourceId));
  ...
  const normalized = deduplicateResearchRecords(rawRecords.map(normalizeResearchRecord).map(scoreEvidence));
  const rawPath = path.resolve(process.cwd(), '../../../data/research/raw/latest.jsonl');
  const normalizedPath = path.resolve(process.cwd(), '../../../data/research/normalized/latest.jsonl');
