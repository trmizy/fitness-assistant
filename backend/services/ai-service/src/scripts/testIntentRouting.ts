...
function assertNutritionFirst(
  ...
  assert.equal(
    nutrition.enabled,
    true,
    `${question}: expected nutrition lookup`,
  );
...
function assertWorkoutOnly(question: string, expectedDate: string) {
  ...
  assert.equal(
    nutrition.enabled,
    false,
    `${question}: should not be nutrition`,
  );
...
assert.equal(
  workoutFollowUpNutrition.enabled,
  false,
  "workout context should not inherit nutrition intent",
);
const workoutFollowUp = detectWorkoutScheduleIntent("ngày mai", workoutHistory);
assert.equal(
  workoutFollowUp.enabled,
  true,
...
assert.equal(
  workoutScheduleSourceLabel("scheduled_session"),
  "Nhật ký tập luyện",
);
assert.equal(
  workoutScheduleSourceLabel("active_workout_program"),
  "Lịch tập đã lưu",
);
...
console.log(
  "PASS: intent routing keeps nutrition lookup ahead of workout schedule and labels sources in Vietnamese.",
);
