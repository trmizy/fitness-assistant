...
export const researchSources: ResearchSource[] = [
  {
    ...
    topics: ['training', 'nutrition', 'recovery', 'body_composition', 'supplements'],
    trust_level: 'high',
    rate_limit_per_minute: 30,
    requires_api_key: false,
    robots_policy_required: false,
    allowed_content: 'abstract',
    notes: 'Uses NCBI E-utilities metadata and abstracts. API key optional via PUBMED_API_KEY.',
  },
  {
    id: 'pmc',
    ...
    notes: 'Only open access content should be used. Connector stores metadata/abstract unless license is clear.',
  },
  {
    id: 'crossref',
  ...
  },
  {
    ...
    robots_policy_required: false,
    allowed_content: 'metadata_only',
    notes: 'Metadata and abstract inverted index when available. Set RESEARCH_CONTACT_EMAIL for polite pool.',
  },
  {
  ...
  },
  {
    ...
    allowed_content: 'metadata_only',
    notes: 'Disabled by default. Requires explicit allowlist and robots.txt check.',
  },
];
...
export function validateSourceRegistry(sources: ResearchSource[] = researchSources): string[] {
  const errors: string[] = [];
  ...
  for (const source of sources) {
    ...
    if (source.type === 'webpage' && !source.robots_policy_required) errors.push(`webpage_requires_robots_policy:${source.id}`);
    if (source.rate_limit_per_minute < 0) errors.push(`negative_rate_limit:${source.id}`);
  }
  return errors;
}
