...
function extractArticle(
  ...
  const abstract = Array.isArray(abstractParts)
    ? abstractParts
        .map((part: any) => readText(part?._ || part))
        .filter(Boolean)
        .join(" ")
  ...
  const doiItem = Array.isArray(article?.ELocationID)
    ? article.ELocationID.find(
        (entry: any) => String(entry?.EIdType || "").toLowerCase() === "doi",
      )
    : undefined;
  const doi = readText(doiItem?._ || doiItem);
  const authors = Array.isArray(article?.AuthorList?.Author)
    ? article.AuthorList.Author.map((author: any) =>
        [author.ForeName, author.LastName].filter(Boolean).join(" "),
      ).filter(Boolean)
    : [];
...
export const pubmedConnector: ResearchConnector = {
  ...
  async fetch(
    ...
    const term = [
      options.topic.query,
      options.topic.min_year ? `${options.topic.min_year}:3000[pdat]` : "",
    ]
      .filter(Boolean)
      .join(" AND ");
    ...
    return list
      .map((item) => extractArticle(item, options.topic.id, fetchedAt))
      .filter((item): item is NormalizedResearchRecord => Boolean(item));
  },
};
