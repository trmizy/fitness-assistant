  ...
  async fetch(options: ResearchConnectorFetchOptions): Promise<NormalizedResearchRecord[]> {
    const response = await axios.get(`${options.source.base_url}/works`, {
      ...
      params: {
        ...
        ...(options.topic.min_year ? { filter: `from_publication_date:${options.topic.min_year}-01-01` } : {}),
    ...
    const records: Array<NormalizedResearchRecord | null> = items.map((item) => {
      ...
      return {
        ...
        doi,
        authors: Array.isArray(item.authorships) ? item.authorships.slice(0, 8).map((entry: any) => entry.author?.display_name).filter(Boolean) : [],
