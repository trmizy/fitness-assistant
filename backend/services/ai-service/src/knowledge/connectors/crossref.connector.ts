  ...
  async fetch(options: ResearchConnectorFetchOptions): Promise<NormalizedResearchRecord[]> {
    ...
    const response = await axios.get(`${options.source.base_url}/works`, {
      ...
      params: {
        ...
        ...(options.topic.min_year ? { filter: `from-pub-date:${options.topic.min_year}` } : {}),
        ...(mailto ? { mailto } : {}),
      },
    });
    const fetchedAt = new Date().toISOString();
    const items: any[] = response.data?.message?.items ?? [];
    const records: Array<NormalizedResearchRecord | null> = items.map((item) => {
      ...
      const abstractText = typeof item.abstract === 'string' ? item.abstract.replace(/<[^>]+>/g, ' ') : '';
      ...
      return {
        ...
        doi,
        authors: Array.isArray(item.author) ? item.author.slice(0, 8).map((author: any) => [author.given, author.family].filter(Boolean).join(' ')).filter(Boolean) : [],
    ...
    return records.filter((item): item is NormalizedResearchRecord => item !== null);
  },
};
