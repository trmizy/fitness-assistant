...
export function chunkResearchRecord(record: NormalizedResearchRecord, maxChars = 1200): ResearchChunk[] {
  ...
  for (let offset = 0; offset < text.length; offset += maxChars) {
    ...
    const chunkId = `${record.external_id}:${chunks.length}`.replace(/[^a-zA-Z0-9:_-]/g, '_');
    chunks.push({
