...
export function toReviewQueueRecord(
  record: NormalizedResearchRecord,
): ReviewQueueRecord {
  return {
    status: shouldRequireReview(record) ? "pending" : "approved",
    reason: shouldRequireReview(record)
      ? "requires_manual_review"
      : "metadata_complete_auto_approved",
...
export function readApprovedReviewRecords(
  filePath = REVIEW_QUEUE_PATH,
  ...
  return fs
    ...
    .filter(
      (record) => record.status === "approved" || record.status === "indexed",
    )
