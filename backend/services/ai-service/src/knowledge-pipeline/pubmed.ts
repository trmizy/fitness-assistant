...
function nodeText(value: unknown): string {
  ...
  if (typeof value === 'object') {
    ...
    return nodeText(record.text ?? record['#text'] ?? Object.values(record).join(' '));
  }
  return '';
}
...
async function searchPubMedIds(query: string, limit: number): Promise<string[]> {
  const response = await axios.get(`${KNOWLEDGE_PIPELINE.pubMedBaseUrl}/esearch.fcgi`, {
    params: {
      db: 'pubmed',
      term: query,
      sort: 'pub+date',
...
async function fetchPubMedXml(ids: string[]): Promise<string> {
  const response = await axios.get(`${KNOWLEDGE_PIPELINE.pubMedBaseUrl}/efetch.fcgi`, {
    params: {
      db: 'pubmed',
      id: ids.join(','),
      retmode: 'xml',
...
function extractArticleIds(article: Record<string, unknown>): Record<string, string> {
...
function extractTags(medline: Record<string, unknown>, article: Record<string, unknown>): string[] {
  ...
  const pubTypes = asArray((article.PublicationTypeList as any)?.PublicationType)
    .map(nodeText)
    .filter(Boolean);
...
function extractPubDate(article: Record<string, unknown>): Date | null {
  ...
  const month = Number.isFinite(monthNumber)
    ? monthNumber - 1
    : Math.max(0, monthNames.findIndex((name) => monthRaw.toLowerCase().startsWith(name)));
  return new Date(Date.UTC(year, month >= 0 ? month : 0, day));
}

export function parsePubMedDocumentsFromXml(xml: string): RawKnowledgeDocument[] {
  ...
  return articles.flatMap((entry): RawKnowledgeDocument[] => {
    ...
    const medline = (articleRoot.MedlineCitation || {}) as Record<string, unknown>;
    const article = ((medline.Article || {}) as Record<string, unknown>);
    const pmid = nodeText(medline.PMID).trim();
    const title = firstNonEmpty([article.ArticleTitle]) ?? `PubMed article ${pmid}`;
    ...
    const cleanText = [title, journal ? `Journal: ${journal}` : '', abstract].filter(Boolean).join('\n');
...
export async function fetchPubMedDocuments(options: PubMedFetchOptions): Promise<RawKnowledgeDocument[]> {
