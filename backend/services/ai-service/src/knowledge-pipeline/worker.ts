...
import { runLocalEvidencePipeline, runPubMedPipeline, runRssPipeline, runWebPipeline } from './service';
...
export function createKnowledgePipelineWorker() {
  const worker = new Worker<LocalEvidencePipelineOptions | PubMedPipelineOptions | RssPipelineOptions | WebPipelineOptions>(
    KNOWLEDGE_PIPELINE.queueName,
    async (job) => {
      if (job.name === 'local-evidence-refresh') {
        return runLocalEvidencePipeline(job.data as LocalEvidencePipelineOptions);
      }
