...
export type KnowledgePipelineJobName =
  | "local-evidence-refresh"
  | "pubmed-refresh"
  | "rss-refresh"
  | "web-refresh";
export type KnowledgePipelineJobData =
  | LocalEvidencePipelineOptions
  | PubMedPipelineOptions
  | RssPipelineOptions
  | WebPipelineOptions;

let knowledgeQueueInstance: Queue<
  KnowledgePipelineJobData,
  unknown,
  KnowledgePipelineJobName
> | null = null;

export function getKnowledgePipelineQueue(): Queue<
  KnowledgePipelineJobData,
  unknown,
  KnowledgePipelineJobName
> {
  if (!knowledgeQueueInstance) {
    knowledgeQueueInstance = new Queue<
      KnowledgePipelineJobData,
      unknown,
      KnowledgePipelineJobName
    >(KNOWLEDGE_PIPELINE.queueName, { connection: redisConnection });
  }
  return knowledgeQueueInstance;
}

export async function enqueueLocalEvidenceRefresh(
...
export async function enqueuePubMedRefresh(
...
export async function scheduleKnowledgeRefreshes(): Promise<
  Array<{ name: KnowledgePipelineJobName; jobId: string | undefined }>
> {
  ...
  const pubmedJob = await queue.add(
    ...
    {
      embed: true,
      limit: Number.parseInt(process.env.KNOWLEDGE_PUBMED_LIMIT || "10", 10),
    },
    {
...
export async function removeKnowledgeRefreshSchedules(): Promise<number> {
  ...
  const targets = repeatable.filter(
    (job) =>
...
export async function updateKnowledgeQueueDepthMetrics(): Promise<
