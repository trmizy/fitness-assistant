...
function repoDataRoot(): string {
  const candidates = [
    process.env.KNOWLEDGE_DATA_ROOT ? path.resolve(process.cwd(), process.env.KNOWLEDGE_DATA_ROOT) : null,
    path.resolve(process.cwd(), 'data'),
    path.resolve(process.cwd(), '..', '..', '..', 'data'),
    path.resolve(__dirname, '..', '..', '..', '..', 'data'),
  ].filter((candidate): candidate is string => Boolean(candidate));
  const existing = candidates.find((candidate) => fs.existsSync(path.join(candidate, 'processed', 'evidence')));
  if (!existing) {
    throw new Error(`Cannot locate processed evidence directory. Checked: ${candidates.map((candidate) => path.join(candidate, 'processed', 'evidence')).join(', ')}`);
  }
  return existing;
}
...
export function listLocalEvidenceDocuments(limit?: number): RawKnowledgeDocument[] {
  ...
  for (const file of files) {
    ...
    for (const chunk of chunks) {
      ...
      const chunkId = String(chunk.id || `${file}:${chunk.chunk_index ?? docs.length}`);
      const url = String(chunk.source_url || `${KNOWLEDGE_PIPELINE.localEvidenceBaseUrl}/${file}#${chunkId}`);
