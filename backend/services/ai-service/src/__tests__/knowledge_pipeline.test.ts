...
import {
  isAllowedByRobots,
  parseWebDocumentFromHtml,
} from "../knowledge-pipeline/web";
import type { RawKnowledgeDocument } from "../knowledge-pipeline/types";

function sampleDoc(
  overrides: Partial<RawKnowledgeDocument> = {},
  ...
  return {
    ...
    cleanText:
      "Protein intake for physically active individuals is commonly described in g/kg/day. Resistance training and total daily protein are key drivers of muscle gain.",
...
describe("knowledge pipeline pure helpers", () => {
  ...
  it("registers local evidence as a tier-1 source", () => {
    const local = DEFAULT_KNOWLEDGE_SOURCES.find(
      (source) => source.id === "source-local-evidence",
    );
  ...
  it("chunks long text with overlap-friendly boundaries", () => {
    const text = Array.from(
      { length: 80 },
      (_, i) => `Sentence ${i} about resistance training.`,
    ).join(" ");
    ...
    assert.equal(
      classifyTopic(
        sampleDoc({
          ...
          cleanText:
            "Clinical practice guideline for bioelectrical impedance analysis, body composition, fat-free mass, fat mass, and total body water measurement conditions.",
  ...
  it("detects unsafe medical or PED advice", () => {
    const hit = detectSafetyIssue(
      "Use clenbuterol and anabolic steroid cycles for fast fat loss.",
    );
  ...
  it("rejects unsafe processed documents", () => {
    const processed = processKnowledgeDocument(
      sampleDoc({
        cleanText:
          "Stop taking medication and use anabolic steroid cycles to cure disease.",
      }),
      1,
    );
  ...
  it("applies basic robots allow/disallow precedence", () => {
    assert.equal(
      isAllowedByRobots("https://example.com/private/page", {
        allow: [],
        disallow: ["/private"],
      }),
      false,
    ...
    assert.equal(
      isAllowedByRobots("https://example.com/private/public/page", {
  ...
  it("parses LLM safety judge JSON safely", () => {
    const parsed = parseSafetyJudgeResponse(
      '{"safe":false,"reason":"unsafe supplement dosage","category":"supplement"}',
    );
