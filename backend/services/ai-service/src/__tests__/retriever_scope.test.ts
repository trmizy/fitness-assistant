...
describe('retriever collection scopes', () => {
  ...
  it('keeps AI Plan generation evidence scope away from Qdrant exercises', () => {
    assert.deepEqual([...RETRIEVER_COLLECTION_SCOPES.planEvidence], ['fitness_evidence']);
    assert.equal(RETRIEVER_COLLECTION_SCOPES.planEvidence.includes('exercises' as never), false);
  });
});
