...
import { researchSources, validateSourceRegistry } from '../knowledge/source_registry';
...
import { shouldRequireReview, toReviewQueueRecord } from '../knowledge/pipeline/review_queue';
...
function record(overrides: Partial<NormalizedResearchRecord> = {}): NormalizedResearchRecord {
  return {
    external_id: 'pmid:1',
    title: 'Resistance training volume systematic review',
    abstract_or_summary: 'This systematic review summarizes resistance training volume and hypertrophy outcomes in adults.',
    source: 'PubMed',
...
test('research source registry is valid and web sources require robots policy', () => {
  ...
  const webSources = researchSources.filter((source) => source.type === 'webpage');
...
test('research normalization adds stable hashes and dedup removes DOI/PMID/title duplicates', () => {
  ...
  const duplicate = normalizeResearchRecord(record({ external_id: 'pmid:2', pmid: '2' }));
  const unique = normalizeResearchRecord(record({ external_id: 'pmid:3', pmid: '3', doi: '10.1000/other', title: 'Protein intake resistance training meta analysis' }));

  assert.ok(first.content_hash);
  assert.equal(deduplicateResearchRecords([first, duplicate, unique]).length, 2);
});
...
test('evidence scoring gives transparent reasons and chunking preserves citation metadata', () => {
  ...
  assert.ok(scored.evidence_score_reason?.includes('systematic_review_or_meta_analysis'));
...
test('review queue requires review for weak web or thin metadata records', () => {
  const weak = scoreEvidence(normalizeResearchRecord(record({
...
test('sanitized coach context does not expose raw user identity or private contact fields', () => {
  const context = buildCoachContext({
    ...
    profile: {
      ...
      currentWeightKg: 82,
      goal: 'WEIGHT_LOSS',
      training: { availableEquipment: [], injuries: [], preferredTrainingDays: [1, 3, 5] } as any,
    },
  });
...
function loadFixture(name: string): NormalizedResearchRecord {
  const filePath = path.resolve(process.cwd(), '../../../data/research/fixtures', name);
  return JSON.parse(fs.readFileSync(filePath, 'utf8')) as NormalizedResearchRecord;
}
...
test('research review policy keeps web fixtures out of auto-index path', () => {
  const web = scoreEvidence(normalizeResearchRecord(loadFixture('webpage_sample.json')));
  const pubmed = scoreEvidence(normalizeResearchRecord(loadFixture('pubmed_sample.json')));
