...
import type { FinalAnswerPayload, RecommendationResult, RetrievalDocument } from '../llm/types';
...
function basePayload(overrides: Partial<FinalAnswerPayload> = {}): FinalAnswerPayload {
  return {
...
test('retriever maps knowledge pipeline evidence payload aliases into citation metadata', () => {
  const doc = retrievalDocumentFromPayload('fitness_evidence', {
...
test('ragService exposes evidenceUsed returned by the orchestrator', async () => {
  const evidenceDoc: RetrievalDocument = retrievalDocumentFromPayload('fitness_evidence', {
  ...
  llmOrchestrator.run = async () => basePayload({
    retrieval: { documents: [evidenceDoc], isEmpty: false },
    evidenceUsed: evidenceUsedFromDocs([evidenceDoc]),
  });

  conversationRepository.create = (() => ({ id: 'conversation-1' }) as any) as typeof conversationRepository.create;

  const result = await ragService.rag('How much protein do I need?', 'user-1', 'Bearer token');

  assert.equal(result.conversationId, 'conversation-1');
  assert.equal(result.evidenceUsed?.length, 1);
  assert.equal(result.evidenceUsed?.[0]?.title, 'Protein and resistance training');
  assert.equal(result.evidenceUsed?.[0]?.source_url, 'https://doi.org/10.1000/protein');
});
