...
test(
  "POST /ai/ask accepts valid Vietnamese request and avoids placeholder markers",
  { concurrency: false },
  async () => {
    ...
    assert.doesNotMatch(
      res.body.data.answer.toLowerCase(),
      /(coming soon|demo|sample|mock|placeholder)/,
    );
  },
);
...
test(
  "POST /ai/ask rejects direct request without gateway token",
  { concurrency: false },
  async () => {
    const res = await request(app)
      .post("/ai/ask")
...
test(
  "POST /ai/ask returns structured 503 when LLM provider is unavailable",
  { concurrency: false },
  async () => {
...
test(
  "POST /ai/generate-workout returns 502 for malformed LLM output",
  { concurrency: false },
  async () => {
...
test(
  "POST /plans/workout/generate returns queued response with real status contract",
  { concurrency: false },
  async () => {
