...
import {
  ...
  runRssPipeline,
  runWebPipeline,
} from "../knowledge-pipeline/service";
...
router.post(
  "/knowledge/local-evidence/async",
  async (req: Request, res: Response, next: NextFunction) => {
    if (!requireInternalSecret(req, res)) return;
    try {
      const limit = Number.isFinite(Number(req.query["limit"]))
      ...
      res
        .status(202)
        .json({ queued: true, jobId: job.id, queue: job.queueName });
    } catch (err) {
      next(err);
    }
  },
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    if (!requireInternalSecret(req, res)) return;
    try {
      const review = await knowledgeRepository.getReviewDocument(
        req.params.reviewId,
      );
      if (!review) {
      ...
      const reviewedBy =
        typeof req.query["reviewedBy"] === "string"
          ? req.query["reviewedBy"]
          : null;
