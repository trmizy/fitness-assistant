...
export async function processNutritionPlanJob(job: Job) {
  ...
  try {
    ...
    let templateResult = parseNutritionTemplate(
      llmResponse.answer,
      mealsPerDay,
    );
...
function parseNutritionTemplate(
  ...
  if (meals.length !== mealsPerDay) {
    return {
      ok: false,
      reason: `Expected ${mealsPerDay} meals, received ${meals.length}`,
    };
  }
...
function buildNutritionPlanFromTemplate(params: {
  ...
  const { goal, mealsPerDay, dailyCaloriesTarget, template, allowedFoods } =
    params;
  ...
  const weeklySchedule = Array.from({ length: 7 }, (_, dayIndex) => {
    const meals = Array.from({ length: mealsPerDay }, (_, mealIndex) => {
      ...
      const primary =
        (wantedFoodId ? allowedById.get(wantedFoodId) : undefined) ??
        proteinFoods[
          (dayIndex + mealIndex) % Math.max(1, proteinFoods.length)
        ] ??
        fallbackFoods[
      ...
      const secondary =
        carbFoods[
          (dayIndex * mealsPerDay + mealIndex) % Math.max(1, carbFoods.length)
        ] ??
        fallbackFoods[
      ...
      const primaryQuantity = clampQuantity(
        templateMeal?.items?.[0]?.quantity,
        primary,
        mealTarget * 0.55,
      );
      const secondaryQuantity =
        secondary.id === primary.id
          ? 0
          : clampQuantity(undefined, secondary, mealTarget * 0.45);
      const items = [
