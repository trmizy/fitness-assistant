...
function formatUnsafe(
  guidance: UnsafeGuidance,
  language: ResponseLanguage,
): string {
  if (language === "vi") {
    return [
...
function formatWorkoutPlan(
  ...
  if (language === "vi") {
    ...
    if ((rec.personalizationSummary || []).length > 0) {
      ...
      rec
        .personalizationSummary!.slice(0, 4)
    ...
    if (plan.isDefaultTemplate) {
      lines.push(
        "> Default template — update your profile to personalize further.",
      );
    }
  }
  ...
  if (language === "vi") {
    ...
    for (const day of plan.days) {
      day.exercises.forEach((ex, idx) => {
        ...
        lines.push(
          `| ${dayCell} | ${groupCell} | ${ex.name} | ${ex.sets} | ${ex.reps} | ${restLabel(ex.restSeconds)} |`,
        );
      });
      if (day.cardio) {
  ...
  lines.push("");
  lines.push(
    language === "vi" ? "### 📈 Ghi Chú Tăng Tiến" : "### 📈 Progression Notes",
  );
  plan.progressionNotes.forEach((note, idx) =>
...
function formatSpecificRoutine(
  rec: RecommendationResult,
  language: ResponseLanguage,
): string {
  ...
  routine.exercises.forEach((ex) =>
    lines.push(
      formatExerciseLine(ex.name, ex.sets, ex.reps, ex.restSeconds, language),
    ),
  );

  lines.push("");
  lines.push(
    language === "vi" ? "### 🔧 Kỹ Thuật Thực Hiện" : "### 🔧 Technique Notes",
  );
  routine.techniqueNotes.forEach((note, idx) =>
    lines.push(`${idx + 1}. ${note}`),
  );
  ...
  routine.overloadGuide.forEach((note, idx) =>
    lines.push(`${idx + 1}. ${note}`),
  );

  lines.push("");
  lines.push(
    language === "vi" ? "### ⚠️ An Toàn Khi Tập" : "### ⚠️ Safety Notes",
  );
  if (rec.workout.avoidedPatterns.length > 0) {
    rec.workout.avoidedPatterns
      .slice(0, 3)
      .forEach((pattern, idx) => lines.push(`${idx + 1}. ${pattern}`));
  } else {
    lines.push(
      language === "vi"
        ? "1. Giữ kỹ thuật chuẩn, dừng buổi tập khi đau nhói bất thường."
        : "1. Maintain strict technique and stop if sharp pain appears.",
    );
  }
...
function formatMealPlan(
  ...
  if (language === "vi") {
    ...
    if ((rec.personalizationSummary || []).length > 0) {
      lines.push("**Cá nhân hóa theo dữ liệu của bạn:**");
      rec
        .personalizationSummary!.slice(0, 5)
        .forEach((item) => lines.push(`- ${item}`));
      lines.push("");
    }
    if (mealPlan.isDefaultTemplate) {
      lines.push(
        "> Thực đơn mẫu — cập nhật thông số cơ thể để tính chính xác hơn.",
      );
      lines.push("");
    }
    lines.push(
      `📊 **Mục tiêu ngày:** ${mealPlan.kcal} kcal | Đạm: **${mealPlan.proteinGrams}g** | Carb: **${mealPlan.carbsGrams}g** | Béo: **${mealPlan.fatGrams}g**`,
    );
    lines.push("");
    lines.push(...formatNutritionSummary(rec, language));
    lines.push("");
    ...
    if (mealPlan.isDefaultTemplate) {
      lines.push(
        "> Template generated from available profile data. Update body metrics for higher precision.",
      );
      lines.push("");
    }
    lines.push(
      `📊 **Daily targets:** ${mealPlan.kcal} kcal | Protein: **${mealPlan.proteinGrams}g** | Carbs: **${mealPlan.carbsGrams}g** | Fats: **${mealPlan.fatGrams}g**`,
    );
    lines.push("");
    lines.push(...formatNutritionSummary(rec, language));
    lines.push("");
  ...
  mealPlan.substitutions.forEach((item, idx) =>
...
function formatGeneral(
  ...
  if (language === "vi") {
    ...
    if ((n.targetCalories ?? 0) > 0) {
      ...
      lines.push(
        `**Macro:** Đạm **${n.proteinGrams}g** | Carb **${n.carbsGrams}g** | Béo **${n.fatGrams}g**`,
      );
      lines.push("");
    }
    ...
    if ((n.targetCalories ?? 0) > 0) {
      ...
      lines.push(
        `**Macros:** Protein **${n.proteinGrams}g** | Carbs **${n.carbsGrams}g** | Fats **${n.fatGrams}g**`,
      );
      lines.push("");
    }
    if (w.sessionsPerWeek > 0) {
      lines.push(
        `**Training:** ${w.sessionsPerWeek} sessions/week — ${w.split}`,
      );
      if (w.focus.length > 0) {
...
function formatCombinedPlan(
  ...
  const title =
    language === "vi"
      ? "## 🎯 Kế Hoạch Tập + Ăn Kết Hợp"
      : "## 🎯 Combined Training + Nutrition Plan";
