...
export interface WorkoutScheduleIntent {
  enabled: boolean;
  target: 'today' | 'tomorrow' | 'this_week' | 'specific_weekday' | 'specific_date' | 'current_schedule';
  rawMessage?: string;
...
function weekdayLabel(dateOnly: string, language: ResponseLanguage = 'vi'): string {
...
function isScheduleConversation(chatHistory?: any[]): boolean {
  ...
  return chatHistory.slice(0, 4).some((entry) => {
    const text = normalizeText(`${entry?.question ?? ''} ${entry?.answer ?? ''}`);
    return /(lich tap|tap gi|workout schedule|scheduled_session|theo lich|ngay mai|hom nay|thu\s*[2-7])/.test(text);
  });
}
...
export function detectWorkoutScheduleIntent(question: string, chatHistory?: any[]): WorkoutScheduleIntent {
  ...
  if (parsedDate) {
    const mismatch = parsedDayOfWeek !== undefined && weekdayIndex(parsedDate) !== parsedDayOfWeek;
    return {
      enabled: true,
...
function muscleGroupsFromExercises(exercises: WorkoutScheduleExercise[], rawExercises?: any[]): string[] {
...
function contextFromProgramDay(day: any, date: string | undefined, source: WorkoutScheduleSource): WorkoutScheduleDayContext {
...
function contextFromSchedule(schedule: any, targetDate?: string): WorkoutScheduleContext | null {
  const day = schedule?.programDay;
  if (!day) return null;
  const dayContext = contextFromProgramDay(day, dateOnly(schedule?.date) ?? targetDate, 'scheduled_session');
  ...
  return {
    ...
    targetDayOfWeek: dayContext.date ? weekdayLabel(dayContext.date) : undefined,
...
function getProgramDays(program: any): any[] {
  return Array.isArray(program?.days)
    ? program.days.slice().sort((a: any, b: any) => Number(a?.dayNumber ?? 0) - Number(b?.dayNumber ?? 0))
    : [];
}

function dayContextFromCurrentProgram(program: any, intent: WorkoutScheduleIntent): WorkoutScheduleContext | null {
  ...
  if (intent.target === 'this_week') {
    const mappedDays = days.map((day: any, idx: number) =>
      contextFromProgramDay(day, intent.weekStart ? addDays(intent.weekStart, idx) : undefined, 'active_workout_program'),
    );
    return {
      activePlanName: program.name,
      planFrequency: program.daysPerWeek,
      ...
      muscleGroups: Array.from(new Set(mappedDays.flatMap((d) => d.muscleGroups))).slice(0, 8),
  ...
  const scheduleRows = Array.isArray(program?.schedules) ? program.schedules : [];
  const exact = scheduleRows.find((s: any) => intent.targetDate && dateOnly(s?.date) === intent.targetDate);
  if (exact?.programDay) {
  ...
  if (matchingSchedule?.programDay) {
    return contextFromSchedule({ ...matchingSchedule, date: intent.targetDate ?? matchingSchedule.date }, intent.targetDate);
  }

  const selected = days[Math.max(0, Math.min(days.length - 1, targetIdx === 0 ? days.length - 1 : targetIdx - 1))] ?? days[0];
  const dayContext = contextFromProgramDay(selected, intent.targetDate, 'active_workout_program');
  return {
    activePlanName: program.name,
    planFrequency: program.daysPerWeek,
    targetDate: intent.targetDate,
...
function aiPlanDays(planJson: any): any[] {
  ...
  if (Array.isArray(planJson?.workout_plan?.weeklySchedule)) return planJson.workout_plan.weeklySchedule;
  if (Array.isArray(planJson?.plan?.weeklySchedule)) return planJson.plan.weeklySchedule;
  if (Array.isArray(planJson?.workout?.weeklySchedule)) return planJson.workout.weeklySchedule;
  return [];
}

function dayContextFromAiPlan(planRow: any, intent: WorkoutScheduleIntent): WorkoutScheduleContext | null {
  const days = aiPlanDays(planRow?.plan);
  if (!days.length) return null;

  const mapDay = (day: any, idx: number, date?: string): WorkoutScheduleDayContext => {
  ...
  if (intent.target === 'this_week') {
    const mappedDays = days.map((day: any, idx: number) => mapDay(day, idx, intent.weekStart ? addDays(intent.weekStart, idx) : undefined));
    return {
  ...
  const targetIdx = intent.targetDate ? weekdayIndex(intent.targetDate) : 1;
  const selected = days[Math.max(0, Math.min(days.length - 1, targetIdx === 0 ? days.length - 1 : targetIdx - 1))] ?? days[0];
  const dayContext = mapDay(selected, days.indexOf(selected), intent.targetDate);
  return {
    activePlanName: planRow.name,
    planFrequency: planRow.daysPerWeek,
    targetDate: intent.targetDate,
...
function dayContextFromRecentLogs(logs: any[], intent: WorkoutScheduleIntent): WorkoutScheduleContext | null {
  ...
  return {
    ...
    source: 'recent_workout_logs',
    message: 'No stored schedule was found; only recent workout logs are available.',
  };
}

async function fetchSchedules(intent: WorkoutScheduleIntent, authHeader?: string): Promise<any[]> {
...
async function fetchCurrentProgram(authHeader?: string): Promise<any | null> {
  ...
  const res = await axios.get(`${FITNESS_SERVICE_URL}/workouts/programs/current`, {
    headers: authHeaders(authHeader),
    timeout: 3500,
  });
  const data = unwrap<any>(res);
...
async function fetchRecentLogs(authHeader?: string): Promise<any[]> {
  ...
  const data = unwrap<any>(res);
  return Array.isArray(data) ? data : Array.isArray(data?.workouts) ? data.workouts : [];
}

function emptyContext(intent: WorkoutScheduleIntent, message?: string): WorkoutScheduleContext {
  return {
    targetDate: intent.targetDate,
    targetDayOfWeek: intent.targetDayOfWeek,
    weekStart: intent.weekStart,
...
export function buildWorkoutScheduleContextBlock(context: WorkoutScheduleContext): string {
  const lines = [
  ...
  if (context.days?.length) {
    for (const day of context.days.slice(0, 7)) {
      lines.push(`day=${day.date ?? day.dayNumber ?? '?'} | title=${day.title ?? 'Workout'} | exercises=${day.exercises.length}`);
      for (const ex of day.exercises.slice(0, 12)) {
        lines.push(`- ${ex.name} | sets=${ex.sets ?? '?'} | reps=${ex.reps ?? '?'} | rest=${ex.restSeconds ?? '?'}s`);
      }
    }
  }
...
export function workoutScheduleSourceLabel(source: WorkoutScheduleSource, language: ResponseLanguage = 'vi'): string {
  if (language !== 'vi') {
    return {
...
export function formatWorkoutScheduleAnswer(context: WorkoutScheduleContext, language: ResponseLanguage): string {
  const isVi = language === 'vi';
  const targetLabel = context.targetDate
    ? `${context.targetDayOfWeek ?? ''} ${context.targetDate}`.trim()
    : context.weekStart && context.weekEnd
      ? `${context.weekStart} -> ${context.weekEnd}`
      : isVi ? 'ngày này' : 'that date';
  if (!context.scheduledWorkoutFound || (!context.exercises.length && !context.days?.some((d) => d.exercises.length))) {
  ...
  const headingDate = context.targetDate
    ...
    : context.weekStart && context.weekEnd
      ? `${context.weekStart} -> ${context.weekEnd}`
      : isVi ? 'lịch hiện tại' : 'current schedule';
  ...
  if (context.days && context.days.length > 1) {
    ...
    return [intro, '', ...sections, '', isVi ? `Nguồn: ${workoutScheduleSourceLabel(context.source, language)}.` : `Source: ${workoutScheduleSourceLabel(context.source, language)}.`].join('\n');
  }

  return [
    intro,
    '',
    context.muscleGroups.length ? `**Nhóm cơ:** ${context.muscleGroups.join(', ')}` : '',
    '',
    '| # | Bài tập | Sets | Reps | Rest |',
    '|---|---------|------|------|------|',
    renderExercises(context.exercises),
    '',
    isVi ? `Nguồn: ${workoutScheduleSourceLabel(context.source, language)}.` : `Source: ${workoutScheduleSourceLabel(context.source, language)}.`,
  ].filter(Boolean).join('\n');
}

export const workoutScheduleContextResolver = {
  async resolve(intent: WorkoutScheduleIntent, userId?: string, authHeader?: string): Promise<WorkoutScheduleContext> {
    ...
    try {
      ...
      if (intent.target === 'this_week' && schedules.length > 0) {
        const days = schedules
          .map((schedule) => contextFromSchedule(schedule, dateOnly(schedule?.date)))
          .filter(Boolean)
          .map((ctx) => ctx!.days?.[0])
        ...
        return {
          ...
          muscleGroups: Array.from(new Set(days.flatMap((d) => d.muscleGroups))).slice(0, 8),
          exercises: days[0]?.exercises ?? [],
          days,
          source: 'scheduled_session',
        };
      }
      const exactSchedule = schedules.find((s) => !intent.targetDate || dateOnly(s?.date) === intent.targetDate);
      const exactContext = contextFromSchedule(exactSchedule, intent.targetDate);
    ...
    try {
      ...
      logger.warn({ err }, 'Failed to load current workout program for AI coach');
    }

    try {
      const plans = await conversationRepository.findPlansByUser(userId, PlanStatus.COMPLETED, 5);
      for (const plan of plans) {
        const planContext = dayContextFromAiPlan(plan, intent);
        if (planContext?.scheduledWorkoutFound) return planContext;
      }
    } catch (err) {
      logger.warn({ err }, 'Failed to load generated AI plans for AI coach schedule context');
    }
    ...
    try {
      ...
      logger.warn({ err }, 'Failed to load recent workout logs for AI coach schedule context');
    }

    return emptyContext(intent, 'No stored workout schedule found.');
  },

  debug(intent: WorkoutScheduleIntent, context: WorkoutScheduleContext, userId?: string): void {
