...
function hasWorkoutSignal(q: string): boolean {
  return /\b(tap|lich\s*tap|buoi\s*tap|workout|training|exercise|gym|sets?|reps?)\b/.test(
    q,
  );
}
...
function isNutritionConversation(chatHistory?: any[]): boolean {
  ...
  return chatHistory.slice(0, 4).some((entry) => {
    const text = normalizeNutritionText(
      `${entry?.question ?? ""} ${entry?.answer ?? ""}`,
    );
    return /(thuc\s*don|bua|an\s*gi|nutrition|meal|calo|macro|protein|carb|fat|dinh\s*duong)/.test(
      text,
    );
  });
}

export function detectNutritionLookupIntent(
  question: string,
  chatHistory?: any[],
  ...
  const shortFollowUp =
    /^(con|vay|thi\s*sao|ngay\s*mai|mai|hom\s*nay|today|tomorrow|ngay|\d{1,2}\s*[/-])/.test(
      q,
    );
...
function mapProgramMeal(meal: any): NutritionMeal {
  ...
  const foods = (Array.isArray(meal.items) ? meal.items : []).map(
    (item: any) => ({
      name: String(
        item.foodName ??
          item.customFoodName ??
  ...
  return {
    ...
    totals: {
      calories: numberOrNull(
        meal.tableTotals?.calories ??
          meal.plannedTotals?.calories ??
          meal.calories,
      ),
      protein: numberOrNull(
        meal.tableTotals?.protein ??
          meal.plannedTotals?.protein ??
          meal.proteinGrams,
      ),
      carbs: numberOrNull(
        meal.tableTotals?.carbs ?? meal.plannedTotals?.carbs ?? meal.carbGrams,
      ),
      fat: numberOrNull(
        meal.tableTotals?.fat ?? meal.plannedTotals?.fat ?? meal.fatGrams,
      ),
    },
...
function contextFromLogs(
  intent: NutritionLookupIntent,
  logs: any[],
): NutritionScheduleContext | null {
  ...
  const meals = Array.from(grouped.entries()).map(([mealType, mealLogs]) =>
    mapLogToMeal(mealLogs, mealType),
  );
...
function contextFromDailyTask(
  intent: NutritionLookupIntent,
  task: any,
): NutritionScheduleContext | null {
  if (!task) return null;
  const allMeals = Array.isArray(task.meals)
    ? task.meals.map(mapProgramMeal)
    : [];
  const meals = filterMeals(allMeals, intent.mealType).filter(
  ...
  if (meals.length > 0) {
    return {
      ...
      dailyCaloriesTarget: numberOrNull(
        program?.dailyCaloriesTarget ?? task.day?.totalCalories,
      ),
      proteinTarget: numberOrNull(
        program?.proteinTargetGrams ?? task.day?.proteinGrams,
      ),
...
function contextFromGeneratedPlan(
  intent: NutritionLookupIntent,
  plan: any,
): NutritionScheduleContext | null {
  const meals = candidateMealsFromPlan(plan).map((meal: any) => {
    const mealTypeRaw = normalizeNutritionText(
      String(meal.mealType ?? meal.type ?? meal.mealName ?? meal.name ?? "all"),
    );
    ...
    const foods = foodValues.map((food: any) => {
      ...
      return {
        name: String(
          food.name ?? food.foodName ?? food.customFoodName ?? "Món ăn",
        ),
  ...
  const filtered = filterMeals(meals, intent.mealType).filter(
    (meal) => meal.foods.length > 0,
  );
  if (!filtered.length) return null;
  return {
...
export function buildNutritionContextBlock(
  context: NutritionScheduleContext,
): string {
  const lines = [
  ...
  for (const meal of context.meals.slice(0, 8)) {
    lines.push(
      `meal=${meal.mealType} | title=${meal.title ?? mealLabel(meal.mealType, "en")} | foods=${meal.foods.length}`,
    );
    for (const food of meal.foods.slice(0, 12)) {
      lines.push(
        `- ${food.name} | qty=${food.quantity ?? "?"}${food.unit ?? ""} | kcal=${food.calories ?? "?"}`,
      );
    }
  }
...
function formatTargets(
  ...
  const parts = [
    context.dailyCaloriesTarget != null
      ? `${Math.round(context.dailyCaloriesTarget)} kcal`
      : null,
    context.proteinTarget != null
      ? `${Math.round(context.proteinTarget)}g protein`
      : null,
    context.carbTarget != null
      ? `${Math.round(context.carbTarget)}g carb`
      : null,
...
export function formatNutritionAnswer(
  ...
  const targetLabel = [
    mealLabel(context.mealType, language),
    context.targetDate,
  ]
    .filter(Boolean)
    .join(" ");
  if (!context.plannedMealsFound || !context.meals.length) {
    ...
    return [
      ...
      targetLine,
      isVi
        ? `Nguồn: ${nutritionSourceLabel(context.source, language)}.`
        : `Source: ${nutritionSourceLabel(context.source, language)}.`,
    ]
      .filter(Boolean)
      .join("\n");
  }
  ...
  for (const meal of context.meals) {
    lines.push(
      `### ${mealLabel(meal.mealType, language)}${meal.title ? ` - ${meal.title}` : ""}`,
    );
    for (const food of meal.foods) {
      const qty =
        food.quantity != null ? ` (${food.quantity}${food.unit ?? ""})` : "";
      const kcal =
    ...
    if (meal.totals?.calories != null) {
      lines.push(
        `Tổng: ${Math.round(meal.totals.calories)} kcal | Protein ${Math.round(Number(meal.totals.protein ?? 0))}g | Carb ${Math.round(Number(meal.totals.carbs ?? 0))}g | Fat ${Math.round(Number(meal.totals.fat ?? 0))}g`,
      );
    }
    lines.push("");
  }
...
export const nutritionContextResolver = {
  async resolve(
    ...
    userId?: string,
    authHeader?: string,
  ): Promise<NutritionScheduleContext> {
    ...
    try {
      const plans = await conversationRepository.findNutritionPlansByUser(
        userId,
      ...
      logger.warn(
        { err },
        "Failed to load generated AI nutrition plans for AI coach",
      );
    }
