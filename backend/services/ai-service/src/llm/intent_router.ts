...
function parseTrainingDays(question: string): number | undefined {
  ...
  const match = question.match(
    /(\d)\s*(?:bu[oô][i\u0300\u0301\u0303\u0309\u0323]?|sessions?|days?|ng[aà]y)/i,
  );
...
function inferGoal(
  question: string,
  profile?: UserProfile,
): IntentRoute["goalHint"] {
  const q = question.toLowerCase();
  if (
    /(fat loss|gi[aả]m m[oỡ]|giam mo|si[eế]t m[oỡ]|siet mo|cut|gi[aả]m c[aâ]n|giam can)/i.test(
      q,
    )
  )
    return "fat_loss";
  if (/(muscle gain|t[aă]ng c[oơ]|tang co|bulk|hypertrophy)/i.test(q))
    return "muscle_gain";
  if (
    /(recomp|recomposition|t[aá]i c[aấ]u tr[uú]c c[oơ] th[eể]|tai cau truc co the|v[uừ]a gi[aả]m m[oỡ] v[uừ]a t[aă]ng c[oơ]|vua giam mo vua tang co)/i.test(
      q,
    )
  )
...
function isUnsafeWeightLoss(question: string): boolean {
  const q = question.toLowerCase();
  const hasRapidLoss =
    /(giam|lose).{0,20}(\d{1,2})\s*kg.{0,20}(tuan|week|ngay|day)/i.test(q);
  const hasExtremePhrase =
    /(cang nhanh cang tot|nhanh nhat|cap toc|extreme|rapid)/i.test(q);
  const hasOneWeekTarget =
    /(1\s*(tuan|week)|mot\s*tuan)/i.test(q) && /(giam|lose).{0,20}kg/i.test(q);
...
function inferIntent(question: string): RoutedIntentType {
  ...
  if (
    /(hoan thien ho so|cap nhat ho so|profile|thieu thong tin|bo sung thong tin)/i.test(
      q,
    )
  ) {
  ...
  if (
    /\d+\s*bu[oổ]i\s*[/]?\s*(?:\d+\s*)?(?:tu[aầ]n|week)/i.test(q) ||
  ...
  if (
    /(th[uứ]\s*(2|3|4|5|6|7|hai|ba|tứ?|tu|năm|nam|sáu|sau|bảy|bay))/i.test(q)
  ) {
  ...
  if (
    /(v[uừ]a gi[aả]m m[oỡ] v[uừ]a t[aă]ng c[oơ]|vua giam mo vua tang co|recomposition|recomp|t[aá]i c[aấ]u tr[uú]c c[oơ] th[eể]|tai cau truc co the)/i.test(
      q,
    )
  ) {
  ...
  if (
    /(l[iị]ch t[aậ]p|workout|plan t[aậ]p|ch[uươ]ng tr[iì]nh t[aậ]p).*(dinh d[uư][oơ]ng|th[uự]c [dđ][oơ]n|meal plan|[aă]n u[oố]ng)/i.test(
      q,
    ) ||
    /(dinh d[uư][oơ]ng|th[uự]c [dđ][oơ]n|meal plan|[aă]n u[oố]ng).*(l[iị]ch t[aậ]p|workout|plan t[aậ]p|ch[uươ]ng tr[iì]nh t[aậ]p)/i.test(
      q,
    )
  ) {
  ...
  if (
    /(push|pull|legs|ppl|upper|lower)/i.test(q) &&
    /(bu[oổ]i|session|routine|l[iị]ch t[aậ]p)/i.test(q)
  ) {
    return "muscle_group_routine_request";
  }

  if (
    /(tay tr[uướ]c|tay sau|ng[uự]c|l[uư]ng|ch[aâ]n|vai|b[uụ]ng|biceps|triceps|chest|back|legs|shoulders|core|forearm)/i.test(
      q,
    ) &&
  ...
  if (
    /(tay tr[uướ]c|tay sau|biceps|triceps|forearm)/i.test(q) &&
    /(b[aà]i t[aậ]p|b[aà]i n[aà]o|nh[uữ]ng b[aà]i|go[iị] y|g[iợ]i y|h[oỗ] tr[uợ] t[iố]t|n[eê]n t[aậ]p g[iì]|c[oó] nh[uữ]ng b[aà]i)/i.test(
      q,
    )
  ) {
    return "muscle_group_routine_request";
  ...
  if (
    /(tay tr[uướ]c|tay sau|biceps|triceps|forearm)/i.test(q) &&
    /(t[aậ]p|train|work out|workout)/i.test(q)
  ) {
    return "muscle_group_routine_request";
  }
  ...
  if (
    /(injury|pain|ch[aấ]n\s*th[uươ][oơ]ng|chan\s*thuong|[dđ]au\s*g[oố]i|dau\s*goi|[dđ]au\s*l[uư]ng|dau\s*lung|[dđ]au\s*vai|dau\s*vai|[dđ]au\s*kh[oớ]p|knee\s*pain|shoulder\s*pain|back\s*pain)/i.test(
      q,
    )
  ) {
    return "general_fitness_knowledge";
  ...
  if (
    /(ch[aâ]n|legs|đùi|mông|vai|shoulder|b[uụ]ng|core|abs)/i.test(q) &&
  ...
  if (
    /(l[iị]ch t[aậ]p c[uụ] th[eể]|t[uừ]ng b[aà]i|b[aà]i t[aậ]p c[uụ] th[eể] l[aà] g[iì]|specific exercise)/i.test(
      q,
    )
  ) {
  ...
  if (
    /(l[iị]ch t[aậ]p|ch[uươ][oơ]ng tr[iì]nh t[aậ]p|split|plan t[aậ]p|workout plan)/i.test(
      q,
    )
  ) {
