...
function mapInBody(entry?: InBodyEntry): InBodyMetrics | undefined {
  ...
  return {
    ...
    // Segmental analysis
    segmentalMuscle: (entry.rightArmMuscle || entry.leftArmMuscle || entry.trunkMuscle || entry.rightLegMuscle || entry.leftLegMuscle) ? {
    ...
    segmentalFat: (entry.rightArmFat || entry.leftArmFat || entry.trunkFat || entry.rightLegFat || entry.leftLegFat) ? {
...
export const profileExtractor = {
  ...
  async extract(userId?: string, authorizationHeader?: string): Promise<PersonalizationContext> {
    ...
    const [profileRes, inBodyRes, workoutsRes, nutritionRes, workoutProgramRes, nutritionProgramRes] =
    ...
    if (workoutProgramRes.status === 'rejected') {
      logger.debug({ error: (workoutProgramRes as any).reason }, 'Could not load current workout program');
    }
    if (nutritionProgramRes.status === 'rejected') {
      logger.debug({ error: (nutritionProgramRes as any).reason }, 'Could not load current nutrition program');
    }
    ...
    const currentWorkoutProgram = rawWorkoutProgram && typeof rawWorkoutProgram === 'object'
      ? {
          ...
          dayCount: Array.isArray(rawWorkoutProgram.days) ? rawWorkoutProgram.days.length : undefined,
    ...
    const currentNutritionProgram = rawNutritionProgram && typeof rawNutritionProgram === 'object'
      ? {
