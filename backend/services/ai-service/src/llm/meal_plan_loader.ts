...
function getPlans(): Record<string, string>[] {
  if (!_plans)
    _plans = parseCSV(path.join(NUTRITION_DIR, "gym_meal_plans.csv"));
  return _plans;
}

function getItems(): Record<string, string>[] {
  if (!_items)
    _items = parseCSV(path.join(NUTRITION_DIR, "gym_meal_plan_items.csv"));
  return _items;
}
...
function selectPlan(
  ...
  let candidates = plans.filter(
    (p) =>
      p["goal_code"] === goalCode && Number(p["meals_per_day"]) === mealsPerDay,
  );
