...
function chooseSplit(daysPerWeek: number, goal: string, level: string): string {
  ...
  if (daysPerWeek === 3)
    return goal === "fat_loss"
      ? "full_body_plus_conditioning"
      : "upper_lower_full";
  if (daysPerWeek === 4)
    return goal === "muscle_gain" ? "upper_lower" : "push_pull_legs_upper";
  if (daysPerWeek >= 5) {
    if (level === "BEGINNER") return "upper_lower_plus_accessory";
    return goal === "muscle_gain"
      ? "push_pull_legs_upper_lower"
      : "push_pull_legs_plus_conditioning";
  }
  return "full_body";
}
...
function inferFocus(goal: string): string[] {
  if (goal === "fat_loss")
    return ["compound training", "cardio support", "daily movement"];
  if (goal === "muscle_gain")
    return ["progressive overload", "hypertrophy volume", "recovery"];
...
export const workoutPlanSelector = {
  select(profile: UserProfile, intent: InputIntent): WorkoutRecommendation {
    ...
    if (!intent.parsedTrainingDays && !profile.training.trainingDaysPerWeek) {
      assumptions.push(
        "Assumed 3 sessions per week due to missing training frequency.",
      );
    }
