...
function detectDetailMode(question: string): boolean {
  return /(đầy đủ|chi tiết|không rút gọn|ghi rõ|từng ngày|từng bữa|day by day|step by step|detailed|full plan)/i.test(
    question,
  );
}

function mapRouteCategory(
  routeIntent: InputIntent["routeIntent"],
  question: string,
  ...
  if (routeIntent === "meal_plan_request") {
    if (
      ...
      !/thực đơn|meal plan|bữa/i.test(q)
    ) {
  ...
  if (routeIntent === "muscle_group_routine_request")
...
function inferIntent(question: string): InputIntent["intent"] {
  ...
  if (
    workoutSignals.test(q) &&
    (mealSignals.test(q) || personalSignals.test(q))
  ) {
...
function parseTrainingDays(question: string): number | undefined {
  ...
  const match =
    q.match(
      /(\d+)\s*(?:buoi|sessions?|days?)\s*(?:\d+\s*)?(?:tuan|week|per week)/i,
    ) ||
    q.match(
      /(\d+)\s*(?:buoi|sessions?|days?)(?:\s*(?:moi|per)\s*(?:tuan|week))?/i,
    );
...
function inferMealPreference(question: string): string | undefined {
  const q = question.toLowerCase();
  if (
    /(dị ứng sữa|di ung sua|không sữa|khong sua|lactose|dairy free|no dairy)/i.test(
      q,
    )
  )
    return "dairy_free";
  ...
  if (/(ít nấu|it nau|quick prep|nhanh gọn|meal prep nhanh)/i.test(q))
    return "low_cook";
...
export const inputParser = {
  parse(question: string, profile?: UserProfile): InputIntent {
    ...
    const minimumExercisesPerDay =
      parseMinimumExercisesPerDay(normalizedQuestion);
    const mealPreferenceHint = inferMealPreference(normalizedQuestion);
    const mentionsInjury =
      /(injury|pain|ch[aấ]n th[uươ][oơ]ng|chan thuong|[dđ]au g[oố]i|dau goi|[dđ]au l[uư]ng|dau lung|[dđ]au vai|dau vai|[dđ]au kh[oớ]p|knee pain|shoulder pain|back pain)/i.test(
        normalizedQuestion,
      );
    const requestsCardio =
      /(cardio|c[aà]o tim|aerobic|ch[aạ]y b[oộ]|chay bo|[dđ][aạ]p xe|dap xe|swimming|b[oơ]i l[oộ]i|rowing|b[uướ][oớ]c nhanh|buoc nhanh)/i.test(
        normalizedQuestion,
      );
    const mealsMatch =
      stripGymDiacritics(normalizedQuestion).match(
        /(\d)\s*bua(?:\s*an)?(?:\s*moi\s*ngay)?(?:\s*trong\s*(?:\d+\s*)?(?:ngay|tuan|week))?/i,
      ) ||
      stripGymDiacritics(normalizedQuestion).match(
        /(\d)\s*(?:meals?|bua)\s*(?:a\s*day|per\s*day)/i,
      );
    const parsedMealsPerDay = mealsMatch
      ? Math.min(6, Math.max(1, Number(mealsMatch[1])))
      : undefined;
    ...
    return {
      ...
      missingFields: Array.from(
        new Set([...missingFields, ...routed.missingFields]),
      ),
    };
  },
};
