...
function objectiveFromGoal(
  goal?: UserProfile["goal"],
  goalHint?: InputIntent["goalHint"],
): string {
  if (goalHint === "fat_loss" || goal === "WEIGHT_LOSS") return "fat_loss";
  if (goalHint === "muscle_gain" || goal === "MUSCLE_GAIN")
    return "muscle_gain";
  if (goalHint === "maintenance" || goal === "MAINTENANCE")
    return "maintenance";
...
function mealTemplateFromGoal(objective: string): string {
  if (objective === "fat_loss")
    return "high protein with moderate calorie deficit";
  if (objective === "muscle_gain")
    return "high protein with slight calorie surplus";
...
function buildMealRecommendation(
  ...
  if (!preference) {
    assumptions.push(
      "No dietary preference provided; using a general mixed-food baseline template.",
    );
  }
...
function selectDaysByCount(count: number): DayPlan[] {
  if (count <= 3)
    return [fullBodyA(), fullBodyB(), fullBodyC()].slice(0, count);
  if (count === 4)
    return [upperDayA4(), lowerDayA4(), upperDayB4(), lowerDayB4()];
  if (count === 5)
    return [pushDayA(), pullDayA(), legsDayA(), upperDayB(), lowerDayB()];
  if (count === 6)
    return [
      pushDayA(),
      pullDayA(),
...
function buildWorkoutPlanTemplate(
  profile: UserProfile,
  intent: InputIntent,
): WorkoutPlanTemplate {
  const requestedDays =
    intent.parsedTrainingDays || profile.training.trainingDaysPerWeek || 3;

  const minExercises = Math.max(
    intent.minimumExercisesPerDay || 5,
    intent.detailMode ? 6 : 5,
  );
  const objective =
  ...
  return {
    isDefaultTemplate:
      !profile.training.trainingDaysPerWeek && !intent.parsedTrainingDays,
...
function buildChestRoutine(detailMode = false): SpecificRoutineTemplate {
  return {
    ...
    sessionGoal:
      "Buổi ngực đầy đủ cho người mới đến trung cấp, ưu tiên kỹ thuật chuẩn.",
    exercises: [
      buildExercise(
        1,
        "Machine Chest Press",
        4,
        detailMode ? "8-12" : "10-12",
        90,
      ),
      buildExercise(
        2,
...
function buildPushDayRoutine(detailMode = false): SpecificRoutineTemplate {
  return {
    isDefaultTemplate: true,
    sessionGoal:
      "Push day hoàn chỉnh: ngực, vai, tay sau theo thứ tự từ compound đến isolation.",
    exercises: [
  ...
  return {
    ...
    exercises: [
      ...
      buildExercise(
        ...
        "8-10",
        90,
        "Best compound support for both arms and pressing strength",
      ),
    ],
    techniqueNotes: [
...
function buildSpecificRoutineByIntent(
  intent: InputIntent,
  equipment: string[] = [],
): SpecificRoutineTemplate {
  ...
  if (/push day|push/i.test(q))
    routine = buildPushDayRoutine(Boolean(intent.detailMode));
  else if (intent.muscleGroupHint === "chest")
    routine = buildChestRoutine(Boolean(intent.detailMode));
  else if (intent.muscleGroupHint === "back")
    routine = buildBackRoutine(Boolean(intent.detailMode));
  else if (
    (plain.includes("tay truoc") && plain.includes("tay sau")) ||
    (plain.includes("biceps") && plain.includes("triceps"))
  ) {
    routine = buildArmsRoutine();
  } else if (intent.muscleGroupHint === "biceps" || /tay truoc|biceps/i.test(q))
    routine = buildBicepsRoutine();
  else if (
    intent.muscleGroupHint === "legs" ||
    /chan|legs|dui|mong/i.test(plain)
  )
    routine = buildLegsRoutine(Boolean(intent.detailMode));
  else if (
    intent.muscleGroupHint === "shoulders" ||
    /vai|shoulder/i.test(plain)
  )
    routine = buildShouldersRoutine(Boolean(intent.detailMode));
  else if (intent.muscleGroupHint === "core" || /bung|core|abs/i.test(plain))
    routine = buildCoreRoutine(Boolean(intent.detailMode));
  else routine = buildGeneralSpecificRoutine();

  if (equipment.length === 0) return routine;
  return {
    ...routine,
    exercises: filterExercisesByEquipment(routine.exercises, equipment),
  };
}
...
function buildPersonalizationSummary(profile: UserProfile): string[] {
  ...
  if (profile.currentWeightKg || profile.inBody?.weightKg)
    notes.push(
      `Cân nặng: ${profile.currentWeightKg || profile.inBody?.weightKg}kg`,
    );
  if (profile.heightCm) notes.push(`Chiều cao: ${profile.heightCm}cm`);
  if (profile.goal) notes.push(`Mục tiêu: ${profile.goal}`);
  if (profile.experienceLevel)
    notes.push(`Kinh nghiệm: ${profile.experienceLevel}`);
  if (profile.training.injuries.length > 0)
    notes.push(
      `Chấn thương cần tránh: ${profile.training.injuries.join(", ")}`,
    );
  if (profile.training.availableEquipment.length > 0)
    notes.push(
      `Thiết bị sẵn có: ${profile.training.availableEquipment.join(", ")}`,
    );
  if (profile.foodPreference)
    notes.push(`Ưu tiên ăn uống: ${profile.foodPreference}`);
  return notes;
}
...
function buildGeneralSpecificRoutine(): SpecificRoutineTemplate {
  return {
    isDefaultTemplate: true,
    sessionGoal:
      "Full-body hypertrophy session with balanced push, pull, and lower body work.",
    exercises: [
...
function buildLegsRoutine(detailMode = false): SpecificRoutineTemplate {
  return {
    ...
    exercises: [
      buildExercise(
        1,
        detailMode ? "Back Squat" : "Goblet Squat",
        4,
        detailMode ? "5-7" : "8-12",
        detailMode ? 120 : 90,
        "Compound chính — tứ đầu + mông",
      ),
      buildExercise(
        2,
        "Romanian Deadlift",
        ...
        "8-10",
        90,
        "Hip hinge — gân khoeo + lưng dưới",
      ),
      buildExercise(
        3,
        detailMode ? "Bulgarian Split Squat" : "Leg Press",
        3,
        detailMode ? "8-10 mỗi chân" : "10-12",
        75,
        detailMode ? "Đơn chân — cân bằng lực" : "Tứ đầu volume",
      ),
...
function buildShouldersRoutine(detailMode = false): SpecificRoutineTemplate {
  return {
    isDefaultTemplate: true,
    sessionGoal:
      "Buổi vai toàn diện — vai trước, vai giữa, vai sau và rotator cuff.",
    exercises: [
      buildExercise(
        1,
        "Overhead Press",
        ...
        detailMode ? "5-7" : "8-10",
        90,
        "Compound chính — vai trước + thân trên",
      ),
      ...
      buildExercise(
        5,
        detailMode ? "Arnold Press" : "Cable Lateral Raise",
        3,
        "10-12",
        60,
        detailMode
          ? "Vai toàn phần — biên độ rộng"
          : "Vai giữa — căng cơ liên tục",
      ),
    ],
    techniqueNotes: [
...
function buildCoreRoutine(detailMode = false): SpecificRoutineTemplate {
  return {
    isDefaultTemplate: true,
    sessionGoal:
      "Core stability — chống ưỡn, kiểm soát cột sống. Core mạnh cải thiện mọi bài tập, nhưng không thể giảm mỡ cục bộ.",
    exercises: [
      buildExercise(
        1,
        "Plank",
        3,
        detailMode ? "45-60s" : "30-45s",
        45,
        "Anti-extension — nền tảng core",
      ),
      buildExercise(
        2,
        "Dead Bug",
        ...
        "8-10 mỗi bên",
        45,
        "Anti-extension + phối hợp tay chân",
      ),
      buildExercise(
        3,
        "Hollow Body Hold",
      ...
      buildExercise(
        ...
        detailMode ? "8-10" : "5-8",
        60,
        "Anti-extension nâng cao — chỉ roll xa khi giữ được lưng thẳng",
      ),
      buildExercise(
        5,
        "Copenhagen Plank",
      ...
      ...(detailMode
        ? [
            buildExercise(
              ...
              "12-15",
              45,
              "Flexion có tải — rectus abdominis",
            ),
          ]
        : []),
    ],
...
function buildMealPlanTemplate(
  profile: UserProfile,
  intent: InputIntent,
): MealPlanTemplate {
  ...
  const isDairyFree = /dairy_free|no_whey|không sữa|lactose|di ung sua/i.test(
    preference,
  );
  ...
  const meals = isDairyFree
    ? [
        {
          mealName: "Bữa sáng",
          foods: ["Yến mạch 60g", "Sữa hạt không đường 250ml", "Chuối 1 quả"],
        },
        {
          mealName: "Bữa trưa",
          foods: ["Ức gà 150g", "Cơm trắng 180g", "Rau xanh 200g"],
        },
        {
          mealName: "Bữa tối",
          foods: ["Cá hồi 150g", "Khoai lang 250g", "Salad 1 tô"],
        },
      ]
    : [
        {
          mealName: "Bữa sáng",
          foods: ["Yến mạch 60g", "Sữa chua Hy Lạp 200g", "Chuối 1 quả"],
        },
        {
          mealName: "Bữa trưa",
          foods: ["Ức gà 150g", "Cơm trắng 180g", "Rau xanh 200g"],
...
function buildFollowUps(
  ...
  if (
    ...
    const extras = isVi
      ? [
          "Bạn tập ở nhà hay phòng gym?",
          "Bạn có đau hoặc chấn thương nào cần tránh không?",
        ]
...
export const recommendationEngine = {
  recommend(
    ...
    const asksForMuscleRoutine =
      Boolean(intent.muscleGroupHint) &&
      /(lich tap|routine|buoi tap|bai tap|workout|exercise)/i.test(
        intent.normalizedQuestion,
      );
    const responseIntent =
      routedIntent === "general_fitness_knowledge" && asksForMuscleRoutine
    ...
    const specificRoutine =
      responseIntent === "specific_exercise_request" ||
      responseIntent === "muscle_group_routine_request"
        ? buildSpecificRoutineByIntent(
            intent,
            profile.training.availableEquipment,
          )
        : undefined;
    ...
    const mealPlan =
      responseIntent === "meal_plan_request" ||
      responseIntent === "combined_plan_request"
    ...
    return {
      ...
      specificRoutine,
      mealPlan,
      followUpQuestions: buildFollowUps(
        responseIntent,
