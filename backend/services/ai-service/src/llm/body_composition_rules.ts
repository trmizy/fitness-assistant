...
export function analyzeBodyComposition(profile: UserProfile): BodyCompAnalysis {
  ...
  if (bfPct && isModerateHighBF(bfPct, gender) && /fat_loss|weight_loss|giam_mo|cutting/.test(goal)) {
    ...
    analysis.planConstraints.volumeGuidance  = '3–5 sets per muscle group, 8–15 rep range for fat loss while preserving muscle';
  ...
  if (muscleMassKg && experience === 'BEGINNER') {
    const expectedLeanMin = gender === 'FEMALE'
    ...
    if (muscleMassKg < expectedLeanMin) {
      ...
      analysis.planConstraints.splitGuidance  = 'Full body (3 days) or Upper/Lower (4 days) — not PPL until intermediate level';
      analysis.planConstraints.volumeGuidance = '2–3 sets per exercise, 8–12 reps, focus on technique over load';
  ...
  if (bmi && bmi >= BMI_OVERWEIGHT && muscleMassKg && bfPct && !isModerateHighBF(bfPct, gender)) {
    analysis.adjustments.push({
  ...
  // Deduplicate queries
  analysis.evidenceQueries = Array.from(new Set(analysis.evidenceQueries)).slice(0, 8);
...
export function formatBodyCompAnalysis(analysis: BodyCompAnalysis): string {
  ...
  if (analysis.planConstraints) {
    ...
    if (c.maxDeficitKcal) constraints.push(`Caloric deficit tối đa: ${c.maxDeficitKcal} kcal`);
    if (c.minProteinGKg)  constraints.push(`Protein tối thiểu: ${c.minProteinGKg} g/kg/ngày`);
    if (c.splitGuidance)  constraints.push(`Split phù hợp: ${c.splitGuidance}`);
    if (c.volumeGuidance) constraints.push(`Volume guidance: ${c.volumeGuidance}`);
