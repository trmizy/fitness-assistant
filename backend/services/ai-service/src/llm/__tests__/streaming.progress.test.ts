...
describe("streaming progress events — pipeline behavior", () => {
  it("normal request → progress events fire in correct order", async () => {
    ...
    assert.ok(
      ...
      `Expected at least 3 progress events, got ${progressMessages.length}`,
    );
    ...
    assert.deepEqual(
      received,
      progressMessages,
      "Callback receives same messages as internal list",
    );
  });
  ...
  it("off-topic question → safety gate short-circuits, no progress events emitted", async () => {
    ...
    assert.equal(
      safetyCheck.type,
      "off_topic",
      `"${offTopicQ}" should be detected as off-topic`,
    );

    const { progressMessages } = await mockPipeline(offTopicQ, (msg) =>
      received.push(msg),
    );
    ...
    assert.equal(
      ...
      0,
      "Off-topic must not emit progress events",
    );
  ...
  it("medical emergency → safety gate short-circuits, no progress events emitted", async () => {
    const received: string[] = [];
    const safetyCheck = safetyGuard.check(
    ...
    if (safetyCheck.type === "medical_emergency") {
      ...
      assert.equal(
        progressMessages.length,
        0,
        "Medical emergency must not emit progress events",
      );
    } else {
  ...
  it('LLM fallback path emits "kế hoạch an toàn" event', async () => {
    ...
    const { captured } = await mockFallbackPipeline((msg) =>
      received.push(msg),
    );
