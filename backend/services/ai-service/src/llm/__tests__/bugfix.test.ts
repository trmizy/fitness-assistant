...
describe('E. Required sections and language lock validations', () => {
  ...
  it('routes mixed arms questions to a combined arms routine', () => {
    const profile = minimalProfile();
    const intent = inputParser.parse('vậy nếu tập tay trước và tay sau thì có những bài tập nào và bài nào hỗ trợ tốt cho tôi', profile);
...
describe('G. safetyGuard.check() — extended safety gate', () => {
  ...
  it('flags off-topic programming question (off_topic)', () => {
    const result = safetyGuard.check('viet cho toi mot doan code python de sort array');
  ...
  it('does NOT flag fitness+injury question as off_topic (false positive guard)', () => {
    const result = safetyGuard.check('toi bi dau goi, bai tap nao phu hop cho toi');
    assert.equal(result.type, 'safe', `Should be safe but got: ${result.type}`);
  });

  it('does NOT flag nutrition question as off_topic', () => {
    const result = safetyGuard.check('toi can an bao nhieu protein moi ngay de tang co');
    assert.equal(result.type, 'safe', `Should be safe but got: ${result.type}`);
  });

  it('evaluates safe workout question as safe', () => {
    const result = safetyGuard.check('cho toi lich tap 4 buoi tuan de tang co bap');
  ...
  it('answerValidator flags extreme protein-per-kg in general knowledge answer', () => {
    const rec = minimalRecommendation();
    rec.responseIntent = 'general_fitness_knowledge';
    const answer = 'To build muscle, eat 6g per kg of bodyweight in protein daily.';
    const result = answerValidator.validate(answer, rec);
    const hasExtremeProtein = result.warnings.some((w) => /extreme protein-per-kg/i.test(w));
    assert.equal(hasExtremeProtein, true, `Expected extreme protein warning, got: ${JSON.stringify(result.warnings)}`);
  });

  it('answerValidator does NOT flag normal protein advice in general knowledge', () => {
    const rec = minimalRecommendation();
    rec.responseIntent = 'general_fitness_knowledge';
    const answer = 'Aim for 1.6–2.2g per kg bodyweight in protein for muscle gain.';
    const result = answerValidator.validate(answer, rec);
    const hasExtremeProtein = result.warnings.some((w) => /extreme protein-per-kg/i.test(w));
...
describe('H. Legs / Shoulders / Core routines', () => {
  ...
  it('"Tôi muốn giảm mỡ bụng, tập bụng mỗi ngày" routes to core routine (not general_fitness_knowledge)', () => {
    const profile = minimalProfile();
    const intent = inputParser.parse('Tôi muốn giảm mỡ bụng, tập bụng mỗi ngày được không?', profile);
    assert.equal(intent.routeIntent, 'muscle_group_routine_request');
    const rec = recommendationEngine.recommend(profile, intent, 'vi');
    const answer = responseFormatter.format(rec, 'vi');
    assert.match(answer, /Plank/i);
  ...
  it('validator passes cleanly for legs routine — no false warnings', () => {
    ...
    const answer = responseFormatter.format(rec, 'vi');
    const result = answerValidator.validate(answer, rec, 'vi', profile);
    const hasRoutineWarnings = result.warnings.some((w) => /legs_|Missing required section/i.test(w));
    assert.equal(hasRoutineWarnings, false, `Unexpected warnings: ${JSON.stringify(result.warnings)}`);
  });
  ...
  it('validator passes cleanly for shoulders routine — no false warnings', () => {
    ...
    const answer = responseFormatter.format(rec, 'vi');
    const result = answerValidator.validate(answer, rec, 'vi', profile);
    const hasRoutineWarnings = result.warnings.some((w) => /shoulders_|Missing required section/i.test(w));
    assert.equal(hasRoutineWarnings, false, `Unexpected warnings: ${JSON.stringify(result.warnings)}`);
  });
  ...
  it('validator passes cleanly for core routine — no false warnings', () => {
    ...
    const answer = responseFormatter.format(rec, 'vi');
    const result = answerValidator.validate(answer, rec, 'vi', profile);
    const hasRoutineWarnings = result.warnings.some((w) => /core_|Missing required section/i.test(w));
    assert.equal(hasRoutineWarnings, false, `Unexpected warnings: ${JSON.stringify(result.warnings)}`);
  });
});
...
describe('I. Equipment-based exercise substitution', () => {
  ...
  const BARBELL_PATTERN = /^Barbell\b|^Back Squat$|^Bench Press$|^Overhead Press$|^Deadlift$|^Romanian Deadlift$/;
  ...
  it('full gym profile (empty equipment) → default plan keeps barbell exercises', () => {
    ...
    const allExercises = rec.workoutPlan!.days.flatMap((d) => d.exercises.map((e) => e.name));
    ...
    assert.equal(hasBarbellOrSquat, true, `Default plan should contain at least one barbell exercise. Got: ${allExercises.join(', ')}`);
  });
  ...
  it('padding exercises (Cable Crunch, Seated Calf Raise) are also filtered for dumbbell-only users', () => {
    ...
    const intent = inputParser.parse('Chi tiết lịch tập 5 ngày tăng cơ với dumbbell', profile);
    const rec = recommendationEngine.recommend(profile, { ...intent, detailMode: true }, 'vi');

    assert.ok(rec.workoutPlan !== undefined);
    for (const day of rec.workoutPlan!.days) {
      for (const ex of day.exercises) {
        assert.notEqual(ex.name, 'Cable Crunch', `"Cable Crunch" should be replaced for dumbbell users on ${day.day}`);
        assert.notEqual(ex.name, 'Seated Calf Raise', `"Seated Calf Raise" should be replaced for dumbbell users on ${day.day}`);
      }
    }
  });
  ...
  it('padding exercises (Cable Crunch, Seated Calf Raise) are also filtered for bodyweight-only users', () => {
    ...
    const intent = inputParser.parse('Chi tiết lịch tập 5 ngày tăng cơ bodyweight', profile);
    const rec = recommendationEngine.recommend(profile, { ...intent, detailMode: true }, 'vi');

    assert.ok(rec.workoutPlan !== undefined);
    for (const day of rec.workoutPlan!.days) {
      for (const ex of day.exercises) {
        assert.notEqual(ex.name, 'Cable Crunch', `"Cable Crunch" must not appear in bodyweight plan on ${day.day}`);
        assert.notEqual(ex.name, 'Seated Calf Raise', `"Seated Calf Raise" must not appear in bodyweight plan on ${day.day}`);
      }
    }
  });
});
    ...
    const history = [
      {
        ...
        answer: 'Tôi đề xuất lịch Push/Pull/Legs 5 ngày/tuần phù hợp với mục tiêu tăng cơ của bạn.',
      },
    ];
    const ctx = extractSessionContext(history);
    assert.match(ctx, /5 ngày\/tuần/, 'Should detect 5-day frequency from answer');
    assert.match(ctx, /Push\/Pull\/Legs/, 'Should detect PPL split from answer');
    assert.match(ctx, /giữ nhất quán/, 'Should include consistency instruction');
  });
    ...
    const history = [
      {
        ...
        answer: 'Bạn có thể tập push day nhưng cần tránh Overhead Press để bảo vệ vai.',
      },
    ];
    const ctx = extractSessionContext(history);
    assert.match(ctx, /chấn thương/, 'Should flag injury concern from question');
  });
    ...
    const history = [
      {
        ...
        answer: 'Bạn nên tránh overhead press và các bài vai trước cho đến khi hồi phục hoàn toàn.',
      },
    ];
    const ctx = extractSessionContext(history);
    assert.match(ctx, /overhead press|chấn thương/i, 'Should detect overhead press warning or injury from answer');
  });
...
describe('K. answerValidator — protein/calorie safety limits', () => {
  function generalRec(): RecommendationResult {
    return { ...minimalRecommendation(), responseIntent: 'general_fitness_knowledge' };
  }

  it('answer with 5g/kg protein → warns about extreme protein-per-kg', () => {
    const answer = 'You should eat 5g per kg bodyweight to maximise muscle gain.';
    const { warnings } = answerValidator.validate(answer, generalRec());
    assert.ok(warnings.some((w) => /protein-per-kg/i.test(w)), `Expected protein-per-kg warning. Got: ${warnings.join('; ')}`);
  });
  ...
  it('answer with 300 kcal (<800) → warns about out-of-range calories', () => {
    ...
    const { warnings } = answerValidator.validate(answer, generalRec());
    assert.ok(warnings.some((w) => /calorie value outside safe range/i.test(w)), `Expected calorie-range warning. Got: ${warnings.join('; ')}`);
  });

  it('answer with 7000 kcal (>6000) → warns', () => {
    const answer = 'Athletes sometimes eat 7000 kcal per day during heavy training phases.';
    const { warnings } = answerValidator.validate(answer, generalRec());
    assert.ok(warnings.some((w) => /calorie value outside safe range/i.test(w)));
  });
...
describe('L. answerValidator — unsafe phrases and emoji encoding', () => {
  ...
  it('"ignore pain" → safety warning', () => {
    const { warnings } = answerValidator.validate('You can ignore pain and keep training.', generalRec());
    assert.ok(warnings.some((w) => /unsafe phrase/i.test(w)));
  });

  it('"guarantee results" → safety warning', () => {
    const { warnings } = answerValidator.validate('This plan will guarantee results within 30 days.', generalRec());
    assert.ok(warnings.some((w) => /unsafe phrase/i.test(w)));
  });

  it('"train through injury" → safety warning', () => {
    const { warnings } = answerValidator.validate('It is fine to train through injury if the pain is mild.', generalRec());
    assert.ok(warnings.some((w) => /unsafe phrase/i.test(w)));
  });

  it('"không cần bác sĩ" → safety warning', () => {
    const { warnings } = answerValidator.validate('Bạn không cần bác sĩ, chỉ cần tập đủ là được.', generalRec());
    assert.ok(warnings.some((w) => /unsafe phrase/i.test(w)));
  });
  ...
  it('answer with emojis → no false calorie/protein warnings', () => {
    ...
    const answer = '💪 Great progress! Keep training at 2g per kg protein and ~2200 kcal/day. 🏋️ Stay consistent!';
    const { warnings } = answerValidator.validate(answer, generalRec());
    const calWarnings = warnings.filter((w) => /calorie value outside safe range|protein-per-kg/i.test(w));
    assert.equal(calWarnings.length, 0, `Expected no false warnings from emoji text. Got: ${calWarnings.join('; ')}`);
  });

  it('safe advice → no safety warnings', () => {
    const answer = 'Follow progressive overload, rest adequately, and consult a doctor if pain persists.';
    const { warnings } = answerValidator.validate(answer, generalRec());
    assert.ok(!warnings.some((w) => /unsafe phrase/i.test(w)));
  });
});
...
describe('M. intentRouter — supplement/recovery/deload/progress → general knowledge', () => {
  it('"supplement gì nên dùng?" → general_fitness_knowledge', () => {
    const { intent } = intentRouter.route('Tôi nên dùng supplement gì để tăng cơ?');
    assert.equal(intent, 'general_fitness_knowledge');
  });

  it('"thời gian recovery" → general_fitness_knowledge', () => {
    const { intent } = intentRouter.route('Thời gian nghỉ ngơi (recovery) tốt nhất giữa các buổi tập là bao lâu?');
    assert.equal(intent, 'general_fitness_knowledge');
  });

  it('"deload là gì?" → general_fitness_knowledge', () => {
    const { intent } = intentRouter.route('Deload là gì và khi nào nên deload?');
    assert.equal(intent, 'general_fitness_knowledge');
  });

  it('"theo dõi progress" → general_fitness_knowledge', () => {
    const { intent } = intentRouter.route('Làm thế nào để theo dõi progress hiệu quả nhất?');
    assert.equal(intent, 'general_fitness_knowledge');
  });

  it('"tại sao tôi không tăng cơ?" → general_fitness_knowledge', () => {
    const { intent } = intentRouter.route('Tại sao sau 3 tháng tập tôi vẫn không tăng cơ?');
    assert.equal(intent, 'general_fitness_knowledge');
  });
});
