...
function hasInjuryWarning(plan: StructuredCoachPlan): boolean {
  const text = [
    ...
    .join(" ")
    .toLowerCase();
  return /injury|pain|modify|avoid|replace|stop|medical|clinician|chan thuong|dau/.test(
    text,
  );
}
...
export function validateStructuredPlan(
  ...
  if (
    plan.nutrition_plan.target_calories != null &&
    plan.nutrition_plan.target_calories < 0
  ) {
    errors.push(
      issue("negative_calories", "Target calories cannot be negative."),
    );
  }
  ...
  if (weight && plan.nutrition_plan.protein_grams != null) {
    ...
    if (gramsPerKg < 1.2 || gramsPerKg > 2.8) {
      warnings.push(
        issue(
          "protein_outside_expected_range",
          "Protein target is outside the expected fitness coaching range.",
          "warning",
        ),
      );
    }
  ...
  if (
    availableDays != null &&
    plan.training_plan.days_per_week > availableDays
  ) {
    errors.push(
      issue(
        "available_days_exceeded",
        "Training days exceed available days in CoachContext.",
      ),
    );
  }

  if (coachContext.profile.experience_level === "beginner") {
    const maxDailySets = Math.max(
      0,
      ...plan.training_plan.weekly_schedule.map(totalSetsForDay),
    );
    if (maxDailySets > 24) {
      errors.push(
        issue("beginner_volume_too_high", "Beginner daily volume is too high."),
      );
    } else if (maxDailySets > 18) {
      warnings.push(
        issue(
          "beginner_volume_high",
          "Beginner daily volume is high and should be monitored.",
          "warning",
        ),
      );
    }
  ...
  if (coachContext.constraints.injuries.length > 0 && !hasInjuryWarning(plan)) {
    errors.push(
      issue(
        "injury_warning_missing",
        "Plan must include safety notes or modifications when injuries are present.",
      ),
    );
  }
  ...
  if (importantMissing.length > 0 && plan.missing_data_questions.length === 0) {
    errors.push(
      issue(
        ...
        "Plan must ask follow-up questions when important context is missing.",
      ),
    );
  }
...
export function legacyPlanContentToStructuredPlan(
  ...
  const protein = recommendProteinTarget(
    coachContext.profile.weight_kg,
    coachContext.goal.primary_goal,
    coachContext.profile.experience_level,
  );
  ...
  return {
    ...
    training_plan: {
      ...
      session_duration_minutes:
        coachContext.constraints.session_duration_minutes,
    ...
    missing_data_questions:
      coachContext.missing_fields.length > 0
        ? coachContext.missing_fields
            .slice(0, 3)
            .map((field) => `Please provide ${field}.`)
        : [],
    evidence_used: evidenceUsed,
  };
}
...
export function validateLegacyPlanContent(
  ...
  return validateStructuredPlan(
    legacyPlanContentToStructuredPlan(content, coachContext, evidenceUsed),
    coachContext,
  );
}
...
export function buildDeterministicFallbackPlan(
  ...
  const daysPerWeek = Math.max(
    1,
    Math.min(
      coachContext.goal.requested_days_per_week ??
        coachContext.constraints.available_days?.length ??
        3,
  ...
  return {
    ...
    training_plan: {
      ...
      session_duration_minutes:
        coachContext.goal.requested_session_duration_minutes,
        ...
        exercises: [
          {
            ...
            reps: "8-12",
            rest_seconds: 90,
            note: "Use a controlled range of motion.",
          },
          {
            name: "Chest Press or Push-up",
            sets: 3,
            reps: "8-12",
            rest_seconds: 90,
            note: "Stop if shoulder pain appears.",
          },
          {
    ...
    progression_rule:
      "Add reps first, then load, when all sets are completed with stable technique.",
    deload_rule:
      "Reduce sets by 30-40 percent for one week if fatigue, soreness, or performance decline persists.",
    safety_notes:
      coachContext.constraints.injuries.length > 0
        ? [
    ...
    missing_data_questions: coachContext.missing_fields
      .slice(0, 3)
      .map((field) => `Please provide ${field}.`),
    evidence_used: evidenceUsed,
  };
...
export function buildLegacyDeterministicPlanContent(
  ...
  const dayCount = Math.max(
    ...
    Math.min(
      options.daysPerWeek,
      coachContext.constraints.available_days?.length ?? options.daysPerWeek,
    ),
  );
  const fallbackExercises = options.allowedExercises.slice(
    0,
    Math.max(1, options.exercisesPerDay),
  );
  return {
    goal: options.goal,
    ...
    nutritionSummary:
      "Use calculated calorie and protein targets from CoachContext when available.",
  };
}
