...
export function estimateFatMass(
  weightKg?: number | null,
  bodyFatPercent?: number | null,
): number | null {
  const weight = finiteNumber(weightKg);
...
export function estimateLeanBodyMass(
  weightKg?: number | null,
  bodyFatPercent?: number | null,
): number | null {
...
export function recommendCalorieTarget(
  ...
  if (!tdeeRange.low_kcal || !tdeeRange.high_kcal) {
    return {
      target_kcal: null,
      range_kcal: null,
      rationale: "missing_tdee_inputs",
    };
  }

  const maintenance = Math.round(
    (tdeeRange.low_kcal + tdeeRange.high_kcal) / 2,
  );
  const normalizedGoal = String(goal || "").toLowerCase();
  if (
    normalizedGoal.includes("weight_loss") ||
    normalizedGoal.includes("fat_loss")
  ) {
    const target = maintenance - 400;
    return {
      target_kcal: target,
      range_kcal: [target - 150, target + 100],
      rationale: "moderate_deficit",
    };
  }
  if (normalizedGoal.includes("muscle_gain")) {
    ...
    return {
      target_kcal: target,
      range_kcal: [target - 100, target + 150],
      rationale: "controlled_surplus",
    };
  }
  if (normalizedGoal.includes("recomposition")) {
    return {
      target_kcal: maintenance - 100,
      range_kcal: [maintenance - 250, maintenance + 100],
      rationale: "near_maintenance_recomposition",
    };
  }
  return {
    target_kcal: maintenance,
    range_kcal: [tdeeRange.low_kcal, tdeeRange.high_kcal],
...
export function recommendProteinTarget(
  ...
  if (
    normalizedGoal.includes("weight_loss") ||
    normalizedGoal.includes("recomposition")
  )
    gramsPerKg = 1.8;
...
export function detectTrainingRisk(
  summary: TrainingSummaryContext,
  context?: Partial<CoachContext>,
): string[] {
  const flags: string[] = [];
  const experience = context?.profile?.experience_level;
  const maxSets = Object.values(summary.weekly_sets_by_muscle).reduce(
    (max, sets) => Math.max(max, sets),
    0,
  );
  if (experience === "beginner" && maxSets > 16)
    flags.push("beginner_volume_high");
  if ((summary.sessions_per_week ?? 0) >= 6)
    flags.push("high_training_frequency");
  if ((summary.average_rpe ?? 0) >= 9) flags.push("high_rpe");
  return flags;
}

export function detectNutritionRisk(
  summary: NutritionSummaryContext,
  context?: Partial<CoachContext>,
): string[] {
  const flags: string[] = [];
  const weight = context?.profile?.weight_kg ?? null;
  if (summary.avg_calories != null && summary.avg_calories < 1200)
    flags.push("very_low_calories");
  if (
    weight &&
    summary.avg_protein_g != null &&
    summary.avg_protein_g < weight * 1.2
  )
    flags.push("protein_below_fitness_target");
  return flags;
}
...
export function buildSafetyFlags(
  context: Pick<
    CoachContext,
    "profile" | "training_summary" | "nutrition_summary" | "constraints"
  >,
): string[] {
  return Array.from(
    new Set([
