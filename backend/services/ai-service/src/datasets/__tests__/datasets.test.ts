...
import {
  mapUsdaFoodDetail,
  type UsdaFoodDetail,
} from "../providers/usda/usda.mapper";
import {
  mapOffProductToDetail,
  type OffProduct,
} from "../providers/openfoodfacts/off.mapper";
import {
  mapWgerExercise,
  type WgerExercise,
} from "../providers/wger/wger.mapper";
import {
  mapFreeExercise,
  type FreeExerciseEntry,
} from "../providers/free-exercise-db/free-exercise.mapper";
...
const SAMPLE_WGER: WgerExercise = {
  ...
  name: "Barbell Squat",
  description:
    "<p>Stand with feet shoulder-width apart. Lower until thighs are parallel.</p>",
...
describe("L1. USDA beats Open Food Facts on macro totals", () => {
  it("merged detail carries USDA calories, not OFF calories", () => {
    ...
    assert.equal(
      usdaDetail.nutrition.calories,
      120,
      "USDA fixture has 120 kcal",
    );
    ...
    assert.equal(
      ...
      120,
      "merged calories must be USDA value (120)",
    );
    assert.equal(
      merged.nutrition.proteinG,
      22.5,
      "merged protein must be USDA value (22.5g)",
    );
    assert.equal(
      merged.nutrition.carbsG,
      0,
      "merged carbs must be USDA value (0g)",
    );
    assert.equal(
      merged.nutrition.fatG,
      2.6,
      "merged fat must be USDA value (2.6g)",
    );
  });
...
describe("L2. Open Food Facts enriches branding/barcode without overriding USDA macros", () => {
  ...
  it("merged detail has OFF brand when USDA has none", () => {
    ...
    assert.equal(
      merged.brand,
      "FitMeals",
      "brand from OFF is used when USDA has none",
    );
  });
  ...
  it("merged detail has OFF imageUrl", () => {
    ...
    assert.ok(
      merged.imageUrl?.includes("openfoodfacts"),
      "imageUrl from OFF is preserved",
    );
  });
...
describe("L3. Normalized exercise search across multiple sources", () => {
  it("wger mapper produces valid ExerciseItem", () => {
    ...
    assert.ok(
      !item.instructions?.[0]?.includes("<p>"),
  ...
  it("free-exercise-db mapper produces valid ExerciseItem", () => {
    ...
    assert.ok(
      item.images?.[0]?.startsWith("https://raw.githubusercontent.com"),
    );
  });
...
describe("L4. Disabled provider does not crash startup", () => {
  ...
  it("freeExerciseDbProvider.load returns empty array (no crash) when file does not exist", () => {
    ...
    const items = freeExerciseDbProvider.load(
      "/nonexistent/path/exercises.json",
    );
  ...
  it("freeExerciseDbProvider.load returns exercises from the sample seed file", () => {
    ...
    assert.ok(
      items.length >= 10,
      `Expected at least 10 sample exercises, got ${items.length}`,
    );
