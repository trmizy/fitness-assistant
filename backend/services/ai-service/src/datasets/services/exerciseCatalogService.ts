...
export const exerciseCatalogService = {
  ...
  async search(query: string, limit = 20): Promise<ExerciseSearchResult> {
    ...
    if (cfg.wger.enabled && allItems.length < limit) {
      try {
        const result = await wgerProvider.searchExercises(
          query,
          cfg.wger.baseUrl,
          limit,
        );
    ...
    if (cfg.exerciseDb.enabled && allItems.length < limit) {
      try {
        const result = await exerciseDbProvider.getByTarget(
          query,
  ...
  async getByBodyPart(
    ...
    if (cfg.exerciseDb.enabled && allItems.length < limit) {
      try {
        ...
        console.warn(
          "[exerciseCatalogService] ExerciseDB bodyPart lookup failed:",
          err,
        );
      }
    }
