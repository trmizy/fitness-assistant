...
async function embed(text: string): Promise<number[]> {
  const r = await axios.post(`${LLM_BASE_URL}/api/embeddings`, {
...
async function ingestEvidenceJsonl(): Promise<{ ok: number; failed: number }> {
  ...
  if (files.length === 0) { console.log('  ⚠  No .jsonl files in evidence/'); return { ok: 0, failed: 0 }; }

  let ok = 0, failed = 0;
  let batch: Array<{ id: string; vector: number[]; payload: Record<string, unknown> }> = [];

  for (const file of files) {
    const lines = fs.readFileSync(path.join(EVIDENCE, file), 'utf-8')
      .split('\n').filter(Boolean);
    ...
    for (const line of lines) {
      ...
      try { chunk = JSON.parse(line); } catch { failed++; continue; }

      if (!chunk.content || chunk.content.length < 20) { failed++; continue; }
      ...
      try {
        ...
        const pointId = stableId(chunk.id || `${file}_${chunk.chunk_index ?? ok}`);
...
function computeNhanesNorms(): EvidenceChunk[] {
  ...
  const lines = fs.readFileSync(mergedPath, 'utf-8').split('\n').filter(Boolean);
  ...
  const stats: Record<string, { weights: number[]; bmis: number[]; bfpcts: number[] }> = {
  ...
  for (const r of rows) {
    ...
    if (gender && weight && weight > 20 && weight < 300) stats[gender].weights.push(weight);
    if (gender && bmi   && bmi   > 10 && bmi   < 80   ) stats[gender].bmis.push(bmi);
    if (gender && bfpct && bfpct >= 0  && bfpct <= 70  ) stats[gender].bfpcts.push(bfpct);
  ...
  for (const g of ['M', 'F'] as const) {
    ...
    chunks.push({
      ...
      tags: ['NHANES', 'body weight', 'BMI', 'population norm', g === 'M' ? 'male' : 'female'],
  ...
  for (const g of ['M', 'F'] as const) {
    ...
    chunks.push({
      ...
      evidence_level: 'population_normative',
      source_url: 'https://www.cdc.gov/nchs/nhanes/',
      tags: ['NHANES', 'body fat', 'DXA', 'body composition', g === 'M' ? 'male' : 'female'],
      extraction_method: 'aggregated_statistics',
      content: [
...
async function ingestNhanesNorms(): Promise<{ ok: number; failed: number }> {
  ...
  if (norms.length === 0) {
    console.log('  ⚠  No NHANES norms computed (merged_2017.jsonl missing or empty)');
    return { ok: 0, failed: 0 };
  }

  let ok = 0, failed = 0;
  process.stdout.write(`  📊  NHANES body comp norms (${norms.length} summary chunks) `);

  for (const chunk of norms) {
    try {
      const vector = await embed([chunk.title, chunk.content, (chunk.tags ?? []).join(' ')].join(' '));
...
async function main() {
  ...
  try {
    ...
    try {
      ...
      console.error(`  ❌  Cannot connect to Qdrant at http://${QDRANT_HOST}:${QDRANT_PORT}: ${err.message}`);
      process.exit(1);
    }
  }
  ...
  try {
    ...
    console.error('    Start Ollama: ollama serve && ollama pull nomic-embed-text');
    process.exit(1);
  }
  ...
  if (INCLUDE_NHANES) console.log(`    NHANES norms:    ${nh.ok} ok / ${nh.failed} failed`);
  console.log(`    Collection: ${COLLECTION}`);

  if (ev.failed + nh.failed > 0) {
    console.warn(`⚠   ${ev.failed + nh.failed} items failed — check embedding service`);
    process.exit(1);
  }
}
