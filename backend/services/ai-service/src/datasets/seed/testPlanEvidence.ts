...
const allowedExercises: AllowedExerciseItem[] = [
  ...
  },
  {
    ...
    bodyPart: "UPPER_BODY",
    typeOfActivity: "STRENGTH",
    typeOfEquipment: "MACHINE",
    muscleGroupsActivated: ["CHEST", "TRICEPS"],
  },
  {
    id: "ex-lat-pulldown",
    ...
    typeOfActivity: "STRENGTH",
    typeOfEquipment: "MACHINE",
    muscleGroupsActivated: ["BACK", "BICEPS"],
  },
  {
    id: "ex-seated-row",
  ...
  },
  {
    ...
    typeOfActivity: "STRENGTH",
    typeOfEquipment: "MACHINE",
    muscleGroupsActivated: ["QUADS", "GLUTES"],
  },
  {
    id: "ex-rdl",
    ...
    typeOfActivity: "STRENGTH",
    typeOfEquipment: "BARBELL",
    muscleGroupsActivated: ["HAMSTRINGS", "GLUTES"],
  },
  {
    ...
    typeOfActivity: "STRENGTH",
    typeOfEquipment: "MACHINE",
    muscleGroupsActivated: ["SHOULDERS", "TRICEPS"],
  },
  {
    id: "ex-cable-crunch",
    ...
    typeOfActivity: "STRENGTH",
    typeOfEquipment: "CABLE",
    muscleGroupsActivated: ["ABS", "CORE"],
  },
];
...
const perDayCatalogs: DayExerciseCatalog[] = [
  {
    dayIndex: 0,
    dayGoal: "Ngực + Vai + Tay sau",
    focusMuscleGroups: ["CHEST", "SHOULDERS", "TRICEPS"],
    exercises: allowedExercises.slice(0, 2).concat(allowedExercises[6]),
  },
  {
    dayIndex: 1,
    dayGoal: "Lưng + Tay trước",
    focusMuscleGroups: ["BACK", "BICEPS"],
    exercises: allowedExercises.slice(2, 4),
  },
  {
    dayIndex: 2,
    dayGoal: "Chân + Mông",
    focusMuscleGroups: ["LEGS", "GLUTES"],
    exercises: allowedExercises.slice(4, 6),
  },
  {
    dayIndex: 3,
    dayGoal: "Full body + Core",
    focusMuscleGroups: ["FULL_BODY"],
...
async function main() {
  ...
  assert(
    ...
    "Expected adjustment_reason from body composition rules",
  );

  const evidenceDocs = await retriever.retrieveEvidence(
    analysis.evidenceQueries,
  );
  assert(
    evidenceDocs.length > 0,
    "Expected retrieved evidence from fitness_evidence",
  );

  const evidenceBundle = buildPlanEvidenceBundle(analysis, evidenceDocs);
  assert(
    evidenceBundle.evidence_used.length > 0,
    "Expected evidence_used from retriever metadata",
  );
  assert(
    evidenceBundle.evidence_used.every((item) =>
      item.source_url.startsWith("http"),
    ),
    "Expected real source_url values",
  );
  assert(
    evidenceBundle.adjustment_reason.some((item) =>
      /bodyFatPct|BMI|waist|muscle/i.test(item.metric),
    ),
    "Expected adjustment_reason metric to reference body metrics",
  );
  ...
  assert(
    parsed.ok,
    parsed.ok ? "unreachable" : `Plan JSON invalid: ${parsed.reason}`,
  );

  const content = attachEvidenceToPlanContent(
  ...
  assert(
    output.plan.nutrition || output.nutrition_suggestion,
    "Expected plan.nutrition or nutrition_suggestion",
  );
  ...
  assert(
    output.evidence_used.every((item: any) =>
      item.source_url?.startsWith("http"),
    ),
    "Expected real source_url in evidence_used",
  );

  const retrievedUrls = new Set(
    evidenceBundle.evidence_used.map((item) => item.source_url),
  );
  assert(
    output.evidence_used.every((item: any) =>
      retrievedUrls.has(item.source_url),
    ),
    "evidence_used contains a non-retrieved source_url",
  );
