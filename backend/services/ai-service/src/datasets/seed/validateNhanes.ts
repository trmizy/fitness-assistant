...
interface ColRange { min: number; max: number; nullable?: boolean }
...
function validateSpec(spec: ValidationSpec): Report {
  ...
  for (const [col, count] of Object.entries(rangeFails)) {
    if (count > 0) {
      report.issues.push(`OUT_OF_RANGE: ${col} failed on ${count} rows in ${spec.file}`);
    }
  }
  ...
  for (const [col, count] of Object.entries(nullCounts)) {
    if (count > rows.length * 0.3) {
      ...
      report.issues.push(`HIGH_NULL_RATE: ${col} is null in ${count}/${rows.length} rows (${((count/rows.length)*100).toFixed(0)}%) in ${spec.file} — may indicate non-DXA cohort`);
    }
  }
...
function validateEvidenceJsonl(): Report {
  ...
  for (const file of files) {
    const lines = fs.readFileSync(path.join(evidenceDir, file), 'utf-8').split('\n').filter(Boolean);
    for (const line of lines) {
      try {
        const chunk = JSON.parse(line);
        const required = ['id', 'title', 'source_type', 'category', 'content', 'evidence_level'];
        const missing = required.filter(k => !chunk[k]);
        if (missing.length > 0) {
          report.rejected++;
          report.issues.push(`MISSING FIELDS [${missing.join(',')}] in ${file}`);
...
async function main() {
  ...
  for (const spec of SPECS) {
    ...
    totalOk  += r.ok;
    totalBad += r.rejected;
    totalIssues += r.issues.filter(i => i.startsWith('FILE NOT FOUND') || i.startsWith('MISSING COLUMN') || i.startsWith('OUT_OF_RANGE')).length;
  }
  ...
  console.log(`${mStatus}  merged_2017.jsonl: ${merged.ok} ok / ${merged.rejected} rejected`);
  ...
  console.log(`${eStatus}  evidence/*.jsonl: ${evidence.ok} ok / ${evidence.rejected} rejected`);
  for (const iss of evidence.issues) console.log(`      ↳ ${iss}`);
  totalIssues += evidence.rejected;

  console.log('');
  console.log(`📊  Total: ${totalOk + merged.ok + evidence.ok} valid rows/chunks | ${totalBad + merged.rejected + evidence.rejected} rejected`);
