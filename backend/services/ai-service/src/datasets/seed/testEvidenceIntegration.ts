...
async function checkCollectionExists(): Promise<number> {
  ...
  try {
    ...
    const count =
      (info as any).vectors_count ?? (info as any).points_count ?? 0;
    return count;
  } catch {
    return -1;
  }
...
async function testRetrieval(query: string): Promise<void> {
  ...
  if (results.length === 0) {
    console.log(
      "   → No results returned (collection may be empty or score too low)",
    );
    return;
  }
  for (const r of results) {
    ...
    if (r.score < MIN_SCORE) {
      console.log(
        `   [score=${score} BELOW THRESHOLD ${MIN_SCORE}] ${p.title ?? "?"}`,
      );
      continue;
    }
    console.log(
      `   ✅ score=${score} | ${p.title ?? "?"} | type=${p.source_type ?? "?"} | level=${p.evidence_level ?? "?"}`,
    );
    if (p.content) {
      console.log(
        `      content preview: ${String(p.content).slice(0, 120)}...`,
      );
    }
  }
}
...
async function main() {
  ...
  console.log(
    `     maxDeficitKcal:    ${analysis.planConstraints.maxDeficitKcal ?? "not set"}`,
  );
  console.log(
    `     minProteinGKg:     ${analysis.planConstraints.minProteinGKg ?? "not set"}`,
  );
  console.log(
    `     resistanceRequired: ${analysis.planConstraints.resistanceRequired}`,
  );
