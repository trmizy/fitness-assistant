...
interface PaperMeta {
  id: string; title: string; file: string; source_url: string;
  source_type: 'guideline' | 'paper' | 'dataset';
  category: 'body_composition' | 'training' | 'nutrition' | 'bia_validation' | 'physical_activity';
  evidence_level: string; tags: string[];
}

const PAPERS: PaperMeta[] = [
  { id: 'espen-bia-1', title: 'ESPEN BIA Guidelines Part I (2019)', file: 'ESPEN_BIA_Part1_2019.pdf', source_url: 'https://espen.org/documents/BIA1.pdf', source_type: 'guideline', category: 'bia_validation', evidence_level: 'clinical_guideline', tags: ['BIA', 'body composition', 'impedance', 'ESPEN', 'clinical'] },
  { id: 'espen-bia-2', title: 'ESPEN BIA Guidelines Part II (2019)', file: 'ESPEN_BIA_Part2_2019.pdf', source_url: 'https://espen.org/documents/BIA2.pdf', source_type: 'guideline', category: 'bia_validation', evidence_level: 'clinical_guideline', tags: ['BIA', 'body composition', 'disease', 'sports', 'ESPEN'] },
  { id: 'hhs-pa-2018', title: 'Physical Activity Guidelines for Americans 2nd Edition (HHS 2018)', file: 'HHS_Physical_Activity_Guidelines_2018.pdf', source_url: 'https://health.gov/sites/default/files/2019-09/Physical_Activity_Guidelines_2nd_edition.pdf', source_type: 'guideline', category: 'physical_activity', evidence_level: 'government_guideline', tags: ['physical activity', 'exercise', 'HHS', 'aerobic', 'resistance training'] },
  { id: 'who-pa-2020', title: 'WHO Guidelines on Physical Activity (2020)', file: 'WHO_Physical_Activity_Guidelines_2020.pdf', source_url: 'https://iris.who.int/server/api/core/bitstreams/faa83413-d89e-4be9-bb01-b24671aef7ca/content', source_type: 'guideline', category: 'physical_activity', evidence_level: 'government_guideline', tags: ['physical activity', 'WHO', 'global', 'sedentary'] },
  { id: 'issn-protein-2017', title: 'ISSN Position Stand: Protein and Exercise (2017)', file: 'ISSN_Protein_Exercise_2017.pdf', source_url: 'https://doi.org/10.1186/s12970-017-0177-8', source_type: 'paper', category: 'nutrition', evidence_level: 'expert_consensus', tags: ['protein', 'muscle', 'g/kg', 'ISSN', 'sports nutrition'] },
  { id: 'issn-diets-2017', title: 'ISSN Position Stand: Diets and Body Composition (2017)', file: 'ISSN_Diets_Body_Composition_2017.pdf', source_url: 'https://doi.org/10.1186/s12970-017-0174-y', source_type: 'paper', category: 'nutrition', evidence_level: 'expert_consensus', tags: ['diet', 'fat loss', 'macros', 'ISSN', 'body composition'] },
  { id: 'issn-performance-2017', title: 'ISSN Position Stand: Nutritional Performance (2017)', file: 'ISSN_Nutritional_Performance_2017.pdf', source_url: 'https://doi.org/10.1186/s12970-017-0189-4', source_type: 'paper', category: 'nutrition', evidence_level: 'expert_consensus', tags: ['performance', 'creatine', 'caffeine', 'carbohydrate', 'ISSN'] },
  { id: 'issn-timing-2014', title: 'ISSN Position Stand: Nutrient Timing (2014)', file: 'ISSN_Nutrient_Timing_2014.pdf', source_url: 'https://doi.org/10.1186/1550-2783-11-20', source_type: 'paper', category: 'nutrition', evidence_level: 'expert_consensus', tags: ['nutrient timing', 'pre-workout', 'post-workout', 'anabolic window', 'ISSN'] },
];
...
function splitIntoChunks(text: string, chunkSize: number, overlap: number): string[] {
  ...
  while (start < text.length) {
    ...
    if (lastPeriod > chunkSize * 0.6 && end < text.length) chunk = chunk.slice(0, lastPeriod + 1);
...
async function main() {
  ...
  for (const paper of PAPERS) {
    ...
    if (fs.existsSync(outPath) && !FORCE) { console.log(`  ⏭  ${paper.id} — skipping`); skipped++; continue; }
    if (!fs.existsSync(pdfPath)) { console.log(`  ⚠  ${paper.file} not found — will use knowledge chunks`); missing++; }
    ...
    try {
      ...
      if (rawText && rawText.length > 200) {
        ...
        if (knowledgeChunks.length === 0) { console.log(` ⚠  no knowledge chunks defined`); missing++; continue; }
      ...
      if (registry[paper.id]) {
        registry[paper.id].processedPath = path.join('data', 'processed', 'evidence', paper.id + '.jsonl');
        registry[paper.id].processedAt = new Date().toISOString();
        registry[paper.id].chunkCount = total;
      } else {
        registry[paper.id] = {
          ...paper, localPath: path.relative(process.cwd(), pdfPath),
          processedPath: path.join('data', 'processed', 'evidence', paper.id + '.jsonl'),
          downloadedAt: null, processedAt: new Date().toISOString(), chunkCount: total,
        };
      }
  ...
  // Index file
  const index = PAPERS
    .filter(p => fs.existsSync(path.join(OUT_EVIDENCE, p.id + '.jsonl')))
    .map(p => ({ id: p.id, title: p.title, file: p.id + '.jsonl', category: p.category, evidence_level: p.evidence_level, tags: p.tags }));
  fs.writeFileSync(path.join(OUT_EVIDENCE, '_index.json'), JSON.stringify(index, null, 2), 'utf-8');
