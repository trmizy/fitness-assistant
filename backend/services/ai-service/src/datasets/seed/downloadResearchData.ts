...
interface SourceDef {
  ...
  url:         string;
  localPath:   string;
  category:    'nhanes' | 'bia_guideline' | 'physical_activity' | 'nutrition_paper';
...
const SOURCES: SourceDef[] = [
  ...
  // Physical Activity Guidelines
  {
    ...
    localPath: path.join(RAW_PAPERS, 'HHS_Physical_Activity_Guidelines_2018.pdf'),
  ...
  },
  {
    ...
    localPath: path.join(RAW_PAPERS, 'WHO_Physical_Activity_Guidelines_2020.pdf'),
    category: 'physical_activity',
    evidenceLevel: 'government_guideline',
  ...
  {
    id: 'issn-energy-2017',
    title: 'ISSN Position Stand: Nutritional Considerations for Performance (2017)',
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      await new Promise<void>((resolve, reject) => {
        ...
        const req = protocol.get(url, { timeout: 60000, headers: { 'User-Agent': 'fitness-assistant-research-pipeline/1.0' } }, (res) => {
          if (res.statusCode === 301 || res.statusCode === 302 || res.statusCode === 303 || res.statusCode === 307 || res.statusCode === 308) {
            file.close();
            fs.unlinkSync(dest + '.tmp');
            // Follow redirect
            downloadFile(res.headers.location!, dest, retries - attempt + 1).then(resolve).catch(reject);
            return;
          }
          ...
          finished(Readable.from(res).pipe(file)).then(() => {
      ...
      if (isLast) throw err;
      console.warn(`  ⚠  Attempt ${attempt}/${retries} failed: ${err.message} — retrying…`);
...
function loadSources(): Record<string, any> {
  if (fs.existsSync(SOURCES_FILE)) {
    try { return JSON.parse(fs.readFileSync(SOURCES_FILE, 'utf-8')); } catch { return {}; }
  }
  return {};
}
