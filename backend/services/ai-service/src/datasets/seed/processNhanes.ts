...
const BMX_COLS  = ['SEQN', 'BMXWT', 'BMXHT', 'BMXBMI', 'BMXWAIST', 'BMXHIP', 'BMXLEG', 'BMXARMC'];
const DXX_COLS  = ['SEQN', 'DXDTOTOT', 'DXDLEAN', 'DXDFAT', 'DXDBFAT', 'DXDTOBJ', 'DXDLIMB'];
const DXXAG_COLS = ['SEQN', 'DXDAGFAT', 'DXDANFAT', 'DXDGYFAT', 'DXXAGBFP', 'DXXANFAT', 'DXXGYFAT'];
...
const RENAME: Record<string, string> = {
  ...
  BMXWAIST: 'waist_cm', BMXHIP: 'hip_cm', BMXLEG: 'leg_cm', BMXARMC: 'arm_circ_cm',
  // DXX
  DXDTOTOT: 'total_mass_kg', DXDLEAN: 'lean_mass_kg', DXDFAT: 'fat_mass_kg',
  DXDBFAT: 'body_fat_pct', DXDTOBJ: 'bone_mineral_kg',
  // DXXAG
  DXDAGFAT: 'android_fat_kg', DXDANFAT: 'android_fat2_kg', DXDGYFAT: 'gynoid_fat_kg',
  DXXAGBFP: 'android_bfp', DXXANFAT: 'android_total_fat_kg', DXXGYFAT: 'gynoid_total_fat_kg',
  // DEMO
  RIDAGEYR: 'age_years', RIAGENDR: 'gender_code', RIDRETH3: 'race_code', DMDHHSIZ: 'household_size',
};
...
function renameRow(row: Record<string, any>): Record<string, any> {
  ...
  for (const [k, v] of Object.entries(row)) {
    ...
    if (k === 'SEQN') { out['SEQN'] = v; continue; }
    out[RENAME[k] ?? k.toLowerCase()] = v;
  }
  // Decode gender: 1=Male 2=Female
  if (out.gender_code != null) {
    out.gender = out.gender_code === 1 ? 'M' : out.gender_code === 2 ? 'F' : null;
  }
  return out;
}
...
interface FileSpec { id: string; file: string; keepCols: string[]; year: string }
const FILES: FileSpec[] = [
  { id: 'nhanes-2021-bmx',  file: 'BMX_L.xpt',   keepCols: BMX_COLS,   year: '2021' },
  { id: 'nhanes-2017-bmx',  file: 'BMX_J.xpt',   keepCols: BMX_COLS,   year: '2017' },
  { id: 'nhanes-2017-dxx',  file: 'DXX_J.xpt',   keepCols: DXX_COLS,   year: '2017' },
  { id: 'nhanes-2017-dxxag',file: 'DXXAG_J.xpt', keepCols: DXXAG_COLS, year: '2017' },
...
async function processFile(spec: FileSpec): Promise<Map<number, Record<string, any>>> {
  ...
  console.log(`  ✅  ${spec.file} → ${renamed.length} rows → ${path.basename(outCsv)}`);
...
async function main() {
  ...
  if (!fs.existsSync(mergedPath) || FORCE) {
    ...
    for (const seqn of allSeqn) {
      const merged = { SEQN: seqn, survey_year: 2017, ...(demo.get(seqn) ?? {}), ...(bmx.get(seqn) ?? {}), ...(dxx.get(seqn) ?? {}), ...(ag.get(seqn) ?? {}) };
  ...
  if (!fs.existsSync(readmePath)) {
    fs.writeFileSync(readmePath, `# NHANES Body Composition Data
...
`, 'utf-8');
  ...
  if (fs.existsSync(SOURCES_FILE)) {
    ...
    for (const spec of FILES) {
      ...
      if (registry[spec.id]) {
        registry[spec.id].processedPath = path.join('data', 'processed', 'nhanes', csvFile);
