...
function localizePlanNote(value: string): string {
  ...
  const exactMap: Record<string, string> = {
    ...
    "accessory lift":
      "Bài bổ trợ, tập kiểm soát và không đánh đổi form để tăng tạ.",
    "optional 10 min easy warm-up":
      "Có thể khởi động nhẹ 10 phút trước buổi tập.",
...
function normalizePlanCandidate(parsed: unknown): unknown {
  ...
  const wrapped =
    ...
    (Array.isArray(parsed.plan.weeklySchedule) ||
      Array.isArray(parsed.plan.weekly_schedule))
      ? parsed.plan
      : isRecord(parsed.workout_plan) &&
          (Array.isArray(parsed.workout_plan.weeklySchedule) ||
            Array.isArray(parsed.workout_plan.weekly_schedule))
        ? parsed.workout_plan
        : isRecord(parsed.workout) &&
            (Array.isArray(parsed.workout.weeklySchedule) ||
              Array.isArray(parsed.workout.weekly_schedule))
          ? parsed.workout
          : parsed;

  const normalized: UnknownRecord = { ...wrapped };

  const normalizedGoal = asNonEmptyString(
    wrapped.goal,
    asNonEmptyString(wrapped.name, "General fitness"),
  );
  normalized.goal = normalizedGoal;
  normalized.durationWeeks = asIntOrFallback(
    wrapped.durationWeeks ?? wrapped.duration,
    4,
  );
  ...
  normalized.weeklySchedule = schedule
    ...
    .map((dayItem, dayIndex) => {
      ...
      const dayGoal = asNonEmptyString(
        dayItem.goal,
        asNonEmptyString(dayItem.focus, "General training"),
      );

      const exercises = Array.isArray(dayItem.exercises)
        ? dayItem.exercises
        : [];
      const normalizedExercises = exercises
        .filter((exercise) => isRecord(exercise))
        .map((exercise, exerciseIndex) => ({
          exerciseId: asNonEmptyString(
            (exercise as any).exerciseId || (exercise as any).id,
            "",
          ),
          ...
          note:
            typeof exercise.note === "string" && exercise.note.trim()
              ? localizePlanNote(exercise.note)
              : undefined,
        }));
      ...
      return {
        ...
        cardio:
          typeof dayItem.cardio === "string" && dayItem.cardio.trim()
            ? localizePlanNote(dayItem.cardio)
            : undefined,
      };
    });

  const progression = Array.isArray(wrapped.progressionNotes)
    ? wrapped.progressionNotes
    : [];
  ...
  const recovery = Array.isArray(wrapped.recoveryNotes)
    ? wrapped.recoveryNotes
    : [];
  ...
  if (
    typeof wrapped.nutritionSummary === "string" &&
    wrapped.nutritionSummary.trim().length > 0
  ) {
...
export type SavePlanToWorkoutLogRequest = z.infer<
  typeof SavePlanToWorkoutLogRequestSchema
>;
...
export function parsePlanContent(
  raw: string,
): { ok: true; content: PlanContent } | { ok: false; reason: string } {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        return {
          ok: false,
          reason: "JSON is malformed and could not be repaired",
        };
      }
    }
  }
...
export function buildPlanPrompt(
  ...
  if (trainingLocation === "HOME") {
    locationNote =
      "Training location: HOME (bodyweight/dumbbell/resistance band only — NO barbell/machine/cable).\n";
  } else if (equipmentPreference === "MACHINE_ONLY") {
    locationNote =
      "Equipment: MACHINE ONLY (cable machines, weight stack machines, smith machine — NO free weights/barbell/dumbbell/bodyweight as main exercises).\n";
  } else {
    locationNote =
      "Equipment: GYM MIXED (machine + cable + barbell + dumbbell + bodyweight all available).\n";
  }
