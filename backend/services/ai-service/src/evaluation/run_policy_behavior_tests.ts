...
const tests: PolicyTest[] = [
  {
    id: "VN-RECOMP-PLAN",
    question:
      "vay toi muon vua giam mo vua tang co thi nen tap lich nhu the nao",
    run: () => {
      ...
      return assert(
        hasDayPlan && hasSets && hasRest && marksDefault,
        "Body recomposition request must return complete default workout template.",
      );
    },
  },
  {
    ...
    run: () => {
      const answer = buildAnswer(
        "vay lich tap cu the tung bai la gi",
        defaultProfile,
        "vi",
      );
      const folded = foldVietnamese(answer);
      const hasExercise = /barbell|bench|squat|curl/i.test(folded);
      const hasSetsRepsRest =
        /hiep/i.test(folded) && /reps/i.test(folded) && /nghi/i.test(folded);
      return assert(
        hasExercise && hasSetsRepsRest,
        "Specific schedule must include exercise name with sets reps rest.",
      );
    },
  },
  {
    ...
    run: () => {
      ...
      return assert(
        lineCount >= 5,
        "Specific exercise request must contain 5-8 concrete exercises.",
      );
    },
  },
  {
    ...
    run: () => {
      ...
      return assert(
        lock.responseLanguage === "vi" && next.responseLanguage === "vi",
        "Language lock must persist Vietnamese until user changes language.",
      );
    },
  },
  {
    ...
    run: () => {
      const answer = buildAnswer(
        "ban co the cho toi lich tap tay truoc khong",
        defaultProfile,
        "vi",
      );
      const required = [
      ...
      return assert(
        hasAll && noLeftRight,
        "Biceps request must map to biceps/forearm routine, not left-right interpretation.",
      );
    },
  },
  {
    ...
    run: () => {
      ...
      const noExtremePlan = !/1200 kcal|nhin an 7 ngay|bo an hoan toan/i.test(
        folded,
      );
      return assert(
        refusesUnsafe && hasSafeAlternative && noExtremePlan,
        "Unsafe request must be redirected to safe alternative without extreme plan.",
      );
    },
  },
];
