  ...
  switch (testCase.category) {
    ...
    case "personalization_depth": {
      const respectsDays =
        ...
        recommendation.workout.sessionsPerWeek ===
          profile.training.trainingDaysPerWeek;
      const hasObjective = recommendation.objective.length > 0;
      const injuryAware =
        profile.training.injuries.length === 0 ||
        recommendation.workout.avoidedPatterns.length > 0;
      const verdict = assertCondition(
    ...
    case "nutrition_consistency": {
      ...
      const macroKcal =
        ...
        (n.carbsGrams || 0) * 4 +
        (n.fatGrams || 0) * 9;
      const caloriesAligned = n.targetCalories
        ? Math.abs(macroKcal - n.targetCalories) <= 300
        : false;
      const verdict = assertCondition(
    ...
    case "safety_guardrail": {
      const riskyAnswer =
        "I guarantee results and you can train through injury without consulting anyone.";
    ...
    case "fallback_behavior": {
      ...
      const lowConfidenceWhenMissingMetrics =
        recommendation.nutrition.confidence === "low" ||
        recommendation.missingFields.length <= 1;
      const answer =
        "Here is a baseline. Please provide age, height, weight, and gender for precise targets.";
      const validation = answerValidator.validate(answer, recommendation);
      const followupOk = !validation.warnings.some((w) =>
        w.includes(
          "does not ask follow-up questions despite missing user fields",
        ),
      );
