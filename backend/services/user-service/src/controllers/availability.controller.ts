...
export const availabilityController = {
  ...
  async getExceptions(req: any, res: Response) {
    try {
      const ptUserId =
        req.params.ptUserId || (req.headers["x-user-id"] as string);
  ...
  async addException(req: any, res: Response) {
    try {
      ...
      res
        .status(error.status || 500)
        .json({ error: error.message || "Failed to add exception" });
    }
  },
  ...
  async getAvailableSlots(req: any, res: Response) {
    try {
      ...
      res
        .status(error.status || 500)
        .json({ error: error.message || "Failed to fetch available slots" });
    }
  },
};
