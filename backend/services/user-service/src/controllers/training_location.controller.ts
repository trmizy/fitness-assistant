...
async function assertApprovedPT(
  userId: string,
  res: Response,
): Promise<boolean> {
  ...
  if (!profile?.isPT) {
    res
      .status(403)
      .json({ error: "Chỉ PT đã được duyệt mới được quản lý địa điểm" });
    return false;
  }
  const app = await prisma.pTApplication.findFirst({
...
async function validateProvince(
  ...
  if (!province) {
    res
      ?.status(400)
      .json({ error: `provinceCode ${provinceCode} không hợp lệ` });
    return false;
  }
  if (wardCode) {
    ...
    if (!ward || ward.provinceCode !== provinceCode) {
      res?.status(400).json({
        error: `wardCode ${wardCode} không thuộc tỉnh ${provinceCode}`,
      });
      return false;
    }
  }
...
export const trainingLocationController = {
  getMyLocations: async (req: AuthRequest, res: Response) => {
    try {
      const locations = await trainingLocationRepository.findByPtUserId(
        req.user!.id,
      );
      res.json(locations);
    } catch {
  ...
  createLocation: async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (!provinceCode) {
        res.status(400).json({ error: "provinceCode bắt buộc" });
        return;
      }
      if (
        !(await validateProvince(
          Number(provinceCode),
          wardCode ? Number(wardCode) : null,
          res,
        ))
      )
        return;
      if (gymName && String(gymName).length > 120) {
        res.status(400).json({ error: "gymName tối đa 120 ký tự" });
        return;
      }
      if (addressLine && String(addressLine).length > 255) {
        res.status(400).json({ error: "addressLine tối đa 255 ký tự" });
        return;
      }
      ...
      res
        .status(500)
        .json({ error: err.message || "Failed to create training location" });
    }
  },
  ...
  updateLocation: async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const existing = await trainingLocationRepository.findByIdAndOwner(
        id,
        userId,
      );
      if (!existing) {
        res
          .status(403)
          .json({ error: "Không có quyền chỉnh sửa địa điểm này" });
        return;
      }
      ...
      if (provinceCode !== undefined || wardCode !== undefined) {
        if (!(await validateProvince(updateProvinceCode, updateWardCode, res)))
          return;
      }
      if (gymName !== undefined && String(gymName).length > 120) {
        res.status(400).json({ error: "gymName tối đa 120 ký tự" });
        return;
      }
      if (addressLine !== undefined && String(addressLine).length > 255) {
        res.status(400).json({ error: "addressLine tối đa 255 ký tự" });
        return;
      }
      ...
      res
        .status(500)
        .json({ error: err.message || "Failed to update training location" });
    }
  },
  ...
  deleteLocation: async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (!existing) {
        res.status(403).json({ error: "Không có quyền xóa địa điểm này" });
        return;
      }
