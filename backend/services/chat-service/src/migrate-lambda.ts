...
const CHAT_DATABASE_NAME = "fitness_assistant_chat";
const REFUSAL_MESSAGE = "Refusing to run Chat Service migrations against non-chat database.";
...
function assertChatDatabase(database: string): void {
  if (database !== CHAT_DATABASE_NAME) {
...
async function loadDatabaseSecret(): Promise<DatabaseSecret> {
  ...
  if (!secretId) {
    throw new Error("DATABASE_SECRET_ID is required for Chat Service migration Lambda.");
  }
...
async function ensureChatDatabaseExists(secret: DatabaseSecret): Promise<boolean> {
  assertChatDatabase(secret.database);
  ...
  try {
    ...
    await maintenanceClient.query(`CREATE DATABASE ${quotePostgresIdentifier(CHAT_DATABASE_NAME)}`);
    return true;
  } finally {
...
export async function handler(): Promise<MigrationResult> {
  ...
  const databaseCreated = await ensureChatDatabaseExists(secret);
