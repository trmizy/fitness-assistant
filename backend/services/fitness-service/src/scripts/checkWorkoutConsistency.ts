  ...
  for (const schedule of schedules) {
    ...
    const completedProgramExerciseIds = new Set(
      logged
        .filter(
          (exercise: any) =>
            exercise.programExerciseId && isExerciseComplete(exercise),
        )
    ...
    const totalSets = planned.length
      ...
      : logged.reduce(
          (sum: number, exercise: any) => sum + exercise.workoutSets.length,
          0,
        );
    const completedSets = logged.reduce(
      (sum: number, exercise: any) =>
        sum + exercise.workoutSets.filter((set: any) => set.completed).length,
      0,
    );
    ...
    if (JSON.stringify(expected) !== JSON.stringify(actual)) {
      const issue: Issue = {
        ...
        message:
          "Workout schedule cached progress does not match workout set completion.",
        expected,
        actual,
      };
      if (fixSafe) {
...
main()
  .catch((err) => {
    ...
    if (
      message.includes("program_exercise_id") ||
      message.includes("programExerciseId")
    ) {
      console.error(
