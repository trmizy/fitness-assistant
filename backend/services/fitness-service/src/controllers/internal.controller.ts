...
const HOME_EQUIPMENT = ['BODYWEIGHT', 'DUMBBELLS', 'RESISTANCE_BAND', 'KETTLEBELL', 'MEDICINE_BALL', 'FOAM_ROLLER'];
const MACHINE_ONLY_EQUIPMENT = ['MACHINE', 'CABLE'];
const MIXED_GYM_EQUIPMENT = ['MACHINE', 'CABLE', 'BARBELL', 'DUMBBELLS', 'BODYWEIGHT', 'KETTLEBELL'];

export const internalController = {
  async exercisesForAiPlans(req: Request, res: Response): Promise<void> {
    try {
      const { goal, bodyPart, equipment, typeOfActivity, limit, muscleGroup, trainingLocation, equipmentPreference } = req.query as Record<string, string>;
      ...
      if (muscleGroup) {
        ...
        if (g === 'WEIGHT_LOSS' || g === 'FAT_LOSS' || g === 'GIAM_MO' || g === 'LOSE_FAT') {
          mapped = {
            OR: [
          ...
          mapped = {
            OR: [
              { typeOfActivity: 'STRENGTH' },
              { typeOfEquipment: { in: ['BARBELL', 'MACHINE', 'DUMBBELLS', 'CABLE'] } },
            ],
          };
      ...
      logger.info(
        { goal, muscleGroup, trainingLocation, equipmentPreference, returned: exercises.length },
  ...
  async foodsForAiNutrition(_req: Request, res: Response): Promise<void> {
    try {
      ...
      const commonKeywords = [
        'chicken', 'egg', 'rice', 'oat', 'banana', 'salmon', 'tuna', 'beef',
        'pork', 'turkey', 'milk', 'yogurt', 'tofu', 'soy', 'beans', 'potato',
        'broccoli', 'spinach', 'apple', 'orange', 'peanut', 'almond', 'bread',
        'pasta', 'cheese', 'shrimp', 'fish',
      ];
      const excludedKeywords = [
        'oil', 'shortening', 'candy', 'candies', 'chewing gum', 'sweetener',
        'sugar substitute', 'gelatin dessert', 'beverage', 'alcoholic',
