...
async function seedProgramWithSchedule(
  ...
  const uniqueExerciseCount = options.duplicateGlobalExercise ? 1 : options.exerciseCount;
    ...
    data: {
      ...
      days: {
        create: {
          ...
          exercises: {
            create: Array.from({ length: options.exerciseCount }, (_unused, index) => ({
              exerciseId: options.duplicateGlobalExercise ? exerciseIds[0] : exerciseIds[index],
              order: index + 1,
              sets: 1,
              reps: 10,
            })),
...
test(
  ...
  async () => {
    ...
    try {
      ...
      assert.equal(startedSchedule.workout?.exercises.every((exercise) => Boolean(exercise.programExerciseId)), true);

      const expectedProgress = [25, 50, 75, 100];
      for (const [index, plannedExercise] of seeded.plannedExercises.entries()) {
        const result = await service.completeScheduleExercise(userId, seeded.schedule.id, plannedExercise.id);
      ...
      assert.equal(
        completedSchedule.workout?.exercises.every((exercise) =>
          exercise.workoutSets.length > 0 && exercise.workoutSets.every((set) => set.completed),
        ),
        true,
      );

      const idempotent = await service.completeScheduleExercise(userId, seeded.schedule.id, seeded.plannedExercises[0].id);
      ...
      assert.equal(duplicateLogs.every((row) => row._count.id === 1), true);

      await assert.rejects(
        () => service.completeScheduleExercise('other-user', seeded.schedule.id, seeded.plannedExercises[0].id),
      ...
      await assert.rejects(
        () => service.completeScheduleExercise(userId, seeded.schedule.id, wrongSeed.plannedExercises[0].id),
        (err: any) => err?.status === 404,
      );
    } finally {
      await deleteSeed(db, userId);
...
test(
  ...
  async () => {
    ...
    try {
      ...
      const first = await service.completeScheduleExercise(userId, seeded.schedule.id, seeded.plannedExercises[0].id);
      ...
      assert.equal(firstLogged.programExerciseId, seeded.plannedExercises[0].id);
      assert.equal(secondLogged.programExerciseId, seeded.plannedExercises[1].id);
      assert.equal(firstLogged.workoutSets.every((set) => set.completed), true);
      assert.equal(secondLogged.workoutSets.every((set) => set.completed), false);
    } finally {
      await deleteSeed(db, userId);
    }
  },
