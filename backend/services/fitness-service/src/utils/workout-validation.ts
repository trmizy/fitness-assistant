...
export function validateSets(sets: number): string | null {
  ...
  if (sets < WORKOUT_LIMITS.SETS_MIN) return `Sets must be at least ${WORKOUT_LIMITS.SETS_MIN}`;
  if (sets > WORKOUT_LIMITS.SETS_MAX) return `Sets cannot exceed ${WORKOUT_LIMITS.SETS_MAX}`;
  return null;
}

export function validateReps(reps: number): string | null {
  if (!Number.isInteger(reps)) return 'Reps must be a whole number';
  if (reps < WORKOUT_LIMITS.REPS_MIN) return `Reps must be at least ${WORKOUT_LIMITS.REPS_MIN}`;
  if (reps > WORKOUT_LIMITS.REPS_MAX) return `Reps cannot exceed ${WORKOUT_LIMITS.REPS_MAX}`;
  return null;
}
...
export function validateWeight(weight: number): string | null {
  ...
  if (weight > WORKOUT_LIMITS.WEIGHT_MAX) return `Weight cannot exceed ${WORKOUT_LIMITS.WEIGHT_MAX} kg`;
  return null;
}

export function validateExercisesCount(count: number): string | null {
  if (count < WORKOUT_LIMITS.EXERCISES_MIN) return 'At least one exercise is required';
...
export function validateDuration(minutes: number): string | null {
  ...
  if (minutes < WORKOUT_LIMITS.DURATION_MIN) return `Duration must be at least ${WORKOUT_LIMITS.DURATION_MIN} minute`;
  if (minutes > WORKOUT_LIMITS.DURATION_MAX) return `Duration cannot exceed ${WORKOUT_LIMITS.DURATION_MAX} minutes`;
  return null;
}
...
export function validateWorkoutDate(isoDate: string): string | null {
  ...
  const maxFuture = new Date(Date.now() + WORKOUT_LIMITS.DATE_MAX_FUTURE_DAYS * 86_400_000);
  if (d > maxFuture) {
