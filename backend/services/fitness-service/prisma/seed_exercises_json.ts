  ...
  for (const item of exercises) {
    try {
      ...
      if (item.category === "cardio") typeOfActivity = ExerciseType.CARDIO;
        ...
        typeOfActivity = ExerciseType.MOBILITY;
      ...
      if (equip.includes("barbell")) typeOfEquipment = EquipmentType.BARBELL;
        ...
        typeOfEquipment = EquipmentType.DUMBBELLS;
      else if (equip.includes("kettlebell"))
        typeOfEquipment = EquipmentType.KETTLEBELL;
      else if (equip.includes("machine"))
        typeOfEquipment = EquipmentType.MACHINE;
      else if (equip.includes("cable")) typeOfEquipment = EquipmentType.CABLE;
      else if (equip.includes("medicine ball"))
        typeOfEquipment = EquipmentType.MEDICINE_BALL;
      else if (equip.includes("foam roller"))
        typeOfEquipment = EquipmentType.FOAM_ROLLER;
      else if (equip.includes("body only"))
      ...
      if (["abdominals", "lower back"].includes(primary))
        bodyPart = BodyPart.CORE;
      else if (
        ["quadriceps", "hamstrings", "glutes", "calves"].includes(primary)
      )
...
async function seedFoods() {
  ...
  if (!csvPath) {
    ...
    console.warn(
      "   Generate it with: pnpm --filter @gym-coach/fitness-service db:process-usda",
    );
    return;
  }
  ...
  const rows: FoodRow[] = await new Promise<FoodRow[]>((resolve, reject) => {
    ...
    rl.on("line", (line) => {
      ...
      const [cal, prot, carb, fat, source] = rest
        .slice(lastQuote + 2)
        .split(",");
  ...
  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    ...
    if (total % 2000 === 0)
      console.log(`  Imported ${total}/${rows.length}...`);
  }
