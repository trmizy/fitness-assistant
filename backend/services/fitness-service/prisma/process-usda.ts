...
const TARGET_NUTRIENTS = new Set([NID_CALORIES, NID_PROTEIN, NID_FAT, NID_CARBS]);
...
async function main() {
  ...
  interface Nutrients { calories: number; protein: number; fats: number; carbs: number }
  ...
  await streamLines(path.join(DATA_DIR, 'food_nutrient.csv'), (cols) => {
    ...
    rowsRead++;
    if (rowsRead % 2_000_000 === 0) process.stdout.write(`  ${rowsRead / 1_000_000}M rows...\r`);

    const fdcId     = parseInt(cols[1], 10);
    const nutrientId = parseInt(cols[2], 10);
    const amount    = parseFloat(cols[3]);

    if (!foods.has(fdcId) || !TARGET_NUTRIENTS.has(nutrientId) || isNaN(amount)) return;
