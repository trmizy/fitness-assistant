...
const GYM_DATABASE_NAME = "fitness_assistant_gym";
const REFUSAL_MESSAGE = "Refusing to run Gym Service migrations against non-gym database.";
...
function assertGymDatabase(database: string): void {
  if (database !== GYM_DATABASE_NAME) {
...
async function loadDatabaseSecret(): Promise<DatabaseSecret> {
  ...
  if (!secretId) {
    throw new Error("DATABASE_SECRET_ID is required for Gym Service migration Lambda.");
  }
...
async function ensureGymDatabaseExists(secret: DatabaseSecret): Promise<boolean> {
  assertGymDatabase(secret.database);
  ...
  try {
    ...
    await maintenanceClient.query(`CREATE DATABASE ${quotePostgresIdentifier(GYM_DATABASE_NAME)}`);
    return true;
  } finally {
...
export async function handler(): Promise<MigrationResult> {
  ...
  const databaseCreated = await ensureGymDatabaseExists(secret);
