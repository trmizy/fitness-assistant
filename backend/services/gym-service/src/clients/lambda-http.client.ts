  ...
  const event = {
    ...
    requestContext: {
      http: {
        ...
        path: params.path,
        sourceIp: "gym-service",
        userAgent: "gym-service-lambda-invoke",
      },
      requestId: `gym-${Date.now()}`,
    },
...
export function throwForLambdaHttpError(result: { statusCode: number; body: any }): void {
  ...
  const err: any = new Error(result.body?.error?.message || result.body?.error || `Lambda HTTP call failed with ${result.statusCode}`);
