...
test('production + default secret → throws', () => {
  assert.throws(
    () => validateInternalSecret('dev_internal_service_secret_change_in_production', 'production'),
...
test('production + valid secret (32+ chars) → does not throw', () => {
  assert.doesNotThrow(() =>
    validateInternalSecret('a-valid-production-secret-that-is-long!', 'production'),
  );
});

test('development + default secret → does not throw', () => {
  assert.doesNotThrow(() =>
    validateInternalSecret('dev_internal_service_secret_change_in_production', 'development'),
  );
});
