...
~~The next implementation task is now: P3.2 — ACTIVITY HEATMAP
(§ 22)~~ — **done.** All 5 real day states derived from existing
columns, zero new schema — see § 46's "Activity heatmap" row for full
evidence, including a real volume-calculation bug found and fixed via
this pass's own integration test.

~~The next implementation task is now: P3.3 — EXERCISE PROGRESS CHARTS
(§ 23)~~ — **done.** All 7 trend types computed from real `WorkoutSet`
...
~~The next implementation task is now: P3.5 — PLANNED VS ACTUAL TRAINING
VOLUME (§ 25)~~ — **done.** `getCycleReport` gained a mode-gated
planned-vs-actual comparison built from real `WorkoutProgramExercise`
targets and real logged `WorkoutSet` data — see § 46's "Planned vs
actual" row for full evidence, including a real schema gap found
(`DISTANCE_TIME` has no plannable distance column).

~~The next implementation task is now: P3.6 — EXERCISE HISTORY DETAIL
PAGE (§ 26)~~ — **done. This closes out P3 (visualization) in full,
6/6.** Extended P3.3's existing chart page in place rather than a
second page for the same exercise — almost every §26 bullet was already
...
**As of 2026-08-28: P0 is DONE/READY, all 9 P1-tier items are DONE, P2
...
complete — and P4 is 2/2 done.** The full 3-provider import trio
...
than attempt unverifiably. P4 polish is now complete for roadmap-tracked web/backend items; the only remaining roadmap entries are the environment-blocked native health integrations.
...
| Superset/grouping | P1 | **DONE (true interleaved active execution)** | New `WorkoutProgramExerciseGroup`/`GroupMember` tables (program-day planning concept only — never touches `WorkoutExercise`/`WorkoutSet`, zero risk to logged history). Original scoped-MVP pass completed exercises sequentially while applying the roadmap's two rest fields; the 2026-08-28 advanced-methods pass has now upgraded active execution to true interleaved round-major navigation while keeping those same persisted group/rest semantics. Grouping UI lives in the existing day-edit mode (multi-select → "Tạo nhóm", with editable rest-duration inputs); active-session shows a "Superset · Bài N/M" badge. `createExerciseGroup` re-sequences the day so members become contiguous by construction, even if selected non-adjacently. See `docs/features/SUPERSET_GROUPING_IMPACT_ANALYSIS.md` for the full scope decision and a pre-existing gap it flagged (`WorkoutProgramExercise.restSeconds` still unread by the timer for non-grouped exercises — out of scope, not fixed). Backend integration 4/4 (new) + 18/18 pre-existing unaffected, unit 7/7 (new, the pure rest-duration function), E2E 2/2 (new, `38-superset-exercise-grouping.spec.ts`), full regression re-run 33/33 (specs 27,29-38, `READY FAIL=0 total=540`) — an initial run hit 1 flaky failure from the gateway's own `/auth/*` rate limiter being cumulatively exhausted by this session's E2E volume across 4 milestones in one day; waited out properly (single `Retry-After` check + one passive wait, no further probing) and the clean re-run confirmed it was rate-limit pressure, not a regression. 2026-08-28 advanced-methods pass upgraded this from scoped MVP to true interleaved active execution: A1 -> B1 -> A2 -> B2, round labels, existing set IDs only, and rest timer restart fixed. Unit 11/11, frontend build pass, E2E regression `38-superset-exercise-grouping.spec.ts` 2/2 pass (`e2e_202608280751230`), new E2E `55-true-interleaved-superset.spec.ts` 1/1 pass (`e2e_202608280752400`). See `docs/ADVANCED_TRAINING_METHODS_IMPLEMENTATION_REPORT.md`. |
...
| Exercise-history detail | P3 | **DONE (extends P3.3's page)** | Per exercise: recent sessions, previous actual sets, progression decisions, PRs, e1RM, charts, logging-mode-specific records, notes, substitutions if available. Real audit finding: almost every §26 bullet was already served by prior work — `statsService.getExerciseProgress` (P3.3) for charts/e1RM/logging-mode records, `workoutRepository.findRecentCompletedSessionsForExercise` (pre-existing, built for `exercise-progression.engine.ts`) for recent sessions/previous actual sets, `workoutService.getExerciseProgression` (pre-existing, deterministic, already live for smart set prefill) for progression decisions, `GET /exercises/:id/substitute` (pre-existing) for substitutions. This milestone is almost entirely COMPOSITION, not new logic — the only genuinely new pieces are the thin aggregator service and a new pure `derivePersonalRecord` (reduces over P3.3's own already-correct per-session data to find the current best, since P3.3's own audit had already found `getPRs`/`findExercisePRs` unsuitable — not re-reused here either). **Scope decision**: extended P3.3's existing `/client/exercise-progress/:exerciseId` page IN PLACE rather than a second page for the same exercise — §26 itself frames this as "a better place" (singular) for deep history, so one coherent destination per exercise, same entry point, not a fragmented second URL. New `GET /workouts/exercises/:exerciseId/history` (composes charts+PR+recent-sessions+progression into one page-ready response; substitutions stay a separate frontend request by design, since a slow/failed lookup must never block the rest of the page). `WorkoutExercise.notes` was already a real column, simply never selected by the recent-sessions query before — fixed additively (no schema change). See `docs/features/EXERCISE_HISTORY_DETAIL_IMPACT_ANALYSIS.md`. Unit 7/7 (new, `exercise-history.util.test.ts`). Backend integration 2/2 (new, `exercise-history.service.integration.test.ts` — real 2-session history, correct newest-first ordering, real note preservation, and personal-record derivation correctly picking the higher-e1RM session over the merely-more-recent one; 404 for a nonexistent exercise). `tsc --noEmit` clean, frontend `npm run build` clean. E2E 1/1 (new, `52-exercise-history-detail.spec.ts` — real seeded 2-session history renders the real PR badge and real recent-sessions list with a real note, through the extended page). Regression: `49-exercise-progress-charts.spec.ts` (1/1, P3.3's own spec against the same now-extended page) + `33-smart-set-prefill.spec.ts` (5/5, shares the `WorkoutLogPage.tsx` entry-point modal) — 6/6 still passing. **This closes out P3 (visualization) in full — 6/6 done.** |
...
| PWA/installability polish | P4 | **DONE** | Dependency-free manual PWA layer for `frontend/web`: manifest, restored icon metadata, SVG + 192/512 PNG install icons, production-only service-worker registration, offline navigation shell, same-origin static-asset runtime cache, explicit update toast (`SKIP_WAITING` only after user action), old-cache cleanup, and hard cache bypass for `/api`, `/socket.io`, `/chat-socket.io`, and non-GET requests. See `docs/features/PWA_INSTALLABILITY_IMPACT_ANALYSIS.md`. Unit 5/5 (`pwa-registration.utils.test.ts`), frontend `npm run build` clean, `node --check public/sw.js` clean, build artifact presence verified, E2E 1/1 (`54-pwa-installability.spec.ts`) passed against production preview in run `e2e_202608280704534`. |
